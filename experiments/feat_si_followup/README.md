...
## Hypothesis
(a) The effect is concentrated in the high-SI decile's underperformance (short leg); (b) the information persists (an older file still predicts), i.e. it is a state, not an information-flow effect; (c) the book's net IR falls sharply when the highest-SI fifth of its shorts pays x3 / x5 / x10 the assumed borrow (Reddit sec 2.10: "the reason you want to short is correlated with the borrow becoming expensive"); (d) capping the short book at SI ratio <= 10% (squeeze / borrow filter, as docs/RESEARCH.md Part I sec 3.7 suggests) keeps most of the gain. No adopt rule beyond the parent's; this is a robustness study.

## Research origin
Own follow-up; docs/RESEARCH.md Part II sec 2.10 (borrow adverse selection) and docs/RESEARCH.md Part I sec 3.7 (short interest "more valuable as a short-selection / squeeze-risk filter").
