...
DESIGN. Frozen experiments/largecap harness (copied in below); FINRA mid-month settlement files (cache/finra_short_interest/, downloaded on first run; only files published before month-end are used); frozen composite. (1) decile tables of next-month excess return by SI-ratio and days-to-cover decile,
D1 (lowest) vs D10 vs the universe mean; (2) IC of the signal built from files 0/1/2/3/6 months older; (3) LP `lc_t10` portfolios: composite, +days-to-cover, +SI ratio, +both (8th/9th equal-weight group), and the composite (+dtc) with shorts barred where SI ratio > 10%; for each, net IR when the highest-SI fifth of the short book pays x1/3/5/10 the assumed borrow.

Run:  .venv/bin/python experiments/feat_si_followup/feat_si_followup.py [--smoke]      (network on first run to fill the FINRA cache)
Outputs (output/): results.json, summary.csv, decile_{si_ratio,days_to_cover}.csv, run.log, portfolio_{holdings,returns}_<variant>_lc_si.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/feat_si_followup/
OUTPUT = HERE / "output"
...
def truncation_test(builder, panel, n_dates=3, seed=0, tol=1e-9):
    ...
    for t in dates:
        ...
        for k, W in full.items():
            ...
            n_cmp += int(both.sum())
    ok = all(v <= tol for v in worst.values()) and n_cmp > 0  # a test that compared nothing has not passed
...
#   short_cap_col, short_cap_val   shorts only allowed where the preds-frame column short_cap_col <= short_cap_val (e.g. a short-interest ratio cap)
# ---------------------------------------------------------------------------
...
def build_portfolio(preds, cfg):
    ...
    for month, grp in preds.groupby("target_month"):
        ...
        if cfg.get("short_cap_col") is not None:
            cap_ok = (g[cfg["short_cap_col"]].fillna(0.0).to_numpy() <= cfg["short_cap_val"])
            allow_short_price = cap_ok if allow_short_price is None else (allow_short_price & cap_ok)
...
# ---------------------------------------------------------------------------
# FINRA bi-monthly short interest. EXTERNAL DATA: cache/finra_short_interest/shrt<YYYYMMDD>.csv (one ~2 MB file per settlement date, downloaded on first run from
# cdn.finra.org). Timing: for characteristic month t only the MID-month settlement file (15th or the prior business day, published ~7 business days later, i.e. before month-end)
# is used, never the month-end file (published after month-end). Symbols are matched to permno through the panel's `ticker` at month t (unmatched = NaN). Shares outstanding
# from the panel (`shares`, millions).
# ---------------------------------------------------------------------------
import urllib.request

FINRA_DIR = CACHE / "finra_short_interest"


def mid_month_settlement(month_end):
...
def fetch_finra(d):
    """Download (once) the file for settlement date d, trying up to 4 earlier business days (holidays). Returns a DataFrame or None."""
    FINRA_DIR.mkdir(parents=True, exist_ok=True)
    for back in range(0, 5):
        dd = d - pd.tseries.offsets.BDay(back)
        f = FINRA_DIR / f"shrt{dd:%Y%m%d}.csv"
        if not f.exists():
            try:  # (urllib's default user agent is rejected with HTTP 403 by the FINRA CDN)
                req = urllib.request.Request(f"https://cdn.finra.org/equity/otcmarket/biweekly/{f.name}", headers={"User-Agent": "fiam-2026-research-bot"})
                with urllib.request.urlopen(req, timeout=60) as r:
                    f.write_bytes(r.read())
            except Exception:
                f.unlink(missing_ok=True)
                continue
        return pd.read_csv(f, sep="|", usecols=["symbolCode", "currentShortPositionQuantity", "daysToCoverQuantity", "settlementDate"],
                           dtype={"symbolCode": str})
    return None


def si_builder_factory(files):
    def builder(p):
        R = wide(p, "ret")
        SH = wide(p, "shares")
        nan = lambda: pd.DataFrame(np.nan, index=R.index, columns=R.columns)
        sir, dtc = nan(), nan()
        for t in R.index[R.index >= pd.Timestamp("2020-06-30")]:
            f = files.get(t)
            if f is None:
                continue
            rows = p[p["eom"] == t].drop_duplicates("ticker")
            m = rows.merge(f, left_on="ticker", right_on="symbolCode", how="inner")
            m = m[m["shares"] > 0]
            sir.loc[t, m["permno"].to_numpy()] = (m["currentShortPositionQuantity"] / (m["shares"] * 1e6)).to_numpy()
            dtc.loc[t, m["permno"].to_numpy()] = m["daysToCoverQuantity"].to_numpy()
        return {"si_ratio": sir, "si_change_3m": sir - sir.shift(3), "days_to_cover": dtc}
    return builder


def make_blocks(ctx):
    months = [t for t in MONTH_INDEX if t >= pd.Timestamp("2020-06-30") and t <= pd.Timestamp("2026-07-31")]
    files = {}
    for t in months:
        files[t] = fetch_finra(mid_month_settlement(t))
    print(f"  FINRA files loaded for {sum(f is not None for f in files.values())}/{len(months)} months", flush=True)
    panel = load_panel(["ret", "shares", "ticker"], ctx)
    builder = si_builder_factory(files)
    trunc = truncation_test(builder, panel, n_dates=2)
    W = builder(panel)
    v = {k: wide_to_meta(w, ctx.meta) for k, w in W.items()}
    print(f"  short-interest match rate on universe test rows: {float(np.isfinite(v['si_ratio'])[ctx.um].mean()):.3f}", flush=True)
    blocks = {
        "si_ratio": {"raw": v["si_ratio"], "sign": -1, "note": "short interest / shares outstanding at the mid-month settlement (heavily shorted stocks underperform)"},
        "si_change_3m": {"raw": v["si_change_3m"], "sign": -1, "note": "3-month change in the short-interest ratio ('surprise in short interest negatively predicts', informed shorts)"},
        "days_to_cover": {"raw": v["days_to_cover"], "sign": -1, "note": "FINRA days-to-cover (short interest / average daily volume)"},
    }
    return blocks, {}, trunc


# ---------------------------------------------------------------------------
# Driver: follow-up on the FINRA short-interest result (leg attribution, staleness, borrow stress, short-side filter)
# ---------------------------------------------------------------------------


def decile_table_by(base_obj, sig, n=10):
    um = base_obj.ctx.um & np.isfinite(sig)
    d = pd.DataFrame({"m": base_obj.months[um], "s": sig[um], "y": base_obj.y[um]})
    d["dec"] = d.groupby("m")["s"].transform(lambda s: pd.qcut(s.rank(method="first"), n, labels=False)) + 1
    mk = d.groupby("m")["y"].mean()  # equal-weight universe mean each month (all names with a signal)
    t = d.groupby(["m", "dec"])["y"].mean().unstack()
    return 100 * t.mean(), 100 * (t.sub(mk, axis=0)).mean(), t
...
def main():
    ...
    base_obj = Base(ctx)
    months = [t for t in MONTH_INDEX if t >= pd.Timestamp("2020-06-30") and t <= pd.Timestamp("2026-07-31")]
    files = {t: fetch_finra(mid_month_settlement(t)) for t in months}
    panel = load_panel(["ret", "shares", "ticker"], ctx)
    W = si_builder_factory(files)(panel)
    sir = wide_to_meta(W["si_ratio"], ctx.meta)
    dtc = wide_to_meta(W["days_to_cover"], ctx.meta)
    res = {"match_rate_universe": float(np.isfinite(sir)[ctx.um].mean())}
    print(f"match rate {res['match_rate_universe']:.3f}; universe SI ratio quantiles: {np.nanquantile(sir[ctx.um], [.1, .5, .9, .99]).round(3).tolist()}; DTC: {np.nanquantile(dtc[ctx.um], [.1, .5, .9, .99]).round(2).tolist()}", flush=True)

    # (1) decile tables: next-month excess return by SI-ratio and days-to-cover decile (universe rows)
    tabs = {}
    for name, sig in (("si_ratio", sir), ("days_to_cover", dtc)):
        mean, rel, t = decile_table_by(base_obj, sig)
        tabs[name] = {"mean_ret_pct": mean.round(3).to_dict(), "ret_minus_universe_mean_pct": rel.round(3).to_dict(),
                      "d1_minus_d10_pct": float(mean.iloc[0] - mean.iloc[-1]),
                      "share_of_d1_minus_d10_from_short_leg(D10 below universe)": float(-rel.iloc[-1] / (rel.iloc[0] - rel.iloc[-1])) if (rel.iloc[0] - rel.iloc[-1]) != 0 else None}
        t.mean().rename("mean").to_csv(OUT / f"decile_{name}.csv")
        print(f"  {name}: mean next-month excess return by decile (%/m, D1 = lowest): {mean.round(2).tolist()}; D1-D10 {tabs[name]['d1_minus_d10_pct']:+.2f}; D10 minus universe {rel.iloc[-1]:+.2f}, "
              f"D1 minus universe {rel.iloc[0]:+.2f}", flush=True)
    res["decile_tables"] = tabs

    # (2) staleness: IC of the signal built from the file k months older (does the information decay slowly = persistent state, or fast = timing?)
    stale = {}
    for k in (0, 1, 2, 3, 6):
        for name in ("si_ratio", "days_to_cover"):
            v = wide_to_meta(W[name].shift(k), ctx.meta)
            ic = base_obj.ic(-urank(v, ctx))
            stale[f"{name}_lag{k}m"] = {"ic": float(ic.mean()), "t": tstat(ic)}
    res["staleness"] = stale
    print("  staleness (IC of -rank, older file):", json.dumps({k: round(v["ic"], 4) for k, v in stale.items()}), flush=True)

    # (3) portfolios: composite, +dtc, +sir; short-side SI cap (a squeeze / borrow filter); and borrow stress on the highest-SI shorts
    ctx.meta["sir"] = sir
    b_d, b_s = -urank(dtc, ctx), -urank(sir, ctx)
    G7sum = base_obj.G7.sum(axis=1)
    sc = {"comp": base_obj.comp, "comp_plus_dtc": (G7sum + b_d) / 8, "comp_plus_sir": (G7sum + b_s) / 8, "comp_plus_dtc_sir": (G7sum + b_d + b_s) / 9}
    base = make_variants(FLOOR)["lc_t10"]
    cap = {**base, "short_cap_col": "sir", "short_cap_val": 0.10}
    V = {"comp": (base, "comp"), "comp_plus_dtc": (base, "comp_plus_dtc"), "comp_plus_sir": (base, "comp_plus_sir"), "comp_plus_dtc_sir": (base, "comp_plus_dtc_sir"),
         "comp_shortcap10": (cap, "comp"), "comp_plus_dtc_shortcap10": (cap, "comp_plus_dtc")}
    rows, nets = [], {}
    for name, (cfg, sk) in V.items():
        s = np.asarray(sc[sk], dtype=float)
        row, net, r = run_variant(base_obj.preds(s), name, cfg, "si")
        fr = pd.read_csv(OUT / f"portfolio_returns_{name}_{TAG}_si.csv", parse_dates=["target_month"]).set_index("target_month")
        hold = pd.read_csv(OUT / f"portfolio_holdings_{name}_{TAG}_si.csv", parse_dates=["target_month"])
        hold = hold.merge(ctx.meta[["permno", "target_month", "sir"]], on=["permno", "target_month"], how="left")
        sh = hold[hold["weight"] < 0].copy()
        sh["b"] = cost_bps(sh["raw_me"].to_numpy())[1] / 1e4 / 12.0 * sh["weight"].abs()
        sh["hi"] = sh["sir"] >= sh.groupby("target_month")["sir"].transform(lambda x: x.quantile(0.8))  # highest-SI fifth of the short book
        stress = {}
        for m in (1, 3, 5, 10):
            adj = sh["b"].where(~sh["hi"], sh["b"] * m).groupby(sh["target_month"]).sum().reindex(fr.index).fillna(0.0)
            ret = fr["port_excess_ret"] - fr["trade_cost"] - adj
            act = ret - 0.04 / 12
            stress[m] = float(np.sqrt(12) * act.mean() / act.std())
        row = {"variant": name, "avg_short_sir": float(sh["sir"].mean()), "avg_long_sir": float(hold[hold["weight"] > 0]["sir"].mean()), "share_short_weight_in_top_SI_fifth": float(sh.loc[sh["hi"], "weight"].abs().sum() / sh["weight"].abs().sum()),
               **{f"ir_net_hiSI_short_borrow_x{m}": v for m, v in stress.items()}, **{k: row[k] for k in ("ir_gross", "ir_net", "beta", "max_dd_net_pct", "cy2025_net", "avg_n_positions", "short_median_mcap_musd")}}
        rows.append(row)
        nets[name] = net
        print(f"  [{name:26s}] IR gross {row['ir_gross']:+.2f} net {row['ir_net']:+.2f} | short-book avg SI ratio {row['avg_short_sir']:.3f} | net IR with the highest-SI fifth of shorts paying "
              f"x3/x5/x10 borrow: {row['ir_net_hiSI_short_borrow_x3']:+.2f}/{row['ir_net_hiSI_short_borrow_x5']:+.2f}/{row['ir_net_hiSI_short_borrow_x10']:+.2f}", flush=True)
    summ = pd.DataFrame(rows)
    summ["paired_net_t_vs_comp"] = summ["variant"].map(lambda k: paired_net(nets[k], nets["comp"])["t"] if k != "comp" else np.nan)
    summ.to_csv(OUT / "summary.csv", index=False)
    res["portfolios"] = rows
    (OUT / "results.json").write_text(json.dumps(_clean(res), indent=2))
    print(f"\n{'=' * 78}\nSUMMARY\n{'=' * 78}")
