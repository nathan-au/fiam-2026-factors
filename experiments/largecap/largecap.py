...
Uses the frozen harness from et.py (copied in below, unchanged, so this file is standalone): data loading, rank transform, walk-forward folds,
...
def predict_arm(ctx, arm, floor, args):
    ...
    return run_walk_forward(ctx, FEATURES, f"lc/{arm}", winsor=not args.raw_target, train_universe=universe)
...
def run_arm(ctx, arm, floor, tag, variants, args, ic_store):
    ...
    if arm != "comp":
        imp.rename("importance").rename_axis("variable").to_csv(OUT / f"{tag}_feature_importance_{arm}.csv")
    ...
    model_stats = {
        ...
        "universe_oos_r2_pct": None if arm == "comp" else 100 * oos_r2(u[TARGET].to_numpy(), u["pred"].to_numpy()),
    ...
    for name, cfg in variants.items():
        ...
        rows.append(summary_row(tag, arm, name, res, {k: v for k, v in model_stats.items()
...
def run_seeds(ctx, floor, tag, variants, n_seeds, args):
    ...
    for s in range(SEED + 1, SEED + n_seeds):
        SEED = s
        pred_vec, _, _ = run_walk_forward(ctx, FEATURES, f"lc/et seed {s}", winsor=not args.raw_target,
                                             train_universe="tradeable")
        preds = preds_frame(ctx, pred_vec)
        ic = universe_ic(preds, floor)
        res = evaluate_variant(preds, HEADLINE_VARIANT, variants[HEADLINE_VARIANT], tag, f"et_seed{s}")
...
def main():
    ...
    for arm in args.arms.split(","):
        ...
        all_rows += rows
        (OUT / f"{tag}_results{args.suffix}.json").write_text(json.dumps(_clean(results), indent=2))
        pd.DataFrame(all_rows).to_csv(OUT / f"{tag}_summary{args.suffix}.csv", index=False)

    pd.DataFrame(ic_store).rename_axis("target_month").to_csv(OUT / f"{tag}_monthly_ic{args.suffix}.csv")
    extra = {}
