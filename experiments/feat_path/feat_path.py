...
HERE = Path(__file__).resolve().parent  # experiments/feat_path/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Path-shape block builders. Every feature at characteristic month t uses only the monthly returns of months t-11..t-1
# (the same window as the panel's ret_12_1, verified to match to 1e-16), i.e. nothing after month t.
# ---------------------------------------------------------------------------


def path_builder(p):
    R = wide(p, "ret")
    A = np.stack([R.shift(k).to_numpy() for k in range(11, 0, -1)])  # (11, T, N); axis 0 = lag 11 (oldest) .. lag 1 (newest)
    cum = np.prod(1.0 + A, axis=0) - 1.0
    pos, neg = np.mean(A > 0, axis=0).astype(float), np.mean(A < 0, axis=0).astype(float)
    bad = np.isnan(A).any(axis=0)
    for X in (pos, neg):
        X[bad] = np.nan
    ID = np.sign(cum) * (neg - pos)  # Da-Gurun-Warachka information discreteness (low = continuous)
    c = np.cumsum(np.log1p(A), axis=0)  # cumulative log-return path, 11 points
    x = np.arange(1, 12, dtype=float)[:, None, None]
    xc = x - x.mean()
    cc = c - c.mean(axis=0, keepdims=True)
    slope = (xc * cc).sum(axis=0) / (xc ** 2).sum()
    r2 = ((xc * cc).sum(axis=0) ** 2) / ((xc ** 2).sum() * (cc ** 2).sum(axis=0) + 1e-18)
    maxdd = np.min(c - np.maximum.accumulate(c, axis=0), axis=0)  # worst drawdown of the path (<= 0)
    mk = lambda a: pd.DataFrame(a, index=R.index, columns=R.columns)
    return {"cum_12_1": mk(cum), "ID": mk(ID), "up_minus_down_frac": mk(pos - neg), "trend_strength": mk(slope * r2),
            "path_maxdd": mk(maxdd)}


def make_blocks(ctx):
    panel = load_panel(["ret"], ctx)
    trunc = truncation_test(path_builder, panel)
    W = path_builder(panel)
    v = {k: wide_to_meta(w, ctx.meta) for k, w in W.items()}
    m_rank, c_rank = urank(v["cum_12_1"], ctx), -urank(v["ID"], ctx)  # c_rank high = continuous information
    blocks = {
        # pre-registered signs (from the cited papers, fixed before results): + = higher is better
        "mom_x_continuity": {"raw": m_rank * (1 + c_rank) / 2, "sign": +1,
                             "note": "12-1 momentum rank scaled by information continuity (Da-Gurun-Warachka 'frog in the pan')"},
        "trend_strength": {"raw": v["trend_strength"], "sign": +1, "note": "slope x R^2 of the 11-point cumulative log-return path (Quantitativo smoothness)"},
        "up_minus_down_frac": {"raw": v["up_minus_down_frac"], "sign": +1, "note": "share of up months minus share of down months, months t-11..t-1"},
        "path_maxdd": {"raw": v["path_maxdd"], "sign": +1, "note": "max drawdown inside the 11-month path (shallower = better)"},
        "mom_12_1_plain": {"raw": v["cum_12_1"], "sign": +1, "note": "CONTROL: plain 12-1 momentum (does continuity add anything over momentum itself?)"},
    }
    return blocks, {}, trunc
...
def main():
    ...
    results = {"baseline": {"comp_ic": float(ic_store["comp"].mean()), "comp_ic_t": tstat(ic_store["comp"]), "lp": comp_lp,
               ...
               "truncation_test": trunc, "blocks": {}, "alt_composites": {}}
    rows = []
