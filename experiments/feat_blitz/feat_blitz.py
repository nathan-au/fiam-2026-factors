"""
FIAM 2026 - Blitz et al. short-term composite (industry-relative reversal, industry momentum, seasonality, ivol) as a large-cap block (feat_blitz.py).

Research origin: docs/RESEARCH.md Part II sec 2.2 (Blitz, Hanauer, Honarvar, Huisman, van Vliet, FAJ 2023: five short-horizon signals in MSCI World constituents only, composite alpha > 12%, uncorrelated with FF factors; the project's composite deliberately has no short-term content).

HYPOTHESIS. A composite of industry-relative 1-month reversal, 1-month industry momentum and same-month seasonality is orthogonal to the composite and adds IC in the >= $2B universe (analyst revisions, the paper's fourth signal, are unavailable). The 2021-26 window contains the July-2025 / Jan-2026 short-horizon unwinds, so a failure is plausible.
...
Run:  .venv/bin/python experiments/feat_blitz/feat_blitz.py [--nperm 200] [--smoke]
...
HERE = Path(__file__).resolve().parent  # experiments/feat_blitz/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Blitz-Hanauer-Honarvar-Huisman-van Vliet short-term composite, large-cap version (analyst revisions are NOT available in the panel).
# All inputs are month-t returns / characteristics known at the characteristic month.
# ---------------------------------------------------------------------------


def blitz_inputs_builder(p):
    return {"ret_1_0": wide(p, "ret"), "ff49": wide(p, "ff49")}


def make_blocks(ctx):
    panel = load_panel(["ret", "ff49"], ctx)
    trunc = truncation_test(blitz_inputs_builder, panel)
    W = blitz_inputs_builder(panel)
    r1 = wide_to_meta(W["ret_1_0"], ctx.meta)
    ff = wide_to_meta(W["ff49"], ctx.meta)
    ff = np.where(np.isfinite(ff), ff, -1).astype(int)
    ind1 = ind_mean(ctx, r1, ff)
    rel = r1 - ind1
    seas1, seas25 = xall_te(ctx, "seas_1_1na"), xall_te(ctx, "seas_2_5na")
    seas = (urank(seas1, ctx) + urank(seas25, ctx)) / 2
    ivol = xall_te(ctx, "ivol_capm_21d")
    sig = {"indrev": -urank(rel, ctx), "indmom1m": urank(ind1, ctx), "seas": seas}
    comp3 = (sig["indrev"] + sig["indmom1m"] + sig["seas"]) / 3
    comp4 = (sig["indrev"] + sig["indmom1m"] + sig["seas"] - urank(ivol, ctx)) / 4
    blocks = {
        "blitz_indrev_1m": {"raw": rel, "sign": -1, "note": "industry-relative 1-month reversal: -(ret_1_0 - FF49 universe mean)"},
        "blitz_indmom_1m": {"raw": ind1, "sign": +1, "note": "1-month FF49 industry momentum"},
        "blitz_seasonality": {"raw": seas, "sign": +1, "note": "mean rank of seas_1_1na and seas_2_5na (same-calendar-month past returns)"},
        "blitz_composite3": {"raw": comp3, "sign": +1, "note": "equal-weight of the three signals above (analyst revisions unavailable)"},
        "blitz_composite4_with_ivol": {"raw": comp4, "sign": +1, "note": "as composite3 plus 1-month idiosyncratic volatility (already inside the composite's volatility group)"},
    }
    return blocks, {}, trunc
