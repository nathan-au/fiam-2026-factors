...
D4 +idiosyncratic volatility +quality (exact equality constraints on the extra exposures, solved in the same LP). Alpha-projection variants P2-P4 instead orthogonalise the composite to the same columns month by month
(Barra-style residual alpha) and keep the D1 constraints. S1/S2 rank the composite within GICS sector and drop / keep the sector caps. Reported: signal IC, IR, beta and rolling beta, max drawdown, 2025 return,
net return in Jun-2025, Jul-2025, Jan-2026, Jul-2026 and Jan-2021, realised formation exposure to each neutraliser, paired net-return t vs D1. The LP relaxes an infeasible turnover cap stepwise (logged in `months_cap_relaxed`).

Run:  .venv/bin/python experiments/neutral_dial/neutral_dial.py [--smoke]
Outputs (output/): results.json, summary.csv, stress_windows_net_return.csv, net_monthly_returns.csv, run.log, portfolio_{holdings,returns}_<variant>_lc_comp.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/neutral_dial/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Driver: neutralisation depth as a dial, with crowded-unwind windows
# ---------------------------------------------------------------------------
NEUTRALISERS = {"x_size": "log market cap", "x_mom": "12-1 momentum rank (ret_12_1)", "x_vol": "idiosyncratic volatility rank (ivol_capm_21d)",
                "x_qual": "quality rank (qmj)"}


def add_neutraliser_columns(ctx):
    m = ctx.meta
    m["x_size"] = np.log(m["raw_me"].to_numpy(dtype=float))
    m["x_mom"] = xall_te(ctx, "ret_12_1")
    m["x_vol"] = xall_te(ctx, "ivol_capm_21d")
    m["x_qual"] = xall_te(ctx, "qmj")


def project_out(ctx, score, cols):
    """Month-by-month OLS residual of the score on the neutraliser columns (universe rows): the alpha vector orthogonal to them."""
    out = np.asarray(score, dtype=float).copy()
    m = ctx.meta
    um = ctx.um
    d = m.loc[um, ["target_month"] + cols].copy()
    d["s"] = out[um]
    for _, g in d.groupby("target_month"):
        A = np.column_stack([np.ones(len(g))] + [(g[c] - g[c].mean()) / (g[c].std() + 1e-12) for c in cols])
        beta = np.linalg.lstsq(A, g["s"].to_numpy(), rcond=None)[0]
        out[g.index.to_numpy()] = g["s"].to_numpy() - A @ beta
    return out
...
def exposures(hold_csv, ctx):
    h = pd.read_csv(hold_csv, parse_dates=["target_month"])
    m = ctx.meta[["permno", "target_month"] + list(NEUTRALISERS)]
    h = h.merge(m, on=["permno", "target_month"], how="left")
    out = {}
    for c in NEUTRALISERS:
        e = (h["weight"] * h[c]).groupby(h["target_month"]).sum()
        out[c] = {"mean": float(e.mean()), "max_abs": float(e.abs().max())}
    return out
...
def main():
    ...
    base_obj = Base(ctx)
    add_neutraliser_columns(ctx)
    base = make_variants(FLOOR)["lc_t10"]
    no_sector = {**base, "sector_net": None, "sector_gross": None}
    comp = base_obj.comp
    X4 = list(NEUTRALISERS)
    V = {  # name: (cfg, score)
        # constraint dial (alpha unchanged, more exposures forced to exactly zero)
        "D0_beta_only": (no_sector, comp),
        "D1_beta_sector (current lc_t10)": (base, comp),
        "D2_plus_size": ({**base, "extra_neutral": ["x_size"]}, comp),
        "D3_plus_size_mom": ({**base, "extra_neutral": ["x_size", "x_mom"]}, comp),
        "D4_plus_size_mom_vol_qual": ({**base, "extra_neutral": X4}, comp),
        # alpha-projection dial (Barra-style residual alpha; LP keeps beta + sector only)
        "P2_alpha_ortho_size": (base, project_out(ctx, comp, ["x_size"])),
        "P3_alpha_ortho_size_mom": (base, project_out(ctx, comp, ["x_size", "x_mom"])),
        "P4_alpha_ortho_all4": (base, project_out(ctx, comp, X4)),
        # sector-neutral ranking instead of sector caps in the LP
        "S1_sector_rank_no_sector_caps": (no_sector, sector_rank(ctx, comp)),
        "S2_sector_rank_plus_sector_caps": (base, sector_rank(ctx, comp)),
    }
    rows, nets, results = [], {}, {}
    for name, (cfg, score) in V.items():
        s = np.asarray(score, dtype=float)
        ic = base_obj.ic(s)
        row, net, res = run_variant(base_obj.preds(s), name, cfg, "comp")
        ex = exposures(OUT / f"portfolio_holdings_{name}_{TAG}_comp.csv", ctx)
        row = {"variant": name, "signal_ic": float(ic.mean()), "signal_ic_t": tstat(ic), **row,
               **{f"expo_{c}_max_abs_formation": ex[c]["max_abs"] for c in X4}}
        stress = [v for v in row["stress_net_ret"].values() if v is not None]
        row["stress_sum_5_windows"] = float(np.sum(stress)) if stress else None
        rows.append(row)
        nets[name] = net
        results[name] = {"row": row, "exposures": ex}
        print(f"  [{name:34s}] IC {row['signal_ic']:.4f} | IR gross {row['ir_gross']:+.2f} net {row['ir_net']:+.2f} | beta {row['beta']:+.2f} "
              f"roll [{row['roll_beta_min']:+.2f},{row['roll_beta_max']:+.2f}] | maxDD {row['max_dd_net_pct']:.0f}% | stress "
              f"{ {k[2:]: (None if v is None else round(100 * v, 1)) for k, v in row['stress_net_ret'].items()} } | relaxed {row['months_cap_relaxed']}", flush=True)
        (OUT / "results.json").write_text(json.dumps(_clean(results), indent=2))
    summ = pd.DataFrame(rows)
    ctrl = nets["D1_beta_sector (current lc_t10)"]
    summ["paired_net_ret_t_vs_D1"] = summ["variant"].map(lambda k: paired_net(nets[k], ctrl)["t"])
    summ.drop(columns=["stress_net_ret"]).to_csv(OUT / "summary.csv", index=False)
    pd.DataFrame({r["variant"]: r["stress_net_ret"] for r in rows}).T.rename(columns=STRESS_MONTHS).to_csv(OUT / "stress_windows_net_return.csv")
    pd.DataFrame(nets).rename_axis("target_month").to_csv(OUT / "net_monthly_returns.csv")
    print(f"\n{'=' * 78}\nSUMMARY\n{'=' * 78}")
    print(summ[["variant", "signal_ic", "ir_gross", "ir_net", "paired_net_ret_t_vs_D1", "beta", "roll_beta_min", "roll_beta_max", "max_dd_net_pct",
                "cy2025_net", "stress_sum_5_windows", "months_cap_relaxed"]].to_string(index=False, float_format=lambda v: f"{v:.3f}"))
