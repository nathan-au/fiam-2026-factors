...
cap within the universe month; sector = 2-digit GICS; the residual target is fit cross-sectionally each month from exposures
measured at the characteristic month. Transforms use only that month's cross-section. Candidate selection and scoring are on
the RAW return; additionally the IC against the residual return and the sector-demeaned return is reported (`ic_vs_*`).

Run:  .venv/bin/python experiments/target_neutral/target_neutral.py [--models et,lgbm] [--seed 42] [--smoke]
...
HERE = Path(__file__).resolve().parent  # experiments/target_neutral/
OUTPUT = HERE / "output"
...
ARMS = {
    ...
    "demean_size": {"target": "demean_size", "winsor": True},
    "demean_sector": {"target": "demean_sector", "winsor": True},
    "demean_size_sector": {"target": "demean_size_sector", "winsor": True},
    "resid_factor": {"target": "resid_factor", "winsor": True},
}
ALT_SCORES = ["resid_factor", "demean_sector"]  # also report IC against these (scoring only; never used for selection)
