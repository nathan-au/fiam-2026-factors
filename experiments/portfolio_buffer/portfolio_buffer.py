...
reproduce the frozen LP when the options are off (control `ctrl_t10` must give gross IR 0.61 / net 0.54). Variants are listed in `V` in main(). Costs: the frozen tiered assumptions of experiments/pm_ablation.
Also reported: max |formation beta| (partial rebalancing can leave small residual exposure), positions, 2025 net return, and net return in the crowded-unwind months.

Run:  .venv/bin/python experiments/portfolio_buffer/portfolio_buffer.py [--smoke]
...
HERE = Path(__file__).resolve().parent  # experiments/portfolio_buffer/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Driver: turnover-control designs on the FROZEN composite signal
# ---------------------------------------------------------------------------


def ema_scores(ctx, half_life):
    """Composite score EMA-smoothed across months per stock (score_s(t) = a*score(t) + (1-a)*score_s(t-1), a = 1 - 0.5^(1/half_life)),
    computed on all universe rows from 2015 on so the test window has a warm start. Only scores known at month t are used."""
    G = grp_scores(ctx, np.arange(len(ctx.tm))).mean(axis=1)
    d = pd.DataFrame({"m": ctx.tm, "p": ctx.all_permno, "s": G})[ctx.u]
    W = d.pivot(index="m", columns="p", values="s").sort_index()
    S = W.ewm(alpha=1 - 0.5 ** (1.0 / half_life), adjust=False, ignore_na=True).mean()
    idx = pd.MultiIndex.from_arrays([ctx.meta["target_month"], ctx.meta["permno"]])
    return S.stack().reindex(idx).to_numpy()
...
def main():
    ...
    free = {**base, "turnover": None}
    sig = {"raw": base_obj.comp}
    for hl in (1, 2, 3):
        s = ema_scores(ctx, hl)
        sig[f"ema{hl}"] = np.where(np.isfinite(s), s, 0.0)
    ic = {k: base_obj.ic(v) for k, v in sig.items()}
    print("Signal universe IC:", {k: round(float(v.mean()), 4) for k, v in ic.items()}, flush=True)

    V = {  # name: (cfg, signal key)
        "ctrl_t10": (base, "raw"), "ctrl_free": (free, "raw"),
        "buf_10_50_free": ({**free, "enter": 0.10, "hold": 0.50}, "raw"),
        "buf_10_30_free": ({**free, "enter": 0.10, "hold": 0.30}, "raw"),
        "buf_20_50_free": ({**free, "enter": 0.20, "hold": 0.50}, "raw"),
        "buf_10_50_t10": ({**base, "enter": 0.10, "hold": 0.50}, "raw"),
        "l1_0.02_free": ({**free, "l1_lambda": 0.02}, "raw"), "l1_0.05_free": ({**free, "l1_lambda": 0.05}, "raw"),
        "l1_0.10_free": ({**free, "l1_lambda": 0.10}, "raw"), "l1_0.20_free": ({**free, "l1_lambda": 0.20}, "raw"),
        "partial50_free": ({**free, "trade_frac": 0.5}, "raw"),
        "partial50_buf_10_50": ({**free, "enter": 0.10, "hold": 0.50, "trade_frac": 0.5}, "raw"),
        # 2% per-name cap (>= 50 names per side, still >= 100 positions) so that a 10% entry zone is not infeasible
        "ctrl_t10_w2": ({**base, "max_weight": 0.02}, "raw"), "ctrl_free_w2": ({**free, "max_weight": 0.02}, "raw"),
        "buf_10_50_free_w2": ({**free, "max_weight": 0.02, "enter": 0.10, "hold": 0.50}, "raw"),
        "buf_10_50_t10_w2": ({**base, "max_weight": 0.02, "enter": 0.10, "hold": 0.50}, "raw"),
        "ema1_t10": (base, "ema1"), "ema2_t10": (base, "ema2"), "ema3_t10": (base, "ema3"),
        "ema2_free": (free, "ema2"),
        "ema2_buf_10_50_free": ({**free, "enter": 0.10, "hold": 0.50}, "ema2"),
        "ema2_partial50_free": ({**free, "trade_frac": 0.5}, "ema2"),  # Garleanu-Pedersen-lite: aim = smoothed score, trade half the gap
    }
    rows, nets, results = [], {}, {"signal_ic": {k: float(v.mean()) for k, v in ic.items()}}
    for name, (cfg, sk) in V.items():
        row, net, res = run_variant(base_obj.preds(sig[sk]), name, cfg, "comp")
        row = {"variant": name, "signal": sk, **{k: v for k, v in cfg.items() if k in ("turnover", "enter", "hold", "l1_lambda", "trade_frac", "max_weight")}, **row}
        rows.append(row)
        nets[name] = net
        results[name] = {"row": row, "gross": res["gross"], "net": res["net"]}
        print(f"  [{name:22s}] IR gross {row['ir_gross']:+.2f} net {row['ir_net']:+.2f} | turnover {row['turnover_one_way_pct_gross']:.1f}% "
              f"cost {row['trade_cost_bp_nav_month']:.1f}bp | beta {row['beta']:+.2f} | maxDD {row['max_dd_net_pct']:.0f}% | 2025 {100 * (row['cy2025_net'] or 0):+.1f}% "
              f"| pos {row['min_n_positions']}-{row['avg_n_positions']:.0f} | relaxed {row['months_cap_relaxed']}", flush=True)
        (OUT / "results.json").write_text(json.dumps(_clean(results), indent=2))
    summ = pd.DataFrame(rows)
    ctrl = nets["ctrl_t10"]  # (the *_w2 buffer variants should also be read against ctrl_t10_w2 / ctrl_free_w2 in the table)
    ctrl_ir, ctrl_to = summ.loc[summ.variant == "ctrl_t10", "ir_net"].iloc[0], summ.loc[summ.variant == "ctrl_t10", "turnover_one_way_pct_gross"].iloc[0]
    pt = {k: paired_net(nets[k], ctrl) for k in nets}
    summ["d_ir_net_vs_ctrl_t10"] = summ["ir_net"] - ctrl_ir
    summ["paired_net_ret_t_vs_ctrl_t10"] = summ["variant"].map(lambda k: pt[k]["t"])
    summ["paired_net_ret_diff_pct_month"] = summ["variant"].map(lambda k: pt[k]["mean_monthly_diff_pct"])
    summ["adopt_rule"] = (summ["d_ir_net_vs_ctrl_t10"] >= 0.10) & (summ["turnover_one_way_pct_gross"] <= ctrl_to + 1e-9) & (summ["paired_net_ret_t_vs_ctrl_t10"] >= 1)
    ...
    results["paired_net_vs_ctrl_t10"] = pt
    (OUT / "results.json").write_text(json.dumps(_clean(results), indent=2))
    print(f"\n{'=' * 78}\nSUMMARY (control = ctrl_t10: net IR {ctrl_ir:.3f}, one-way turnover {ctrl_to:.1f}% of gross)\n{'=' * 78}")
    cols = ["variant", "ir_gross", "ir_net", "d_ir_net_vs_ctrl_t10", "paired_net_ret_t_vs_ctrl_t10", "turnover_one_way_pct_gross",
            "trade_cost_bp_nav_month", "beta", "max_abs_beta_formation", "max_dd_net_pct", "cy2025_net", "adopt_rule"]
