"""
FIAM 2026 - Characteristic-change (momentum in attributes) blocks on the large-cap composite (feat_char_changes.py).

Research origin: docs/RESEARCH.md Part I sec 3.3 item 5 (returns are driven by CHANGES in characteristics; 'momentum in firm attributes should be more investigated', arXiv 2203.07865).

HYPOTHESIS. The 3- and 12-month change in the cross-sectional rank of profitability/valuation/quality/surprise/investment characteristics carries information the levels in the composite do not, and adds IC as an extra group.
...
Run:  .venv/bin/python experiments/feat_char_changes/feat_char_changes.py [--nperm 200] [--smoke]
...
HERE = Path(__file__).resolve().parent  # experiments/feat_char_changes/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Characteristic-CHANGE blocks: k-month change in the cross-sectional percentile rank of a characteristic (all stocks, within month),
# built from the raw panel columns and lagged by calendar months, so only values at months <= t are used.
# ---------------------------------------------------------------------------
CHG_VARS = {"qmj": +1, "gp_at": +1, "be_me": +1, "niq_su": +1, "at_gr1": -1}  # signs = the level's sign in the frozen composite / literature


def chg_builder(p):
    out = {}
    for v in CHG_VARS:
        rk = wide(p, v).rank(axis=1, pct=True)
        for k in (3, 12):
            out[f"d{k}_{v}"] = rk - rk.shift(k)
    return out


def make_blocks(ctx):
    panel = load_panel(list(CHG_VARS), ctx)
    trunc = truncation_test(chg_builder, panel)
    W = chg_builder(panel)
    v = {k: wide_to_meta(w, ctx.meta) for k, w in W.items()}
    blocks = {}
    for k in (12, 3):
        comp = sum(CHG_VARS[c] * urank(v[f"d{k}_{c}"], ctx) for c in CHG_VARS) / len(CHG_VARS)
        blocks[f"chg{k}m_composite"] = {"raw": comp, "sign": +1, "note": f"equal-weight of the signed {k}-month rank changes of {list(CHG_VARS)}"}
    for c, s in CHG_VARS.items():
        blocks[f"d12_{c}"] = {"raw": v[f"d12_{c}"], "sign": s, "note": f"12-month change in the rank of {c} (sign as the level)"}
    return blocks, {}, trunc
