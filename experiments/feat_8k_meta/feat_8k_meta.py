"""
FIAM 2026 - 8-K item-code metadata (intensity, news/no-news reversal, event flags, earnings timing) on the large-cap composite (feat_8k_meta.py).

Research origin: docs/RESEARCH.md Part I sec 3.4 items 1-4 (8-K filing frequency predicts LOWER returns, ~4%/yr spread; reversal only works for non-news moves; 4.02/4.01/2.05 event flags; earnings-announcement premium) and docs/RESEARCH.md Part II sec 2.11 (has_earnings_in_window), H7 (graded decayed event intensity). Uses ONLY the structured filing date and item codes - no filing text and no LLM (those ideas are out of scope).

HYPOTHESIS. 8-K item metadata carries mid/large-cap information the 147 characteristics lack: filing frequency (-), decayed bad-event intensity (-), reversal restricted to no-news months (+), and expected earnings month (+, a falsification test given the premium is reported to have disappeared).
DESIGN. Frozen experiments/largecap harness (copied in below); universe = price >= $5, mcap >= $2B, $10M dollar volume, both betas observed. No model is fitted: the
composite is the unchanged 7-group equal-weight rule and each block is tested as one more equal-weight group (signs fixed before any result). Diagnostics per block: IC alone; paired monthly
IC gain vs the composite; residual IC after orthogonalising the block to the composite (shared-core check, docs/RESEARCH.md Part II sec 2.5); gain by size tercile; within-(month, sector)
permutation null (200 shuffles, docs/RESEARCH.md Part II H4) with Bonferroni over the blocks in this file; LP portfolio `lc_t10` (gross/net IR). A truncation-invariance test (rebuild the
features from rows <= t only and require identical values at t; docs/RESEARCH.md Part II sec 2.9) is run on the feature builder.
PRE-REGISTERED KILL RULE (docs/RESEARCH.md Part I sec 2 / sec 4): kill if the paired IC gain has t < 1, or the gain is confined to the smallest size tercile. PASS requires paired t >= 2,
Bonferroni-adjusted permutation p <= 0.05, and not small-tercile-only.

Run:  .venv/bin/python experiments/feat_8k_meta/feat_8k_meta.py [--nperm 200] [--smoke]
Outputs (output/): results.json, summary.csv, monthly_ic.csv, run.log, portfolio_{holdings,returns}_lc_t10_lc_comp[_plus_<block>].csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/feat_8k_meta/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# 8-K item-code metadata blocks. Uses ONLY the structured filing date and item codes (never the filing text): counts, item families,
# exponentially decayed event intensities, news/no-news split of reversal, expected-earnings-month flags. A filing dated in month m is
# treated as known at the end of month m (conservative: FIAM sec 4). Features are NaN for a stock with no 8-K in the trailing 24 months
# (unmapped / dormant), so "no filings" is never confused with "not in the 8-K file".
# ---------------------------------------------------------------------------
BAD_ITEMS = ("4.02", "4.01", "2.05", "2.06")  # non-reliance, auditor change, restructuring, impairment
HL = 6.0  # half-life (months) of the decayed intensities


def load_8k_items():
    k = pd.read_parquet(FIAM_DIR / "8k_20150101_20260831_identified.parquet", columns=["permno", "filing_date", "items", "document_id"])
    k["eom"] = pd.to_datetime(k["filing_date"]) + pd.offsets.MonthEnd(0)
    k = k.explode("items").rename(columns={"items": "item"})
    return k[["permno", "eom", "document_id", "item"]].reset_index(drop=True)


def _count(p, mask=None, by_doc=True):
    q = p if mask is None else p[mask]
    q = q.drop_duplicates("document_id") if by_doc else q
    return q.groupby(["eom", "permno"]).size().unstack("permno").reindex(MONTH_INDEX)


def _decay(N, hl):
    a, out, run = 0.5 ** (1.0 / hl), np.zeros(N.shape), np.zeros(N.shape[1])
    X = N.fillna(0.0).to_numpy()
    for t in range(len(X)):
        run = X[t] + a * run
        out[t] = run
    return pd.DataFrame(out, index=N.index, columns=N.columns)


def k8_builder(p):
    Nall = _count(p).fillna(0.0)
    cov = Nall.rolling(24, min_periods=1).sum() > 0  # has filed in the trailing 24 months
    msk = lambda F: F.where(cov)
    N202 = _count(p, p["item"] == "2.02").reindex(columns=Nall.columns).fillna(0.0)
    Nbad = _count(p, p["item"].isin(BAD_ITEMS)).reindex(columns=Nall.columns).fillna(0.0)
    Nmg = _count(p, p["item"] == "5.02").reindex(columns=Nall.columns).fillna(0.0)
    e = lambda F, k: (F.shift(k) > 0).astype(float)
    return {
        "count_12m": msk(Nall.rolling(12, min_periods=12).sum()),
        "abnormal_3m": msk(Nall.rolling(3, min_periods=3).sum() - Nall.shift(3).rolling(12, min_periods=12).sum() / 4.0),
        "decayed_all": msk(_decay(Nall, HL)),
        "no_news_month": msk((Nall == 0).astype(float)),
        "bad_decayed": msk(_decay(Nbad, HL)),
        "bad_flag_3m": msk((Nbad.rolling(3, min_periods=3).sum() > 0).astype(float)),
        "mgmt_decayed": msk(_decay(Nmg, HL)),
        "eap_same_month_ly": msk(e(N202, 11)),  # a 2.02 filing in month t-11 = same calendar month as t+1, one year earlier
        "eap_any_quarter": msk(((e(N202, 2) + e(N202, 5) + e(N202, 8) + e(N202, 11)) > 0).astype(float)),
    }


def make_blocks(ctx):
    items = load_8k_items()
    items = items[items["permno"].isin(ctx.permnos)]
    trunc = truncation_test(k8_builder, items, n_dates=2)
    W = k8_builder(items)
    v = {k: wide_to_meta(w, ctx.meta) for k, w in W.items()}
    cov_univ = float(np.isfinite(v["count_12m"])[ctx.um].mean())
    print(f"  8-K coverage of universe test rows: {cov_univ:.3f}", flush=True)
    ret1 = wide_to_meta(wide(load_panel(["ret"], ctx), "ret"), ctx.meta)
    rrev = -urank(ret1, ctx)  # reversal rank: high = big recent loser
    nonews = np.where(np.isfinite(v["no_news_month"]), v["no_news_month"], np.nan)
    blocks = {
        "k8_count_12m": {"raw": v["count_12m"], "sign": -1, "note": "8-K filings in months t-11..t (Management Science: more filings -> lower returns)"},
        "k8_abnormal_3m": {"raw": v["abnormal_3m"], "sign": -1, "note": "last-3-month count minus the prior-12-month quarterly average"},
        "k8_decayed_all_hl6": {"raw": v["decayed_all"], "sign": -1, "note": "graded intensity: exponentially decayed filing count, half-life 6m (H7)"},
        "rev_nonews": {"raw": np.where(nonews == 1, rrev, np.where(nonews == 0, 0.0, np.nan)), "sign": +1,
                       "note": "short-term reversal rank only for stocks with NO 8-K in month t (Novy-Marx/Medhat: reversal is liquidity provision)"},
        "rev_news_control": {"raw": np.where(nonews == 0, rrev, np.where(nonews == 1, 0.0, np.nan)), "sign": +1,
                             "note": "CONTROL: the same reversal rank only for stocks WITH an 8-K in month t (expect no effect)"},
        "bad_event_decayed_hl6": {"raw": v["bad_decayed"], "sign": -1, "note": "decayed intensity of items 4.02/4.01/2.05/2.06 (H7)"},
        "bad_event_flag_3m": {"raw": v["bad_flag_3m"], "sign": -1, "note": "any 4.02/4.01/2.05/2.06 in the last 3 months"},
        "mgmt_5_02_decayed_hl6": {"raw": v["mgmt_decayed"], "sign": -1, "note": "decayed intensity of item 5.02 (officer/director change)"},
        "eap_same_month_last_year": {"raw": v["eap_same_month_ly"], "sign": +1, "note": "earnings-announcement premium: a 2.02 filing in the same calendar month one year earlier (falsification test; Reddit 2.11 has_earnings_in_window)"},
        "eap_any_quarter": {"raw": v["eap_any_quarter"], "sign": +1, "note": "a 2.02 filing at t-2, t-5, t-8 or t-11 (quarterly rhythm)"},
    }
    return blocks, {}, trunc
