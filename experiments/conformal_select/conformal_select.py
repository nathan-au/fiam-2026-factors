...
at or below q, AND that beats a same-size top-k-by-score set. With a signal of IC ~0.037 the win rate of even the top decile is only slightly above the 30% base rate, so the FDR levels that are informative are close to 1 - base rate = 0.70; the experiment
tests whether calibration is honest (realised FDR <= q) and whether it adds selection value over plain top-k. KILL: realised FDR > q in the test window (exchangeability broken by regime change) or no return advantage over same-size top-k.

DESIGN. Frozen experiments/largecap harness (copied in below); frozen composite (no fit) so ALL universe months before the test year are calibration data (72 months before 2021). win = next-month excess return in the top 30% of the universe cross-section (long)
or bottom 30% (short, score sign flipped); null = calibration units that are not winners; conformal p-value = (1 + #null scores >= s) / (n_null + 1); BH at q in {0.60, 0.65, 0.70}; per test month. Compared with the top-k of the same size k.
Toy test on selection quality; no LP portfolio.

Run:  .venv/bin/python experiments/conformal_select/conformal_select.py [--smoke]
Outputs (output/): results.json, summary.csv, selection_counts_by_month.csv, run.log
"""
...
HERE = Path(__file__).resolve().parent  # experiments/conformal_select/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Driver: conformal selection of names (Jin & Candes style, conformal p-values + Benjamini-Hochberg) on the frozen composite
# ---------------------------------------------------------------------------
from scipy.stats import rankdata


def conformal_select(score_test, score_cal_null, q):
    """One-sided conformal p-values against the calibration NULL scores (units that did NOT 'win'), then BH at level q.
    Returns a boolean selection over the test rows. p_i = (1 + #{null_j >= s_i}) / (n_null + 1)."""
    null = np.sort(score_cal_null)
    n = len(null)
    cnt = n - np.searchsorted(null, score_test, side="left")  # number of null scores >= s_i
    p = (1.0 + cnt) / (n + 1.0)
    order = np.argsort(p)
    m = len(p)
    thresh = q * np.arange(1, m + 1) / m
    ok = np.flatnonzero(p[order] <= thresh)
    sel = np.zeros(m, bool)
    if len(ok):
...
def main():
    ...
    base_obj = Base(ctx)
    rows_all = np.arange(len(ctx.tm))
    C = grp_scores(ctx, rows_all).mean(axis=1)  # composite for every universe row since 2015 (no fitting, so all history is calibration data)
    u = ctx.u & np.isfinite(ctx.y)
    tm = ctx.tm
    # within-month label: 'win' = next-month excess return in the top 30% of the universe cross-section (long side); 'lose' = bottom 30% (short side)
    d = pd.DataFrame({"m": tm[u], "y": ctx.y[u]})
    pct = d.groupby("m")["y"].rank(pct=True).to_numpy()
    win, lose = np.zeros(len(tm), bool), np.zeros(len(tm), bool)
    win[np.flatnonzero(u)] = pct >= 0.70
    lose[np.flatnonzero(u)] = pct <= 0.30
    base_rate = 0.30
    rows, per_month = [], []
    for side, lab, sign in (("long", win, +1), ("short", lose, -1)):
        for q in (0.60, 0.65, 0.70):
            sel_n, prec, ret_sel, ret_rest, ret_topk, prec_topk = [], [], [], [], [], []
            for fold in ctx.folds:
                cal = np.flatnonzero(u & (tm < np.datetime64(f"{fold['year']}-01-01")))
                nullscores = sign * C[cal][~lab[cal]]  # calibration units that are NOT winners (resp. losers)
                for mo in np.unique(tm[fold["te"]]):
                    te = fold["te"][(tm[fold["te"]] == mo) & u[fold["te"]]]
                    if len(te) < 200:
                        continue
                    s = sign * C[te]
                    sel = conformal_select(s, nullscores, q)
                    k = int(sel.sum())
                    per_month.append({"side": side, "q": q, "month": str(pd.Timestamp(mo).date()), "n_selected": k, "n_universe": len(te)})
                    if k == 0:
                        continue
                    sel_n.append(k)
                    prec.append(float(lab[te][sel].mean()))
                    ret_sel.append(float(sign * ctx.y[te][sel].mean()))
                    ret_rest.append(float(sign * ctx.y[te][~sel].mean()))
                    topk = np.argsort(-s)[:k]  # same-size selection by raw score (no calibration)
                    ret_topk.append(float(sign * ctx.y[te][topk].mean()))
                    prec_topk.append(float(lab[te][topk].mean()))
            nmonths = len({r["month"] for r in per_month if r["side"] == side and r["q"] == q})
            nsel = np.array(sel_n) if len(sel_n) else np.array([0])
            row = {"side": side, "target_fdr_q": q, "months_total": nmonths, "months_with_any_selection": len(sel_n), "avg_names_selected_when_any": float(nsel.mean()),
                   "precision_of_selected(win_rate)": float(np.mean(prec)) if prec else None, "realised_FDR(1-precision)": float(1 - np.mean(prec)) if prec else None,
                   "base_rate_win": base_rate, "precision_same_size_topk": float(np.mean(prec_topk)) if prec_topk else None,
                   "mean_ret_selected_pct": 100 * float(np.mean(ret_sel)) if ret_sel else None, "mean_ret_unselected_pct": 100 * float(np.mean(ret_rest)) if ret_rest else None,
                   "mean_ret_same_size_topk_pct": 100 * float(np.mean(ret_topk)) if ret_topk else None}
            rows.append(row)
            print(f"  {side:5s} q={q:.2f}: selection in {row['months_with_any_selection']}/{nmonths} months, avg {row['avg_names_selected_when_any']:.1f} names; precision "
                  f"{row['precision_of_selected(win_rate)']} vs base rate {base_rate} (FDR target {q}, realised {row['realised_FDR(1-precision)']}); ret sel {row['mean_ret_selected_pct']} "
                  f"vs rest {row['mean_ret_unselected_pct']} vs same-size top-k {row['mean_ret_same_size_topk_pct']}", flush=True)
    summ = pd.DataFrame(rows)
    summ.to_csv(OUT / "summary.csv", index=False)
    pd.DataFrame(per_month).to_csv(OUT / "selection_counts_by_month.csv", index=False)
    (OUT / "results.json").write_text(json.dumps(_clean({"rows": rows, "definition": "win = next-month excess return in the top 30% of the >= $2B universe cross-section (loser = bottom 30%); "
                                                      "null = calibration units that are not winners; calibration = all universe months before the test year"}), indent=2))
    print(f"\n{'=' * 78}\nSUMMARY\n{'=' * 78}")
