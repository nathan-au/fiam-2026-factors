...
Follows rf.py (docs/RF.md) and rf_2.py (docs/RF_2.md) without touching them. Changes vs rf_2.py:
  ...
  2. Selection on validation rank IC instead of MSE; heavier leaf regularization (min_samples_leaf 200/500,
     was 100), 300 trees, max_samples=0.5.
  3. The portfolio layer described below (turnover budget, sector limits, dual-beta neutrality, costs).
...
# ---------------------------------------------------------------------------
# 5. Model: Random Forest, walk-forward, validated on rank IC
# ---------------------------------------------------------------------------

MODEL_TAG = "rf3"
# Heavier leaf regularization than rf.py / rf_2.py (min_samples_leaf 100 -> 200/500): the label is
# almost pure noise, so bigger leaves = more averaging per prediction. max_samples=0.5 decorrelates the
# trees further (and halves fit time). Otherwise as rf.py: sqrt(p) features per split.
GRID = [{"max_depth": d, "min_samples_leaf": leaf} for d in (6, 8, 12) for leaf in (200, 500)]
N_ESTIMATORS = 300
MAX_SAMPLES = 0.5
...
def fit_fold(Xtr, ytr, Xva):
    ...
    for p in GRID:
        m = RandomForestRegressor(
            n_estimators=N_ESTIMATORS, max_depth=p["max_depth"], min_samples_leaf=p["min_samples_leaf"],
            max_features="sqrt", max_samples=MAX_SAMPLES, n_jobs=-1, random_state=SEED,
        ).fit(Xtr, ytr)
