"""
FIAM 2026 - SEC Form 4 insider-trading blocks on the large-cap composite (feat_insider_form4.py).

Research origin: docs/RESEARCH.md Part I sec 3.7 (insider composite of role, size, clustering gives >= 1%/mo alpha equal-weighted, 'works best for mid/large caps', alpha compressed since pre-2010; SEC EDGAR free, ~2-business-day filing lag).

HYPOTHESIS. Insider open-market purchases (and clusters of buyers) predict higher, insider sales lower returns in mid/large caps and add IC to the composite.

EXTERNAL DATA (Round D of docs/RESEARCH.md Part I sec 4; FIAM sec 4 permits external data joined on a dated identifier): SEC DERA Insider Transactions Data Sets 2020q1 onward, cache/sec_form345/ (generic user agent), issuer CIK mapped to permno through the 8-K file.
...
Run:  .venv/bin/python experiments/feat_insider_form4/feat_insider_form4.py [--nperm 200] [--smoke]      (needs network on the first run to fill the cache)
...
HERE = Path(__file__).resolve().parent  # experiments/feat_insider_form4/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# SEC Form 4 insider open-market trades. EXTERNAL DATA: cache/sec_form345/<YYYY>q<Q>_form345.zip (SEC DERA 'Insider Transactions Data Sets', ~16 MB per quarter, downloaded on first
# run from www.sec.gov with a generic user agent). Timing: a trade is dated by its FILING_DATE (public availability), aggregated by filing month <= t. Issuer CIK -> permno via the
# identified 8-K file (cik, permno), so only stocks in that map are covered (others NaN). Open-market purchases (TRANS_CODE 'P') and sales ('S') of Form 4 non-derivative transactions,
# value = shares x price, scaled by market cap at t.
# ---------------------------------------------------------------------------
import urllib.request
import zipfile

SEC_DIR = CACHE / "sec_form345"
SEC_UA = "fiam-2026-research-bot"
...
def fetch_quarter(y, q):
    ...
    if not f.exists():
        ...
        req = urllib.request.Request(f"https://www.sec.gov/files/structureddata/data/insider-transactions-data-sets/{f.name}", headers={"User-Agent": SEC_UA})
        try:
            with urllib.request.urlopen(req, timeout=120) as r, open(f, "wb") as out:
                out.write(r.read())
        except Exception as e:
            f.unlink(missing_ok=True)
            print(f"    {f.name}: not available ({e.__class__.__name__})", flush=True)
            return None
    return f


def load_trades(first_year=2020):
    k8 = pd.read_parquet(FIAM_DIR / "8k_20150101_20260831_identified.parquet", columns=["cik", "permno"]).dropna().drop_duplicates()
    k8["cik"] = pd.to_numeric(k8["cik"], errors="coerce")
    cik2p = k8.dropna().drop_duplicates("cik").set_index("cik")["permno"]
    out = []
    for y in range(first_year, 2027):
        for q in (1, 2, 3, 4):
            ...
            with zipfile.ZipFile(f) as z:
                sub = pd.read_csv(z.open("SUBMISSION.tsv"), sep="\t", usecols=["ACCESSION_NUMBER", "FILING_DATE", "DOCUMENT_TYPE", "ISSUERCIK"], dtype=str)
                tr = pd.read_csv(z.open("NONDERIV_TRANS.tsv"), sep="\t", usecols=["ACCESSION_NUMBER", "TRANS_CODE", "TRANS_SHARES", "TRANS_PRICEPERSHARE"], dtype=str)
                ow = pd.read_csv(z.open("REPORTINGOWNER.tsv"), sep="\t", usecols=["ACCESSION_NUMBER", "RPTOWNERCIK"], dtype=str)
            sub = sub[sub["DOCUMENT_TYPE"] == "4"]
            tr = tr[tr["TRANS_CODE"].isin(["P", "S"])]
            m = tr.merge(sub, on="ACCESSION_NUMBER").merge(ow, on="ACCESSION_NUMBER")
            m["value"] = pd.to_numeric(m["TRANS_SHARES"], errors="coerce") * pd.to_numeric(m["TRANS_PRICEPERSHARE"], errors="coerce")
            m["permno"] = pd.to_numeric(m["ISSUERCIK"], errors="coerce").map(cik2p)
            m["eom"] = pd.to_datetime(m["FILING_DATE"], format="%d-%b-%Y", errors="coerce") + pd.offsets.MonthEnd(0)
            out.append(m.dropna(subset=["permno", "value", "eom"])[["permno", "eom", "TRANS_CODE", "value", "RPTOWNERCIK"]])
    t = pd.concat(out)
    t["permno"] = t["permno"].astype(int)
    return t.reset_index(drop=True), set(cik2p.astype(int).values)


def ins_builder(p):
    """p: trades with columns permno, eom (filing month), TRANS_CODE, value, RPTOWNERCIK -> wide monthly / trailing aggregates (unscaled)."""
    buy, sell = p[p["TRANS_CODE"] == "P"], p[p["TRANS_CODE"] == "S"]
    mk = lambda d, agg: d.groupby(["eom", "permno"])[agg[0]].agg(agg[1]).unstack("permno").reindex(MONTH_INDEX)
    bv, sv = mk(buy, ("value", "sum")), mk(sell, ("value", "sum"))
    nb = mk(buy, ("RPTOWNERCIK", "nunique"))
    cols = sorted(set(p["permno"]))
    bv, sv, nb = [x.reindex(columns=cols).fillna(0.0) for x in (bv, sv, nb)]
    return {"buy_value_6m": bv.rolling(6, min_periods=6).sum(), "sell_value_6m": sv.rolling(6, min_periods=6).sum(), "n_buyers_3m": nb.rolling(3, min_periods=3).sum()}


def make_blocks(ctx):
    trades, mapped = load_trades()
    trades = trades[trades["permno"].isin(ctx.permnos)]
    print(f"  {len(trades):,} open-market P/S trade rows, {trades['permno'].nunique()} permnos, filing months {trades['eom'].min().date()}..{trades['eom'].max().date()}", flush=True)
    trunc = truncation_test(ins_builder, trades, n_dates=3)
    W = ins_builder(trades)
    me = wide_to_meta(wide(load_panel(["me"], ctx), "me"), ctx.meta)
    v = {k: wide_to_meta(w, ctx.meta) for k, w in W.items()}
    covered = ctx.meta["permno"].isin(mapped).to_numpy()
    for k in v:  # not covered by the cik->permno map = unknown (NaN), not 'no insider trades'
        v[k] = np.where(covered, v[k], np.nan)
    scale = lambda x: x / (me * 1e6)
    print(f"  insider-map coverage of universe test rows: {float(covered[ctx.um].mean()):.3f}", flush=True)
    blocks = {
        "ins_buy_6m_over_mcap": {"raw": scale(v["buy_value_6m"]), "sign": +1, "note": "open-market purchase value (Form 4 code P) in the last 6 filing months / market cap"},
        "ins_net_buy_6m_over_mcap": {"raw": scale(v["buy_value_6m"] - v["sell_value_6m"]), "sign": +1, "note": "(purchases - sales) value in the last 6 months / market cap"},
        "ins_sell_6m_over_mcap": {"raw": scale(v["sell_value_6m"]), "sign": -1, "note": "open-market sale value (code S) in the last 6 months / market cap"},
        "ins_buyers_3m": {"raw": v["n_buyers_3m"], "sign": +1, "note": "cluster buying: number of insider-months buying in the last 3 months (sum of monthly distinct buyers)"},
    }
    return blocks, {}, trunc
