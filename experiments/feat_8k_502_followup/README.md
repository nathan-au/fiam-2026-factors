...
## Research origin
My own follow-up to `experiments/feat_8k_meta` (docs/NEW.md sec 3.4 item 3: 5.02 among the event flags). Only the structured item code and filing date are used; no text, no LLM. Family-wise: the original test was 1 of 10, so the Bonferroni factor stays 10; the variants here share one signal and are not independent tests.
