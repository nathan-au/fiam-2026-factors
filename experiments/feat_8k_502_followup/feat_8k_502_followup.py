...
Run:  .venv/bin/python experiments/feat_8k_502_followup/feat_8k_502_followup.py [--nperm 200] [--smoke]
Outputs (output/): results.json, summary.csv, monthly_ic.csv, run.log, portfolio_{holdings,returns}_lc_t10_lc_comp[_plus_<name>].csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/feat_8k_502_followup/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Follow-up on 8-K item 5.02 (departure / appointment of directors and officers; compensatory arrangements). ONLY the structured filing date and item code
# are used (no text, no LLM). Trailing exponentially decayed count of 5.02 filings, filing dated in month m known at the end of month m; NaN when the stock has no
# 8-K in the trailing 24 months (not covered by the 8-K file). Variants: half-lives 3/6/12 months, plain 12-month count, and versions orthogonalised (month by month,
# universe rows) to size, liquidity (turnover, Amihud) and age.
# ---------------------------------------------------------------------------
...
def _count(p, mask=None):
    q = p if mask is None else p[mask]
    return q.drop_duplicates("document_id").groupby(["eom", "permno"]).size().unstack("permno").reindex(MONTH_INDEX)
...
def f502_builder(p):
    ...
    cov = Nall.rolling(24, min_periods=1).sum() > 0
    N502 = _count(p, p["item"] == "5.02").reindex(columns=Nall.columns).fillna(0.0)
    out = {f"decayed_hl{h}": _decay(N502, h).where(cov) for h in (3, 6, 12)}
    out["count_12m"] = N502.rolling(12, min_periods=12).sum().where(cov)
    out["count_3m"] = N502.rolling(3, min_periods=3).sum().where(cov)
    return out


def orth_raw(ctx, raw, cols):
    """Month-by-month OLS residual of the universe rank of `raw` on the columns (rank-standardised controls)."""
    r = urank(raw, ctx)
    out = np.full(len(r), np.nan)
    d = pd.DataFrame({"m": ctx.meta["target_month"].to_numpy(), "r": r})
    for c, x in cols.items():
        d[c] = x
    d = d[ctx.um]
    names = list(cols)
    for _, g in d.groupby("m"):
        A = np.column_stack([np.ones(len(g))] + [(g[c] - g[c].mean()) / (g[c].std() + 1e-12) for c in names])
        A = np.nan_to_num(A)
        beta = np.linalg.lstsq(A, g["r"].to_numpy(), rcond=None)[0]
        out[g.index.to_numpy()] = g["r"].to_numpy() - A @ beta
    return out
...
def make_blocks(ctx):
    ...
    trunc = truncation_test(f502_builder, items, n_dates=2)
    W = f502_builder(items)
    v = {k: wide_to_meta(w, ctx.meta) for k, w in W.items()}
    controls = {"log_size": np.log(ctx.meta["raw_me"].to_numpy(dtype=float)), "turnover": xall_te(ctx, "turnover_126d"), "amihud": xall_te(ctx, "ami_126d"),
                "age": xall_te(ctx, "age")}
    blocks = {}
    for k in ("decayed_hl3", "decayed_hl6", "decayed_hl12", "count_12m", "count_3m"):
        blocks[f"m502_{k}"] = {"raw": v[k], "sign": -1, "note": f"item 5.02 filings, {k}"}
    blocks["m502_decayed_hl6_orth_size_liq_age"] = {"raw": orth_raw(ctx, v["decayed_hl6"], controls), "sign": -1,
                                                    "note": "hl6 intensity residualised on log size, turnover, Amihud, age (is it a size / liquidity / firm-age proxy?)"}
    blocks["m502_decayed_hl6_orth_size_only"] = {"raw": orth_raw(ctx, v["decayed_hl6"], {"log_size": controls["log_size"]}), "sign": -1, "note": "hl6 intensity residualised on log size only"}
    # weight sensitivity as REPLACEMENT composites: (sum of 7 group scores + k * block) / (7 + k)
    G7sum = grp_scores(ctx, ctx.te_idx).sum(axis=1)
    b = -urank(v["decayed_hl6"], ctx)
    alts = {f"comp_plus_{k}x_m502_hl6": (G7sum + k * b) / (7 + k) for k in (1, 2, 4)}
