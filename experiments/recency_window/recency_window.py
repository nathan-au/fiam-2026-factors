...
Run:  .venv/bin/python experiments/recency_window/recency_window.py [--models et,lgbm] [--seed 42] [--smoke]
Outputs (output/): results.json, summary.csv, paired_vs_control.csv, monthly_ic.csv, run.log, oos_predictions_universe_<model>__<arm>.csv, portfolio_{holdings,returns}_<variant>_lc_<model>__<arm>.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/recency_window/
OUTPUT = HERE / "output"
...
def val_start_month_index(fold):
    return int(fold["year"] - 2) * 12 + 1  # the validation window opens 2 calendar years before the test year


def months_index(tm):
    t = pd.to_datetime(tm)
    return (t.year * 12 + t.month).to_numpy()


def make_weights(ctx, spec):
    kind = spec.get("weight")
    hl = spec.get("half_life")
    if kind is None and hl is None:
        return None

    def f(fold, tr):
        w = np.ones(len(tr))
        if kind == "mcap":
            w = np.minimum(ctx.raw_me[tr], MCAP_CAP)
        elif kind == "sqrt_mcap":
            w = np.sqrt(np.minimum(ctx.raw_me[tr], MCAP_CAP))
        elif kind == "sqrt_dolvol":
            w = np.sqrt(np.clip(ctx.raw_dolvol[tr], 0, None))
        if hl is not None:
            age = val_start_month_index(fold) - months_index(ctx.tm[tr])  # months between the row's target month and the validation start
            w = w * 0.5 ** (np.clip(age, 0, None) / hl)
        return w / np.mean(w)
    return f


def make_select(ctx, spec):
    win = spec.get("window")
    if win is None:
        return None
    return lambda fold, tr: tr[months_index(ctx.tm[tr]) >= val_start_month_index(fold) - win]


def make_blockwise_et(n_blocks, include_full):
    """Numerai 'incremental' style: fit the same ET on n contiguous month blocks of the training window (and optionally on the whole window)
    and average the predictions (a two-layer stack whose second layer is the mean)."""
    def fit(Xtr, ytr, Xva, aux):
        months = pd.factorize(aux["m_tr"], sort=True)[0]
        edges = np.linspace(0, months.max() + 1, n_blocks + 1)
        cands = []
        for p in GRID:
            models = []
            for b in range(n_blocks):
                k = (months >= edges[b]) & (months < edges[b + 1])
                models.append(ExtraTreesRegressor(n_estimators=N_ESTIMATORS, max_features=p["max_features"], min_samples_leaf=max(20, p["min_samples_leaf"] // n_blocks),
                                                  bootstrap=False, n_jobs=-1, random_state=SEED).fit(Xtr[k], ytr[k]))
            if include_full:
                models.append(ExtraTreesRegressor(n_estimators=N_ESTIMATORS, max_features=p["max_features"], min_samples_leaf=p["min_samples_leaf"],
                                                  bootstrap=False, n_jobs=-1, random_state=SEED).fit(Xtr, ytr))
            pf = lambda X, models=models: np.mean([m.predict(X) for m in models], axis=0)
            cands.append({"params": {**p, "blocks": n_blocks, "full": include_full}, "val_pred": pf(Xva), "predict": pf,
...
def main():
    ...
    results, ics = {}, {}
    all_rows = np.ones(len(ctx.y), bool)
    for model in args.models.split(","):
        for arm in arms:
            ...
            if "fitters" in spec and model not in spec["fitters"]:
                continue
            t0 = time.time()
            print(f"\n{'=' * 78}\n{key}  ({ {k: v for k, v in spec.items() if k != 'fitters'} }, seed={SEED})\n{'=' * 78}", flush=True)
            pred, imp, fold_info = run_wf(ctx, FEATURES, ctx.y, model, key, winsor=True, fitter=spec.get("fitters", {}).get(model), weights=make_weights(ctx, spec),
                                          train_mask=all_rows if spec.get("train") == "all" else None, train_select=make_select(ctx, spec))
            ...
            print(f"  IC {stats['universe_rank_ic']:.4f} (t {stats['universe_rank_ic_t']:.2f}) D10-D1 {stats['d10_minus_d1_pct_per_month']:+.2f}%/m | lc_t10 IR gross "
                  f"{g['ir_gross']:.2f} net {g['ir_net']:.2f} | beta {g['beta']:+.2f} | {time.time() - t0:.0f}s", flush=True)
    ...
    for model in args.models.split(","):
        ...
        for arm in arms:
            ...
            paired_rows.append({"model": model, "arm": arm, "ic": results[key]["universe_rank_ic"], "ic_control": results[ck]["universe_rank_ic"], **p,
                                "ir_gross": results[key][HEADLINE_VARIANT]["ir_gross"], "ir_gross_control": results[ck][HEADLINE_VARIANT]["ir_gross"],
                                "ir_net": results[key][HEADLINE_VARIANT]["ir_net"], "ir_net_control": results[ck][HEADLINE_VARIANT]["ir_net"]})
    pr = pd.DataFrame(paired_rows)
    if len(pr):
        pr.to_csv(OUT / f"paired_vs_control{args.suffix}.csv", index=False)
        for arm in arms:
            sub = pr[pr["arm"] == arm]
            if arm == "control" or not len(sub):
                continue
            adopt[arm] = {"families_with_paired_t_ge_2": int((sub["t"] >= 2).sum()), "adopt_rule_A_two_families_t_ge_2": bool((sub["t"] >= 2).sum() >= 2),
                          "adopt_rule_B_ic_ge_0.037_and_t_ge_2": bool(any((results[f"{m}__{arm}"]["universe_rank_ic"] >= 0.037) and (results[f"{m}__{arm}"]["universe_rank_ic_t"] >= 2) for m in sub["model"]))}
    (OUT / f"results{args.suffix}.json").write_text(json.dumps(_clean({**results, "_adoption_rule": adopt}), indent=2))
    rows = [{"key": k, "ic": v["universe_rank_ic"], "t": v["universe_rank_ic_t"], "d10_d1": v["d10_minus_d1_pct_per_month"], "ir_gross_t10": v[HEADLINE_VARIANT]["ir_gross"],
             "ir_net_t10": v[HEADLINE_VARIANT]["ir_net"], "ir_gross_free": v["lc_free"]["ir_gross"]} for k, v in results.items()]
    ...
    if len(pr):
        ...
        print(pr[["model", "arm", "ic", "ic_control", "mean_diff", "t", "ir_gross", "ir_gross_control"]].to_string(index=False, float_format=lambda v: f"{v:.3f}"))
        print("\nAdoption rules:", json.dumps(adopt))


# Arms. Recency weights use w = 0.5^(age/half_life) with age = months from the row's target month to the start of the validation window; windows keep only
# the last N months before the validation window; `era_stack3` = ExtraTrees fitted on 3 contiguous month-blocks of the training window and averaged
# (Numerai deep incremental learning), `era_stack3_full` adds the full-window model as a 4th member.
ARMS = {
    "control": {},
    "hl24": {"half_life": 24},
    "hl48": {"half_life": 48},
    ...
    "win24": {"window": 24},
    "win48": {"window": 48},
    "era_stack3": {"fitters": {"et": make_blockwise_et(3, False)}},
    "era_stack3_full": {"fitters": {"et": make_blockwise_et(3, True)}},
}
