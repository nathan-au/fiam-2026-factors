"""
FIAM 2026 - Truncation-invariance audit of the panel's return-derived characteristics, label alignment and a suspicious-IC scan (truncation_audit.py).

Research origin: docs/RESEARCH.md Part II sec 2.9 (AQuA / r/LocalLLaMA 1vxajio: an LLM wrote a feature dividing 'volume so far' by the day's FINAL total; a second LLM reviewed it as causal; only a clean re-split caught it. The fix: make leakage
inexpressible, and test that a feature computed from rows <= t equals the one computed from the full panel). Also the brief's leakage scrutiny. The 'typed causal-operator registry' half of the idea is for AI-written feature-mining loops and is NOT
implemented (no such loop is in scope); every new feature builder in this round instead carries its own truncation test (feat_*.py).

HYPOTHESIS. (i) Every return-derived characteristic in the panel (ret_1_0 .. ret_60_12) can be rebuilt exactly from the monthly `ret` column using rows <= t only, i.e. none uses information from after the characteristic month. (ii) The test is
capable of failing (a deliberately leaky feature must be flagged). (iii) ret_exc_lead1m is the plain next-month excess return. (iv) No characteristic has an implausibly large IC with next-month return (a leakage red flag). Any failure is a finding.

DESIGN. Frozen experiments/largecap harness (copied in below) for the panel / IC machinery. For 3 random dates t in 2021-2026, every characteristic is rebuilt from `ret` rows with eom <= t and compared with the panel's value at t and with the full-panel rebuild.
Negative control: a feature using next month's return. Label check on all consecutive-month pairs. IC scan: mean monthly rank IC of all 147 characteristics with next-month excess return, all stocks and the >= $2B universe; |IC| > 0.10 flagged.

Run:  .venv/bin/python experiments/truncation_audit/truncation_audit.py [--smoke]
Outputs (output/): results.json, suspicious_ic_scan.csv, run.log
"""
...
HERE = Path(__file__).resolve().parent  # experiments/truncation_audit/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Driver: truncation-invariance audit of the panel's return-derived characteristics + label alignment + suspicious-IC scan
# ---------------------------------------------------------------------------
# Lags (in months back from the characteristic month t) of the monthly returns compounded by each panel return characteristic.
# Verified against the panel (max abs difference ~1e-16 for ret_12_1); a value that cannot be rebuilt from rows <= t fails the test.
RET_CHARS = {"ret_1_0": range(0, 1), "ret_3_1": range(1, 3), "ret_6_1": range(1, 6), "ret_9_1": range(1, 9), "ret_12_1": range(1, 12),
             "ret_12_7": range(7, 12), "ret_60_12": range(12, 60)}


def ret_char_builder(p):
    R = wide(p, "ret")
    out = {}
    for name, lags in RET_CHARS.items():
        prod = np.prod(np.stack([1.0 + R.shift(k).to_numpy() for k in lags]), axis=0) - 1.0
        out[name] = pd.DataFrame(prod, index=R.index, columns=R.columns)
    return out


def leaky_builder(p):
    """NEGATIVE CONTROL: a deliberately leaky feature (compounds the NEXT month's return into a 'feature' at t). Must fail the test."""
    R = wide(p, "ret")
    return {"leaky_ret_next_1m": R.shift(-1), "ok_ret_1_0": R}
...
def main():
    ...
    res = {}
    panel = load_panel(["ret", "ret_exc", "ret_exc_lead1m"] + list(RET_CHARS), ctx)

    # (1) Rebuild each return characteristic from rows <= t only and require equality with the panel's own value at t.
    trunc = truncation_test(ret_char_builder, panel, n_dates=3, tol=1e-9)
    print("truncation test on rebuilt characteristics:", json.dumps(_clean(trunc)), flush=True)
    W = ret_char_builder(panel)
    diff = {}
    for name in RET_CHARS:
        theirs = wide(panel, name)
        both = W[name].notna() & theirs.notna()
        d = (W[name].where(both) - theirs.where(both)).abs()
        diff[name] = {"n_compared": int(both.to_numpy().sum()), "max_abs_diff_vs_panel_column": float(np.nanmax(d.to_numpy())),
                      "share_within_1e-9": float((d.to_numpy()[both.to_numpy()] <= 1e-9).mean())}
    print("rebuilt vs panel columns:", json.dumps(_clean(diff)), flush=True)
    res["truncation_rebuilt_characteristics"] = trunc
    res["rebuilt_vs_panel"] = diff

    # (2) Negative control: the test must FLAG a feature that uses the next month's return
    neg = truncation_test(leaky_builder, panel, n_dates=3, tol=1e-9)
    print("negative control (leaky feature):", json.dumps(_clean(neg)), flush=True)
    res["negative_control_leaky_feature"] = neg

    # (3) Label alignment: ret_exc_lead1m(t) must equal ret_exc(t+1) wherever the next calendar month is present
    nxt = wide(panel, "ret_exc").shift(-1)
    lead = wide(panel, "ret_exc_lead1m")
    both = nxt.notna() & lead.notna()
    d = (nxt.where(both) - lead.where(both)).abs().to_numpy()[both.to_numpy()]
    res["label_alignment"] = {"n_compared": int(both.to_numpy().sum()), "share_equal_1e-9": float((d <= 1e-9).mean()), "max_abs_diff": float(d.max()),
                              "note": "mismatches, if any, are where ret_exc_lead1m is not the plain next-month value (e.g. delisting-return handling) - inspected below"}
    print("label alignment:", json.dumps(_clean(res["label_alignment"])), flush=True)
    bad = (~(d <= 1e-9)).sum()
    res["label_alignment"]["n_mismatch"] = int(bad)

    # (4) Suspicious-IC scan over ALL characteristics (cannot rebuild the non-return ones from the panel, so screen instead): mean monthly rank IC
    #     vs next-month excess return on the full sample and on the >= $2B universe; anything with |IC| > 0.10 is flagged for manual review.
    tm = ctx.tm
    yy = ctx.y
    rows = []
    for j, v in enumerate(ctx.stock_vars):
        x = ctx.Xall[:, j]
        ic_all = monthly_rank_ic(tm, x, yy)
        u = ctx.u
        ic_u = monthly_rank_ic(tm[u], x[u], yy[u])
        rows.append({"characteristic": v, "ic_all_stocks": float(ic_all.mean()), "t_all": tstat(ic_all), "ic_universe": float(ic_u.mean()), "t_universe": tstat(ic_u)})
    scan = pd.DataFrame(rows).sort_values("ic_all_stocks", key=np.abs, ascending=False)
    scan["flag_abs_ic_gt_0.10"] = scan[["ic_all_stocks", "ic_universe"]].abs().max(axis=1) > 0.10
    scan.to_csv(OUT / "suspicious_ic_scan.csv", index=False)
    res["suspicious_ic_scan"] = {"n_characteristics": int(len(scan)), "n_flagged_abs_ic_gt_0.10": int(scan["flag_abs_ic_gt_0.10"].sum()),
                                 "top10_by_abs_ic_all_stocks": scan.head(10)[["characteristic", "ic_all_stocks", "ic_universe"]].to_dict("records"),
                                 "max_abs_ic_universe": float(scan["ic_universe"].abs().max())}
    print("top characteristics by |IC| (all stocks):\n" + scan.head(10)[["characteristic", "ic_all_stocks", "t_all", "ic_universe", "t_universe"]].to_string(index=False, float_format=lambda v: f"{v:.4f}"), flush=True)
    res["coverage_note"] = ("Only the 7 return-derived characteristics can be rebuilt from the panel's own `ret` column; the other characteristics need raw daily / "
                            "accounting data that is not in the panel, so they are screened (IC scan) rather than proven point-in-time.")
