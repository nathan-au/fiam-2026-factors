"""
FIAM 2026 - State-dependent borrow-cost stress and forced-cover shocks on the composite book (borrow_stress.py).

Research origin: docs/RESEARCH.md Part II sec 2.10 (r/algotrading 1w9yp8g, r/quant 1lpzdwr: 'the reason you want to short is correlated with the borrow becoming expensive or unavailable'; a backtest using median borrow
'systematically understates cost in the exact states where the edge is largest'; stress borrow at 2x/5x/10x, add a locate-reject / forced-cover state, screen shorts by price and ADV). The panel has no borrow data, so this
stresses assumptions instead of measuring them.

HYPOTHESIS. The composite's net IR (0.54 at the assumed 30/75/200 bp tiered borrow) survives a 2x-5x borrow multiplier and a forced cover of a few percent of the short book, because its shorts are large caps (median $7B) - i.e.
the short leg is not where the edge (or the fragility) lives. It should NOT survive if the expensive-borrow state coincides with the strongest short theses. There is no pre-registered adopt rule: this is a robustness test; the
outcome is the IR / drawdown under each scenario. A book whose net IR turns negative under a plausible stress is flagged.

DESIGN. Frozen experiments/largecap harness (copied in below); frozen composite; LP `lc_t10`. (1) uniform borrow x1/2/5/10/20 on all shorts; (2) state-dependent: shorts in the lowest-score quartile of the held shorts (strongest
thesis) pay x2/5/10; (3) LP re-solved with no shorts below $10 / $20; (4) forced cover: each month 4% of the short names (random) covered at a +15% / +30% adverse move, 500 Monte-Carlo draws; (5) Jan-2021 squeeze month reported explicitly.

Run:  .venv/bin/python experiments/borrow_stress/borrow_stress.py [--draws 500] [--smoke]
Outputs (output/): results.json, summary.csv, run.log, portfolio_{holdings,returns}_{base_t10,short_px10,short_px20}_lc_comp.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/borrow_stress/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Driver: state-dependent borrow stress on the frozen composite book
# ---------------------------------------------------------------------------
...
def ir_stats(ret):
    ...
    act = ret - 0.04 / 12.0
    dd = drawdown_series(ret.reset_index(drop=True))
    return {"ir": float(np.sqrt(12) * act.mean() / act.std()), "cagr_pct": float(100 * ((1 + ret).prod() ** (12 / len(ret)) - 1)),
            "max_dd_pct": float(100 * dd.min()), "worst_month_pct": float(100 * ret.min())}
...
def main():
    ...
    preds = base_obj.preds(base_obj.comp)
    row0, net0, res0 = run_variant(preds, "base_t10", base, "comp")
    fr = pd.read_csv(OUT / f"portfolio_returns_base_t10_{TAG}_comp.csv", parse_dates=["target_month"]).set_index("target_month")
    hold = pd.read_csv(OUT / f"portfolio_holdings_base_t10_{TAG}_comp.csv", parse_dates=["target_month"])
    gross, tcost, bcost = fr["port_excess_ret"], fr["trade_cost"], fr["borrow_cost"]
    assert np.allclose(gross - tcost - bcost, fr["net_port_excess_ret"]), "cost decomposition does not reproduce the harness net return"
    out = {"baseline": {"ir_gross": row0["ir_gross"], "ir_net": row0["ir_net"]}, "borrow_multiplier": {}, "strong_thesis_multiplier": {},
           "short_price_screen": {}, "forced_cover": {}}
    print(f"baseline net IR {row0['ir_net']:.3f}; avg borrow cost {1e4 * bcost.mean():.1f} bp of NAV/month ({12e4 * bcost.mean() / 1:.0f} bp/yr)", flush=True)

    # (1) uniform borrow multiplier on every short (the practitioners' 2x / 5x / 10x stress)
    for m in (1, 2, 5, 10, 20):
        out["borrow_multiplier"][m] = ir_stats(gross - tcost - m * bcost)
        print(f"  borrow x{m:<2d}: IR {out['borrow_multiplier'][m]['ir']:+.2f}  CAGR {out['borrow_multiplier'][m]['cagr_pct']:+.1f}%  maxDD {out['borrow_multiplier'][m]['max_dd_pct']:.0f}%", flush=True)

    # (2) STATE-dependent: the strongest short theses (lowest-score quartile of the held shorts each month) get the expensive borrow
    s = hold[hold["weight"] < 0].copy()
    s["b"] = cost_bps(s["raw_me"].to_numpy())[1] / 1e4 / 12.0 * s["weight"].abs()
    s["strong"] = s.groupby("target_month")["pred"].transform(lambda x: x <= x.quantile(0.25))
    for m in (2, 5, 10):
        adj = s["b"].where(~s["strong"], s["b"] * m).groupby(s["target_month"]).sum()
        out["strong_thesis_multiplier"][m] = ir_stats(gross - tcost - adj.reindex(gross.index).fillna(0.0))
        print(f"  strong-thesis shorts pay borrow x{m:<2d}: IR {out['strong_thesis_multiplier'][m]['ir']:+.2f}  maxDD {out['strong_thesis_multiplier'][m]['max_dd_pct']:.0f}%", flush=True)

    # (3) short-side price screen: no shorts below $10 / $20 (LP re-solved), with the base and a 5x borrow cost
    for pmin in (10.0, 20.0):
        name = f"short_px{int(pmin)}"
        row, net, res = run_variant(preds, name, {**base, "short_min_price": pmin}, "comp")
        f2 = pd.read_csv(OUT / f"portfolio_returns_{name}_{TAG}_comp.csv", parse_dates=["target_month"]).set_index("target_month")
        out["short_price_screen"][name] = {"base": ir_stats(f2["net_port_excess_ret"]),
                                           "borrow_x5": ir_stats(f2["port_excess_ret"] - f2["trade_cost"] - 5 * f2["borrow_cost"]),
                                           "short_median_mcap_musd": row["short_median_mcap_musd"], "gross_ir": row["ir_gross"]}
        print(f"  {name}: net IR {row['ir_net']:.2f} (gross {row['ir_gross']:.2f}); with borrow x5 IR {out['short_price_screen'][name]['borrow_x5']['ir']:+.2f}", flush=True)

    # (4) forced-cover shock: each month 4% of the short NAMES are covered at a bad price (+shock adverse move on their position)
    rng = np.random.default_rng(0)
    months = list(gross.index)
    per_month = {m: g["weight"].abs().to_numpy() for m, g in s.groupby("target_month")}
    for shock in (0.15, 0.30):
        irs, dds = [], []
        for _ in range(args.draws):
            loss = np.array([(rng.choice(per_month[m], size=max(1, int(round(0.04 * len(per_month[m])))), replace=False).sum() * shock)
                             if m in per_month else 0.0 for m in months])
            st = ir_stats(gross.to_numpy() - tcost.to_numpy() - bcost.to_numpy() - loss)
            irs.append(st["ir"])
            dds.append(st["max_dd_pct"])
        out["forced_cover"][shock] = {"ir_mean": float(np.mean(irs)), "ir_p5": float(np.quantile(irs, 0.05)), "ir_p95": float(np.quantile(irs, 0.95)),
                                      "max_dd_mean_pct": float(np.mean(dds)), "max_dd_p5_pct": float(np.quantile(dds, 0.05)), "draws": args.draws}
        print(f"  forced cover of 4% of shorts at +{int(100 * shock)}%: IR mean {np.mean(irs):+.2f} (p5 {np.quantile(irs, .05):+.2f}), maxDD mean {np.mean(dds):.0f}% (p5 {np.quantile(dds, .05):.0f}%)", flush=True)

    # (5) the squeeze month, explicitly
    jan = pd.Timestamp("2021-01-31")
    out["jan_2021"] = {"net_return_pct": float(100 * fr.loc[jan, "net_port_excess_ret"]) if jan in fr.index else None,
                       "short_leg_pct": float(100 * fr.loc[jan, "short_leg_ret"]) if jan in fr.index else None}
    print("  Jan-2021:", out["jan_2021"], flush=True)
    (OUT / "results.json").write_text(json.dumps(_clean(out), indent=2))
    rows = [{"scenario": "baseline", "ir": row0["ir_net"]}] + [{"scenario": f"borrow x{k}", **v} for k, v in out["borrow_multiplier"].items()] + \
           [{"scenario": f"strong-thesis borrow x{k}", **v} for k, v in out["strong_thesis_multiplier"].items()] + \
           [{"scenario": f"{k} base", **v["base"]} for k, v in out["short_price_screen"].items()] + \
           [{"scenario": f"{k} + borrow x5", **v["borrow_x5"]} for k, v in out["short_price_screen"].items()] + \
           [{"scenario": f"forced cover +{int(100 * k)}% (mean of draws)", "ir": v["ir_mean"], "max_dd_pct": v["max_dd_mean_pct"]} for k, v in out["forced_cover"].items()]
