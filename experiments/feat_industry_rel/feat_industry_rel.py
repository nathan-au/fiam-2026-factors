"""
FIAM 2026 - Industry-relative momentum and industry-relative composites on the large-cap harness (feat_industry_rel.py).

Research origin: docs/RESEARCH.md Part I sec 3.3 items 3-4 (residual / industry-neutral / 52-week-high-neutral momentum; sector-adjusting characteristics 'boosts [the better result] from 20% to 78% of the time', largest for value-weighted large-cap strategies) and Round B.

HYPOTHESIS. Industry-relative momentum variants (defensible replacements for the raw momentum the project dropped) add IC to the composite, and ranking every factor WITHIN industry (a sector-relative composite, or a 50/50 blend) raises the composite's universe IC. Replacement composites are compared with the frozen composite directly (paired IC, LP); momentum variants are additive blocks.
...
Run:  .venv/bin/python experiments/feat_industry_rel/feat_industry_rel.py [--nperm 200] [--smoke]
...
HERE = Path(__file__).resolve().parent  # experiments/feat_industry_rel/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Industry-relative block builders (Fama-French 49 industries from the panel, known at the characteristic month).
# ---------------------------------------------------------------------------


def ind_inputs_builder(p):
    return {"cum_12_1": path_cum(wide(p, "ret")), "ff49": wide(p, "ff49")}


def path_cum(R):
    """Compounded return of months t-11..t-1 (identical to the panel's ret_12_1)."""
    return np.prod(np.stack([1.0 + R.shift(k).to_numpy() for k in range(1, 12)]), axis=0) - 1.0 + 0 * R  # keeps index/columns


def make_blocks(ctx):
    panel = load_panel(["ret", "ff49"], ctx)
    trunc = truncation_test(ind_inputs_builder, panel)
    W = ind_inputs_builder(panel)
    cum = wide_to_meta(pd.DataFrame(W["cum_12_1"]), ctx.meta)
    ff = wide_to_meta(W["ff49"], ctx.meta)
    ff = np.where(np.isfinite(ff), ff, -1).astype(int)
    sec = ctx.meta["sector"].to_numpy()
    ind_mom = ind_mean(ctx, cum, ff)
    # 52-week-high-neutral momentum: residual of 12-1 momentum rank on the George-Hwang nearness-to-52w-high rank (prc_highprc_252d)
    mrk, hrk = urank(cum, ctx), urank(xall_te(ctx, "prc_highprc_252d"), ctx)
    resid52 = np.zeros(len(mrk))
    d = pd.DataFrame({"m": ctx.meta["target_month"].to_numpy(), "a": mrk, "h": hrk})[ctx.um]
    for m, g in d.groupby("m"):
        h = g["h"].to_numpy() - g["h"].mean()
        a = g["a"].to_numpy()
        resid52[g.index.to_numpy()] = a - a.mean() - (h @ (a - a.mean())) / (h @ h) * h
    blocks = {
        "ind_rel_mom": {"raw": cum - ind_mom, "sign": +1, "note": "12-1 momentum minus its FF49 universe mean (industry-relative / residual momentum)"},
        "ind_mom_12_1": {"raw": ind_mom, "sign": +1, "note": "FF49 industry 12-1 momentum (Moskowitz-Grinblatt)"},
        "mom_52wk_high_neutral": {"raw": resid52, "sign": +1, "note": "12-1 momentum rank residualised on the nearness-to-52-week-high rank"},
        "resff3_12_1_existing": {"raw": xall_te(ctx, "resff3_12_1"), "sign": +1, "note": "REFERENCE: the panel's existing FF3-residual momentum"},
    }
    # Replacement composites: rank every factor WITHIN industry (sector-relative characteristics), then the same 7-group rule.
    te_ff = ff
    G_ff = grp_scores(ctx, ctx.te_idx, key=te_ff).mean(axis=1)
    G_sec = grp_scores(ctx, ctx.te_idx, key=sec).mean(axis=1)
    G_ff_sc, base_c = G_ff, grp_scores(ctx, ctx.te_idx).mean(axis=1)
    alts = {
        "comp_indrel_gics2": G_sec,
        "comp_indrel_ff49": G_ff_sc,
        "comp_half_indrel_ff49": 0.5 * base_c + 0.5 * G_ff_sc,
        "comp_half_indrel_gics2": 0.5 * base_c + 0.5 * G_sec,
    }
    return blocks, alts, trunc
