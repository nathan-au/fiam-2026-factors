...
HERE = Path(__file__).resolve().parent  # experiments/feat_tnic_peers/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# TNIC-3 (Hoberg-Phillips text-based network industries) peer signals. EXTERNAL DATA: cache/tnic/tnic3_data.zip (151 MB, downloaded on first run
# from hobergphillips.tuck.dartmouth.edu; the 680 MB text member is streamed from the zip, never unpacked). Timing: the network of business-description
# year `byear` is applied only to months of year >= byear + 1 (all of that year's 10-K text is public by the end of byear); the database ends at byear 2023,
# so months of 2025-26 use the 2023 network (stale by 2-3 years: reported separately via the by-year IC). Peers = the 5 highest-similarity rivals of the
# stock's gvkey that have a return at month t and market cap >= $500M; needs >= 3 such peers.
# ---------------------------------------------------------------------------
import urllib.request
import zipfile

TNIC_ZIP = CACHE / "tnic" / "tnic3_data.zip"
TNIC_URL = "https://hobergphillips.tuck.dartmouth.edu/idata/tnic3_data.zip"
K_TNIC, MIN_PEERS, PEER_MIN_ME, TNIC_LAST_YEAR = 5, 3, 500.0, 2023


def load_tnic_top(min_byear=2019):
    if not TNIC_ZIP.exists():
        TNIC_ZIP.parent.mkdir(parents=True, exist_ok=True)
        print("  downloading TNIC-3 (151 MB)...", flush=True)
        req = urllib.request.Request(TNIC_URL, headers={"User-Agent": "fiam-2026-research-bot"})
        with urllib.request.urlopen(req, timeout=600) as r, open(TNIC_ZIP, "wb") as out:
            out.write(r.read())
    out = []
    with zipfile.ZipFile(TNIC_ZIP) as z:
        name = [n for n in z.namelist() if n.lower().endswith("_data.txt") and "readme" not in n.lower()][0]
        with z.open(name) as f:
            for ch in pd.read_csv(f, sep="\t", chunksize=3_000_000, dtype={"year": int, "gvkey1": int, "gvkey2": int, "score": float}):
                ch = ch.rename(columns={"year": "byear"})  # column is called `year` in the data file (business-description year)
                ch = ch[ch["byear"] >= min_byear]
                if len(ch):
                    out.append(ch)
    t = pd.concat(out).sort_values(["byear", "gvkey1", "score"], ascending=[True, True, False])
    return t.groupby(["byear", "gvkey1"]).head(3 * K_TNIC).reset_index(drop=True)  # keep extra rivals: some will lack a return / size at month t


def tnic_builder_factory(top):
    def builder(p):
        R, ME = wide(p, "ret"), wide(p, "me")
        cum = np.prod(np.stack([1.0 + R.shift(k).to_numpy() for k in range(1, 12)]), axis=0) - 1.0
        cum = pd.DataFrame(cum, index=R.index, columns=R.columns)
        gv = p.assign(gvkey_i=pd.to_numeric(p["gvkey"], errors="coerce")).dropna(subset=["gvkey_i"])
        nan = lambda: pd.DataFrame(np.nan, index=R.index, columns=R.columns)
        out = {"tnic_peer_ret_1m": nan(), "tnic_peer_gap_1m": nan(), "tnic_peer_ret_12_1": nan(), "ind_ret_1m_control": nan()}
        ff = wide(p, "ff49")
        for t in R.index[R.index >= pd.Timestamp("2020-12-31")]:
            byear = min(t.year - 1, TNIC_LAST_YEAR)
            net = top[top["byear"] == byear]
            if net.empty:
                continue
            rows = gv[gv["eom"] == t].sort_values("me", ascending=False).drop_duplicates("gvkey_i")  # one permno per gvkey (largest share class)
            g2p = dict(zip(rows["gvkey_i"].astype(int), rows["permno"]))
            r1, c12, me = R.loc[t], cum.loc[t], ME.loc[t]
            good = (r1.notna() & (me >= PEER_MIN_ME)).to_dict()
            for g1, grp in net.groupby("gvkey1"):
                i = g2p.get(g1)
                if i is None or i not in r1.index or not np.isfinite(r1[i]):
                    continue
                peers = [g2p[g] for g in grp["gvkey2"] if g in g2p and good.get(g2p[g], False) and g2p[g] != i][:K_TNIC]
                if len(peers) < MIN_PEERS:
                    continue
                p1 = float(np.mean(r1[peers]))
                out["tnic_peer_ret_1m"].loc[t, i] = p1
                out["tnic_peer_gap_1m"].loc[t, i] = r1[i] - p1
                out["tnic_peer_ret_12_1"].loc[t, i] = float(np.nanmean(c12[peers]))
        # control: FF49 industry mean return at t (all stocks with >= $500M), excluding self
        for t in R.index[R.index >= pd.Timestamp("2020-12-31")]:
            r1, f, me = R.loc[t], ff.loc[t], ME.loc[t]
            ok = r1.notna() & f.notna() & (me >= PEER_MIN_ME)
            d = pd.DataFrame({"r": r1[ok], "f": f[ok]})
            s, n = d.groupby("f")["r"].transform("sum"), d.groupby("f")["r"].transform("count")
            out["ind_ret_1m_control"].loc[t, d.index] = ((s - d["r"]) / (n - 1)).where(n > 3)
        return out
    return builder
...
def make_blocks(ctx):
    top = load_tnic_top()
    print(f"  TNIC top rivals table: {len(top):,} rows, byears {int(top.byear.min())}-{int(top.byear.max())}", flush=True)
    panel = load_panel(["ret", "me", "gvkey", "ff49"], ctx)
    builder = tnic_builder_factory(top)
    trunc = truncation_test(builder, panel, n_dates=2)
    W = builder(panel)
    v = {k: wide_to_meta(w, ctx.meta) for k, w in W.items()}
    blocks = {
        "tnic_peer_ret_1m": {"raw": v["tnic_peer_ret_1m"], "sign": +1, "note": "mean month-t return of the 5 most similar TNIC-3 rivals (Hoberg-Phillips; lagged >= 1 year network)"},
        "tnic_peer_gap_1m": {"raw": v["tnic_peer_gap_1m"], "sign": -1, "note": "own month-t return minus TNIC peers' (laggards catch up)"},
        "tnic_peer_ret_12_1": {"raw": v["tnic_peer_ret_12_1"], "sign": +1, "note": "TNIC peers' 12-1 momentum ('substantially more significant than SIC-based peers or own-firm momentum')"},
        "ind_ret_1m_control": {"raw": v["ind_ret_1m_control"], "sign": +1, "note": "CONTROL: FF49 industry mean month-t return"},
    }
    top.to_csv(OUTPUT / "tnic3_top_rivals_byear2019plus.csv.gz", index=False)
    return blocks, {}, trunc
