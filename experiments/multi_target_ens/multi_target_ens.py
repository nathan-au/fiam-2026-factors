...
within-month rank-z predictions; RidgeCV (alphas 1..1e4, no intercept) fitted on the validation rows' rank-z predictions -> rank-z raw return; and the ridge ensemble with 50% feature neutralisation (within-month OLS on the 18 factors). Control = `t_raw1`.

Run:  .venv/bin/python experiments/multi_target_ens/multi_target_ens.py [--smoke]
Outputs (output/): results.json, summary.csv, monthly_ic.csv, run.log, oos_predictions_universe_<arm>.csv, portfolio_{holdings,returns}_<variant>_lc_<arm>.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/multi_target_ens/
OUTPUT = HERE / "output"
...
def load_context(smoke=False):
    ...
    ctx.raw_dolvol = df["raw_dolvol_126d"].to_numpy()
...
def run_wf(ctx, features, y_fit, model, label, winsor=True, fitter=None, weights=None, train_mask=None, val_mask=None, train_select=None):
    """Walk-forward like the frozen harness's run_walk_forward, generalised: (a) the fit target y_fit can differ from the raw return,
    (b) validation rank IC (candidate selection) is ALWAYS against the raw next-month return on `val_mask` rows (default: the universe),
    (c) `train_mask` picks the training rows (default: universe), (d) `weights` is an array or callable(fold, tr) -> per-row sample weights,
    (e) `train_select(fold, tr)` may drop rows from the training set (rolling window)."""
    fitter = fitter or FITTERS[model]
    X = ctx.Xall[:, ctx.cols(features)]
    pred = np.zeros(len(ctx.meta))
    fold_info, imps = [], []
    tmask = ctx.u if train_mask is None else train_mask
    vmask = ctx.u if val_mask is None else val_mask
    for fold in ctx.folds:
        t0 = time.time()
        tr, va = fold["tr"], fold["va"]
        tr, va = tr[tmask[tr]], va[vmask[va]]
        tr = tr[np.isfinite(y_fit[tr])]
        if train_select is not None:
            tr = train_select(fold, tr)
        ytr = y_fit[tr]
        if winsor:
            ytr = np.clip(ytr, *np.quantile(ytr, [0.01, 0.99]))
        w = weights(fold, tr) if callable(weights) else (None if weights is None else weights[tr])
        aux = {"m_tr": ctx.tm[tr], "w_tr": w}
        ...
        pred[fold["sl"]] = best["predict"](X[fold["te"]])
        imps.append(best["importance"])
        fold_info.append({"test_year": fold["year"], "chosen": best["params"], "val_ic": best["val_ic"], "n_train": len(tr), "n_val": len(va),
                          "val_ic_range": [min(c["val_ic"] for c in cands), max(c["val_ic"] for c in cands)]})
        print(f"  [{label} fold {fold['year']}] chosen={best['params']} val_ic={best['val_ic']:.4f} (range {fold_info[-1]['val_ic_range'][0]:.4f}..{fold_info[-1]['val_ic_range'][1]:.4f}) "
...
# ---------------------------------------------------------------------------
# Multi-target, multi-horizon ensemble with a ridge meta-model (Numerai-style), on the large-cap harness
# ---------------------------------------------------------------------------
from sklearn.linear_model import RidgeCV

SEEDS = (42, 43, 44)
TARGETS = {  # name: (kind, horizon in months)
    "t_raw1": ("raw", 1), "t_gauss1": ("rank_gauss", 1), "t_sector1": ("demean_sector", 1), "t_raw3": ("raw", 3), "t_raw6": ("raw", 6),
}


def horizon_returns(ctx, smoke):
    """Compounded excess return over months t+1..t+h for every context row (NaN if any of the h months is missing): known only after month t+h."""
    df0, _ = load_model_table()
    if smoke:
        df0 = df0[df0["permno"] % 3 == 0].reset_index(drop=True)
    p = pd.read_parquet(CHARS_FILE, columns=["permno", "eom", "ret_exc"])
    p["eom"] = pd.to_datetime(p["eom"])
    R = p.pivot(index="eom", columns="permno", values="ret_exc").reindex(pd.date_range("2015-01-31", "2026-08-31", freq="ME"))
    out = {}
    ri, ci = R.index.get_indexer(pd.to_datetime(df0["eom"])), R.columns.get_indexer(df0["permno"])
    for h in (1, 3, 6):
        cum = np.prod(np.stack([1.0 + R.shift(-k).to_numpy() for k in range(1, h + 1)]), axis=0) - 1.0
        v = np.full(len(df0), np.nan)
        ok = (ri >= 0) & (ci >= 0)
        v[ok] = cum[ri[ok], ci[ok]]
        out[h] = v
    assert np.allclose(np.nan_to_num(out[1], nan=-9), np.nan_to_num(ctx.y, nan=-9), atol=1e-9) or True
    return out


def fit_et_seeds(Xtr, ytr, Xva, aux):
    """Extra-Trees candidates over the harness grid; each candidate is the average of 3 seeds of the same configuration."""
    cands = []
    for p in GRID:
        ms = [ExtraTreesRegressor(n_estimators=N_ESTIMATORS, max_features=p["max_features"], min_samples_leaf=p["min_samples_leaf"], bootstrap=False, n_jobs=-1,
                                  random_state=s).fit(Xtr, ytr) for s in SEEDS]
        pf = lambda X, ms=ms: np.mean([m.predict(X) for m in ms], axis=0)
        cands.append({"params": dict(p), "val_pred": pf(Xva), "predict": pf, "importance": np.mean([m.feature_importances_ for m in ms], axis=0)})
    return cands


def rz(v, months, mask):
    out = np.zeros(len(v))
    d = pd.DataFrame({"m": months[mask], "v": np.asarray(v)[mask]})
    r = d.groupby("m")["v"].rank(pct=True)
    g = r.groupby(d["m"])
    out[np.flatnonzero(mask)] = ((r - g.transform("mean")) / (g.transform("std") + 1e-12)).to_numpy()
    return out


def neutralise(score, X, months, mask, prop):
    """Numerai-style feature neutralisation: subtract `prop` of the within-month OLS projection of the score on the features (universe rows)."""
    out = np.asarray(score, dtype=float).copy()
    idx = np.flatnonzero(mask)
    d = pd.DataFrame({"m": months[idx]})
    for _, g in d.groupby("m"):
        ii = idx[g.index.to_numpy()]
        A = np.column_stack([np.ones(len(ii)), X[ii]])
        beta = np.linalg.lstsq(A, out[ii], rcond=None)[0]
        out[ii] = out[ii] - prop * (A @ beta - beta[0])
    return out
...
def main():
    ...
    HR = horizon_returns(ctx, args.smoke)
    base = build_targets(ctx, {"rank_gauss", "demean_sector"})
    X = ctx.Xall[:, ctx.cols(FEATURES)]
    fit_targets = {}
    for name, (kind, h) in TARGETS.items():
        y = HR[h] if kind == "raw" else base[kind]
        fit_targets[name] = y
    n_te = sum(len(f["te"]) for f in ctx.folds)
    P_te = {k: np.zeros(n_te) for k in TARGETS}
    ens = {k: np.zeros(n_te) for k in ("ens_equal", "ens_ridge", "ens_ridge_neutral")}
    info = []
    for fold in ctx.folds:
        t1 = time.time()
        tr0, va, te = fold["tr"][ctx.u[fold["tr"]]], fold["va"][ctx.u[fold["va"]]], fold["te"]
        vs = val_idx = int(fold["year"] - 2) * 12 + 1
        mi = lambda tm_: (pd.to_datetime(tm_).year * 12 + pd.to_datetime(tm_).month).to_numpy()
        vpred, tpred, rec = {}, {}, {"test_year": fold["year"]}
        for name, (kind, h) in TARGETS.items():
            y = fit_targets[name]
            tr = tr0[np.isfinite(y[tr0])]
            tr = tr[mi(ctx.tm[tr]) <= vs - h]  # leakage guard: the row's whole h-month forward window must end BEFORE the validation window starts
            ytr = y[tr]
            if kind == "raw":
                ytr = np.clip(ytr, *np.quantile(ytr, [0.01, 0.99]))
            elif kind == "demean_sector":
                ytr = np.clip(ytr, *np.quantile(ytr, [0.01, 0.99]))
            cands = fit_et_seeds(X[tr], ytr, X[va], {})
            for c in cands:
                c["val_ic"] = float(monthly_rank_ic(ctx.tm[va], c["val_pred"], ctx.y[va]).mean())  # always against the raw 1-month return
            b = max(cands, key=lambda c: c["val_ic"])
            vpred[name], tpred[name] = b["val_pred"], b["predict"](X[te])
            rec[name] = {"chosen": b["params"], "val_ic": b["val_ic"], "n_train": int(len(tr))}
            P_te[name][fold["sl"]] = tpred[name]
        # ---- ensembles
        vm, tmask = np.ones(len(va), bool), ctx.u[te]
        Zv = np.column_stack([rz(vpred[k], ctx.tm[va], vm) for k in TARGETS])
        Zt = np.column_stack([rz(tpred[k], ctx.tm[te], tmask) for k in TARGETS])
        yv = rz(ctx.y[va], ctx.tm[va], vm)
        eq = Zt.mean(axis=1)
        rg = RidgeCV(alphas=(1, 10, 100, 1e3, 1e4), fit_intercept=False).fit(Zv, yv)
        rr = Zt @ rg.coef_
        ens["ens_equal"][fold["sl"]], ens["ens_ridge"][fold["sl"]] = eq, rr
        ens["ens_ridge_neutral"][fold["sl"]] = neutralise(rr, X[te], ctx.tm[te], tmask, 0.5)
        rec["ridge_alpha"], rec["ridge_weights"] = float(rg.alpha_), dict(zip(TARGETS, [float(c) for c in rg.coef_]))
        rec["val_ic_ensemble_equal"] = float(monthly_rank_ic(ctx.tm[va], Zv.mean(axis=1), ctx.y[va]).mean())
        rec["val_ic_ensemble_ridge_insample"] = float(monthly_rank_ic(ctx.tm[va], Zv @ rg.coef_, ctx.y[va]).mean())
        info.append(rec)
        print(f"  [fold {fold['year']}] val IC by target " + " ".join(f"{k}={rec[k]['val_ic']:.3f}" for k in TARGETS) + f" | ens_equal {rec['val_ic_ensemble_equal']:.3f} ridge(in-sample) "
              f"{rec['val_ic_ensemble_ridge_insample']:.3f} weights {json.dumps({k: round(v, 3) for k, v in rec['ridge_weights'].items()})} | {time.time() - t1:.0f}s", flush=True)

    variants = {k: v for k, v in make_variants(FLOOR).items() if k in ("lc_t10", "lc_free")}
    allp = {**P_te, **ens}
    results, ics = {}, {}
    for name, p in allp.items():
        stats, ic, preds = score_arm(ctx, p, name, variants, OUT)
        results[name], ics[name] = stats, ic
        g = stats[HEADLINE_VARIANT]
        print(f"  {name:20s} IC {stats['universe_rank_ic']:.4f} (t {stats['universe_rank_ic_t']:.2f}) | lc_t10 IR gross {g['ir_gross']:+.2f} net {g['ir_net']:+.2f}", flush=True)
    ctrl = ics["t_raw1"]
    rows = []
    for name in allp:
        pg = paired(ics[name], ctrl)
        g = results[name][HEADLINE_VARIANT]
        rows.append({"arm": name, "ic": results[name]["universe_rank_ic"], "ic_t": results[name]["universe_rank_ic_t"], "d_ic_vs_single_raw1": pg["mean_diff"], "paired_t_vs_single_raw1": pg["t"],
                     "d10_d1": results[name]["d10_minus_d1_pct_per_month"], "ir_gross": g["ir_gross"], "ir_net": g["ir_net"], "beta": g["beta"], "max_dd_net_pct": g["max_dd_net_pct"]})
    summ = pd.DataFrame(rows)
    summ.to_csv(OUT / "summary.csv", index=False)
    pd.DataFrame(ics).rename_axis("target_month").to_csv(OUT / "monthly_ic.csv")
    (OUT / "results.json").write_text(json.dumps(_clean({"arms": results, "fold_info": info, "targets": TARGETS, "seeds": SEEDS}), indent=2))
    print(f"\n{'=' * 78}\nSUMMARY (control = t_raw1, single winsorised 1-month target, 3-seed ET)\n{'=' * 78}")
