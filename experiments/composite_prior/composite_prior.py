...
DESIGN. Frozen experiments/largecap harness (copied in below): universe, 18 factors, walk-forward folds (expanding train, 2-year validation, annual refit), LP `lc_t10`. Per fold: slope a of the month-demeaned winsorised return on the composite, floored at +0.005 so the prior can never flip sign (a first run without the floor was invalid: see README); arms
  ...
  et_resid        prediction = a*composite + s*ExtraTrees(residual y - a*composite), s in {0, .25, .5, 1} and the ET grid chosen on validation IC (et_resid_forced_s1 = s = 1)
  ridge_prior     ridge on the composite's own 18 universe-ranked inputs shrunk toward the composite weights; lambda in {1e2..1e5, inf} chosen on validation IC (inf = composite)
  orth_alone / comp_plus_orth   H2: the ET-on-residual output with the composite direction projected out each month (alone, and blended 1:1 in z-units with the composite)
Test rows are never used for selection. The composite itself is the control (`comp`).

Run:  .venv/bin/python experiments/composite_prior/composite_prior.py [--seed 42] [--smoke]
Outputs (output/): results.json, summary.csv, monthly_ic.csv, fold_selection.csv, run.log, portfolio_{holdings,returns}_lc_t10_lc_<arm>.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/composite_prior/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Experiment code: composite-as-prior. ML may only add what survives validation; "no ML" is always a candidate.
# ---------------------------------------------------------------------------
import lightgbm as lgb

LGB_PRIOR_GRID = [{"num_leaves": 4, "min_child_samples": 2000}, {"num_leaves": 7, "min_child_samples": 500}]
LGB_PRIOR_ROUNDS, LGB_PRIOR_CK = 50, (10, 25, 50)
RIDGE_LAMBDAS = (1e2, 1e3, 1e4, 1e5, np.inf)  # inf = pure composite weights
A_FLOOR = 0.005  # the composite is a no-fit rule with a pre-specified SIGN: its scale must stay positive (about half the typical fitted slope of ~0.01)


def universe_ranked_factors(ctx):
    """The 18 factors re-ranked within (month, universe) to [-1,1], the composite's own inputs (0 outside the universe)."""
    X = pd.DataFrame(ctx.Xall[:, ctx.cols(FEATURES)], columns=FEATURES)
    X["m"] = ctx.tm
    sub = X[ctx.u]
    R = sub.groupby("m")[FEATURES].rank(pct=True) * 2 - 1
    out = np.zeros((len(X), len(FEATURES)))
    out[np.flatnonzero(ctx.u)] = R.to_numpy()
    return out


def mz(v, months, mask):
    """Within-month z-score over masked rows; 0 elsewhere."""
    out = np.zeros(len(v))
    d = pd.DataFrame({"m": months[mask], "v": np.asarray(v)[mask]})
    g = d.groupby("m")["v"]
    out[np.flatnonzero(mask)] = ((d["v"] - g.transform("mean")) / (g.transform("std") + 1e-12)).to_numpy()
    return out


def orth_fast(v, c, months, mask):
    out = np.zeros(len(v))
    m_idx = np.flatnonzero(mask)
    d = pd.DataFrame({"m": months[m_idx], "v": np.asarray(v)[m_idx], "c": np.asarray(c)[m_idx], "i": m_idx})
    d["vc"] = d["v"] - d.groupby("m")["v"].transform("mean")
    d["cc"] = d["c"] - d.groupby("m")["c"].transform("mean")
    num = (d["vc"] * d["cc"]).groupby(d["m"]).transform("sum")
    den = (d["cc"] ** 2).groupby(d["m"]).transform("sum")
    out[d["i"].to_numpy()] = (d["vc"] - num / den * d["cc"]).to_numpy()
    return out


def run_prior(ctx, C, Xu, W0):
    X = ctx.Xall[:, ctx.cols(FEATURES)]
    n_te = sum(len(f["te"]) for f in ctx.folds)
    names = ["comp", "lgbm_init", "lgbm_init_forced50", "et_resid", "et_resid_forced_s1", "ridge_prior", "orth_alone", "comp_plus_orth"]
    pred = {k: np.zeros(n_te) for k in names}
    info = []
    for fold in ctx.folds:
        t0 = time.time()
        tr, va, te = fold["tr"][ctx.u[fold["tr"]]], fold["va"][ctx.u[fold["va"]]], fold["te"]
        tr = tr[np.isfinite(ctx.y[tr])]
        ydm = pd.Series(ctx.y[tr]).groupby(ctx.tm[tr]).transform(lambda s: s - s.mean()).to_numpy()
        ydm = np.clip(ydm, *np.quantile(ydm, [0.01, 0.99]))
        a_hat = float(C[tr] @ ydm / (C[tr] @ C[tr]))  # slope of (demeaned, winsorised) next-month return on the composite score
        a = max(a_hat, A_FLOOR)  # a negative / ~0 slope in the training window (2015-2020 for the 2023 fold) would FLIP the prior's sign; never allowed
        r_tr = ydm - a * C[tr]
        vic = lambda p: float(monthly_rank_ic(ctx.tm[va], p, ctx.y[va]).mean())
        te_mask, va_mask = ctx.u[te], np.ones(len(va), bool)
        rec = {"test_year": fold["year"], "a": a, "a_hat": a_hat, "val_ic_comp": vic(C[va])}
        # ---- ET on the composite's residual
        et = []
        for p in GRID:
            m = ExtraTreesRegressor(n_estimators=N_ESTIMATORS, max_features=p["max_features"], min_samples_leaf=p["min_samples_leaf"],
                                    bootstrap=False, n_jobs=-1, random_state=SEED).fit(X[tr], r_tr)
            et.append((p, m.predict(X[va]), m.predict(X[te])))
        cand = []
        for (p, ev, ete) in et:
            for s in (0.0, 0.25, 0.5, 1.0):
                cand.append(({"max_features": p["max_features"], "min_samples_leaf": p["min_samples_leaf"], "shrink": s}, s, a * C[va] + s * ev, a * C[te] + s * ete))
        b = max(((vic(c[2]),) + c for c in cand), key=lambda t: t[0])
        rec["et_resid_chosen"], rec["et_resid_val_ic"] = b[1], b[0]
        best_pred_te = b[4]
        forced = max([(vic(a * C[va] + ev), a * C[te] + ete) for (p, ev, ete) in et], key=lambda t: t[0])
        # ---- LightGBM with init_score = a * composite
        lg = []
        for p in LGB_PRIOR_GRID:
            m = lgb.LGBMRegressor(n_estimators=LGB_PRIOR_ROUNDS, learning_rate=0.05, num_leaves=p["num_leaves"], min_child_samples=p["min_child_samples"],
                                  colsample_bytree=0.5, subsample=0.5, subsample_freq=1, reg_lambda=100.0, max_bin=63, n_jobs=-1, random_state=SEED,
                                  verbose=-1).fit(X[tr], ydm, init_score=a * C[tr])
            for k in (0,) + LGB_PRIOR_CK:
                pv = a * C[va] + (m.predict(X[va], num_iteration=k) if k else 0.0)
                pt = a * C[te] + (m.predict(X[te], num_iteration=k) if k else 0.0)
                lg.append(({**p, "n_trees": k}, vic(pv), pt))
        bl = max(lg, key=lambda t: t[1])
        f50 = max([t for t in lg if t[0]["n_trees"] == 50], key=lambda t: t[1])
        rec["lgbm_init_chosen"], rec["lgbm_init_val_ic"] = bl[0], bl[1]
        # ---- ridge shrunk toward the composite weights
        Xtr_u, ytr_ = Xu[tr], ydm
        Xva_u = Xu[va]
        XtX, b0 = Xtr_u.T @ Xtr_u, a * W0
        rhs = Xtr_u.T @ (ytr_ - Xtr_u @ b0)
        rd = []
        for lam in RIDGE_LAMBDAS:
            beta = b0 if not np.isfinite(lam) else b0 + np.linalg.solve(XtX + lam * np.eye(XtX.shape[0]), rhs)
            rd.append((lam, vic(Xva_u @ beta), Xu[te] @ beta))
        br = max(rd, key=lambda t: t[1])
        rec["ridge_chosen_lambda"], rec["ridge_val_ic"] = br[0], br[1]
        # ---- H2: orthogonal-residual ML output blended with the composite (equal z-weights); config picked on validation IC of the blend
        orth_c = []
        for (p, ev, ete) in et:
            ov = orth_fast(ev, C[va], ctx.tm[va], np.ones(len(va), bool))
            blend_v = mz(C[va], ctx.tm[va], np.ones(len(va), bool)) + mz(ov, ctx.tm[va], np.ones(len(va), bool))
            orth_c.append((vic(blend_v), p, ete))
        bo = max(orth_c, key=lambda t: t[0])
        ete = bo[2]
        ot = orth_fast(ete, C[te], ctx.tm[te], te_mask)
        blend_t = mz(C[te], ctx.tm[te], te_mask) + mz(ot, ctx.tm[te], te_mask)
        rec["orth_config"], rec["orth_blend_val_ic"] = bo[1], bo[0]
        sl = fold["sl"]
        pred["comp"][sl] = C[te]
        pred["lgbm_init"][sl], pred["lgbm_init_forced50"][sl] = bl[2], f50[2]
        pred["et_resid"][sl], pred["et_resid_forced_s1"][sl] = best_pred_te, forced[1]
        pred["ridge_prior"][sl] = br[2]
        pred["orth_alone"][sl], pred["comp_plus_orth"][sl] = ot, blend_t
        info.append(rec)
        print(f"  [fold {fold['year']}] a={a:.4f} (fitted {a_hat:.4f}) val IC comp {rec['val_ic_comp']:.4f} | et_resid chosen {rec['et_resid_chosen']} ({rec['et_resid_val_ic']:.4f}) | "
              f"lgbm chosen n_trees={rec['lgbm_init_chosen']['n_trees']} ({rec['lgbm_init_val_ic']:.4f}) | ridge lambda {rec['ridge_chosen_lambda']} ({rec['ridge_val_ic']:.4f}) "
              f"| orth blend val IC {rec['orth_blend_val_ic']:.4f} | {time.time() - t0:.0f}s", flush=True)
    return pred, info
...
def main():
    ...
    base_obj = Base(ctx)
    C = grp_scores(ctx, np.arange(len(ctx.tm))).mean(axis=1)
    Xu = universe_ranked_factors(ctx)
    W0 = np.array([SIGNS[f] / (len(FACTOR_GROUPS) * len(fs)) for g, fs in FACTOR_GROUPS.items() for f in fs])
    assert list(SIGNS) == [f for fs in FACTOR_GROUPS.values() for f in fs] and np.allclose(Xu @ W0, C, atol=1e-9), "composite != ranks @ weights"
    pred, info = run_prior(ctx, C, Xu, W0)

    rows, ic_store, results = [], {}, {"fold_selection": info}
    ic_c = base_obj.ic(pred["comp"])
    lp_c = lp_stats(base_obj, pred["comp"], "comp")["lc_t10"]
    for name, p in pred.items():
        ic = base_obj.ic(p)
        ic_store[name] = ic
        pg = paired(ic, ic_c)
        ri, cor = resid_ic(base_obj, p)
        lp = lp_c if name == "comp" else lp_stats(base_obj, p, name)["lc_t10"]
        by_terc = {lab: float(base_obj.ic(p, base_obj.terc == t).mean()) for t, lab in enumerate(("small", "mid", "large"))}
        row = {"arm": name, "ic": float(ic.mean()), "ic_t": tstat(ic), "d_ic_vs_comp": pg["mean_diff"], "paired_t_vs_comp": pg["t"], "resid_ic_after_comp": float(ri.mean()),
               "resid_ic_t": tstat(ri), "rank_corr_with_comp": float(cor.mean()), "ic_small": by_terc["small"], "ic_mid": by_terc["mid"], "ic_large": by_terc["large"],
               "ir_gross": lp["ir_gross"], "ir_net": lp["ir_net"], "beta": lp["beta"], "max_dd_net_pct": lp["max_dd_net_pct"]}
        rows.append(row)
        results[name] = {**row, "ic_by_year": {int(y): float(v) for y, v in ic.groupby(ic.index.year).mean().items()}, "lp": lp}
        print(f"  {name:20s} IC {row['ic']:.4f} (t {row['ic_t']:.2f}) | vs comp {row['d_ic_vs_comp']:+.4f} (paired t {row['paired_t_vs_comp']:+.2f}) | residual IC {row['resid_ic_after_comp']:+.4f} "
              f"(t {row['resid_ic_t']:+.2f}) | corr w/ comp {row['rank_corr_with_comp']:.2f} | IR gross {lp['ir_gross']:+.2f} net {lp['ir_net']:+.2f}", flush=True)
    summ = pd.DataFrame(rows)
    summ["kill_ml_adds_lt_0.005_ic"] = (summ["d_ic_vs_comp"] < 0.005) & (summ["arm"] != "comp")
    summ["kill_orth_resid_ic_lt_0.01"] = summ["resid_ic_after_comp"] < 0.01
    summ.to_csv(OUT / "summary.csv", index=False)
    pd.DataFrame(ic_store).rename_axis("target_month").to_csv(OUT / "monthly_ic.csv")
    ch = pd.DataFrame([{"test_year": r["test_year"], "et_resid_shrink": r["et_resid_chosen"]["shrink"], "lgbm_n_trees": r["lgbm_init_chosen"]["n_trees"],
                        "ridge_lambda": r["ridge_chosen_lambda"], "slope_a_used": r["a"], "slope_a_fitted": r["a_hat"]} for r in info])
    ch.to_csv(OUT / "fold_selection.csv", index=False)
