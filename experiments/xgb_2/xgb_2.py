"""
FIAM 2026 - XGBoost, round 2: fixing the tuning and the regularization.

Follows xgb.py (experiments/xgb/README.md) without touching it. That run early-stopped on validation MSE (the 2025 fold
stopped after 4 trees), used 0.8 row/column sampling and no leaf-size floor, and lost to plain OLS
(IR 0.12 vs 0.85). Here: no early stopping -- one long, low-learning-rate (0.02) run per config, with the
NUMBER OF TREES (50..800) chosen on validation rank IC; 50% row and column subsampling; min_child_weight
{500, 5000}; lambda=100; depth {2, 4}. Feature arms: `modern_nomom` (12, headline), `modern` (15, with
momentum -- reference), `all_nomom` (139).
...
Run:  .venv/bin/python experiments/xgb_2/xgb_2.py [--arms modern_nomom,modern,all_nomom,modern_nomom_trad] [--suffix _x] [--raw-target] [--smoke]
Outputs (output/): oos_predictions_xgb2_<arm>.csv, xgb2_feature_importance_<arm>.csv,
    portfolio_holdings_<portfolio>_xgb2_<arm>.csv, portfolio_returns_<portfolio>_xgb2_<arm>.csv,
    xgb2_results.json, xgb2_summary.csv     (<arm> in modern_nomom | modern | all_nomom)
"""
...
# ---------------------------------------------------------------------------
# 5. Model: XGBoost, round 2 -- forest-style regularization, trees chosen on rank IC
# ---------------------------------------------------------------------------

MODEL_TAG = "xgb2"
ROUNDS = 800
CHECKPOINTS = (50, 100, 200, 400, 800)  # number of trees tried per config (chosen on validation IC)
# experiments/xgb/README.md's run early-stopped on validation MSE (2025 fold stopped after 4 trees) with a 6-config
# (max_depth x learning_rate) grid, 0.8 row/column sampling and no leaf-size floor. Changes here: (i) NO early
# stopping -- one long, low-learning-rate run per config, with the number of trees picked by validation rank IC
# via iteration_range; (ii) 50% row and 50% column subsampling; (iii) min_child_weight of 500 / 5000 rows per
# leaf and lambda=100; (iv) depth 2 or 4.
GRID = [{"max_depth": d, "min_child_weight": mcw} for d in (2, 4) for mcw in (500, 5000)]
...
def fit_fold(Xtr, ytr, Xva):
    ...
    for p in GRID:
        m = xgb.XGBRegressor(
            n_estimators=ROUNDS, learning_rate=0.02, max_depth=p["max_depth"], min_child_weight=p["min_child_weight"],
            subsample=0.5, colsample_bytree=0.5, reg_lambda=100.0, objective="reg:squarederror",
            tree_method="hist", max_bin=64, n_jobs=-1, random_state=SEED,
        ).fit(Xtr, ytr, verbose=False)
        imp = m.feature_importances_
        for k in CHECKPOINTS:
            cands.append({"params": {**p, "n_trees": k}, "val_pred": m.predict(Xva, iteration_range=(0, k)),
                          "predict": (lambda X, m=m, k=k: m.predict(X, iteration_range=(0, k))), "importance": imp})
    return cands
