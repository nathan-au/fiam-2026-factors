...
+ sectors; (ii) its IC largely disappears once orthogonalised to that set; (iii) it is about as good as a plain value+quality screen; (iv) its IC is concentrated in one or two groups (value, investment) with the liquidity group contributing nothing.
PRE-REGISTERED READING: if the risk-model-residual IC t-stat is < 1 the composite has no alpha beyond the shared core; if a group's Shapley value is <= 0 it should be dropped.

DESIGN. Frozen experiments/largecap harness (copied in below); frozen composite. (1) exact Shapley values of the 7 groups for universe IC (128 equal-weight sub-composites); (2) IC raw / sector-neutral / residual on {log size, 12-1 momentum, betabab, ivol, turnover,
book-to-market} + GICS sectors (month-by-month OLS on universe rows); (3) composite vs a be_me+qmj screen: rank correlation, top-quintile overlap, paired IC; (4) leave-one-group-out LP portfolios (`lc_t10`).

Run:  .venv/bin/python experiments/composite_overlap/composite_overlap.py [--smoke]
Outputs (output/): results.json, group_contributions.csv, leave_one_group_out.csv, run.log, portfolio_{holdings,returns}_{full,drop_<group>}_lc_comp.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/composite_overlap/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Driver: shared-core overlap, group Shapley values, IC definitions
# ---------------------------------------------------------------------------
from itertools import combinations
from math import factorial

RISK_STYLES = {"size": None, "momentum": "ret_12_1", "beta": "betabab_1260d", "resid_vol": "ivol_capm_21d",
               "liquidity": "turnover_126d", "value": "be_me"}  # a public, Barra-flavoured style set (None = log market cap)
QV_SCREEN = ("be_me", "qmj")  # 'simple commoditised core': value + quality


def month_ols_resid(ctx, y, design_fn):
    """Month-by-month cross-sectional OLS residual of y on the design (universe rows); design_fn(meta_rows_df) -> 2-D array."""
    m = ctx.meta
    um = ctx.um
    idx_all = np.flatnonzero(um)
    out = np.zeros(len(m))
    r2 = {}
    for mo, g in m.loc[um].groupby("target_month"):
        ii = g.index.to_numpy()
        A = design_fn(ii)
        yy = np.asarray(y)[ii]
        ...
        res = yy - A @ beta
        out[ii] = res
        r2[mo] = 1.0 - res.var() / yy.var() if yy.var() > 0 else np.nan
    return out, pd.Series(r2)


def shapley_ic(base_obj):
    G = base_obj.G7
    ...
    for k in range(1, n + 1):
        for S in combinations(range(n), k):
            v[S] = float(base_obj.ic(G[:, list(S)].mean(axis=1)).mean())
    phi = {}
    for i in range(n):
        tot = 0.0
        for k in range(n):
            for S in combinations([j for j in range(n) if j != i], k):
                ...
                tot += w * (v[tuple(sorted(S + (i,)))] - v[S])
        phi[names[i]] = tot
    loo = {names[i]: v[tuple(j for j in range(n) if j != i)] for i in range(n)}
    return phi, loo, v[tuple(range(n))]
...
def main():
    ...
    comp = base_obj.comp
    m = ctx.meta
    sector = m["sector"].to_numpy()
    res = {"n_test_months": int(len(np.unique(base_obj.months)))}

    # (1) Shapley decomposition of the composite's universe IC over its seven factor groups (equal-weight sub-composites)
    phi, loo, full = shapley_ic(base_obj)
    res["shapley_ic"] = {"values": phi, "sum": float(sum(phi.values())), "full_composite_ic": full,
                         "leave_one_group_out_ic": loo, "d_ic_when_dropped": {k: loo[k] - full for k in loo}}
    ...
    def design(ii, with_styles=True):
        cols = [np.ones(len(ii))]
        if with_styles:
            for k, c in RISK_STYLES.items():
                cols.append(np.log(m["raw_me"].to_numpy(dtype=float)[ii]) if c is None else xall_te(ctx, c)[ii])
        sec = pd.get_dummies(pd.Series(sector[ii]), drop_first=True).to_numpy(dtype=float)
        return np.column_stack(cols + [sec])

    sec_only, _ = month_ols_resid(ctx, comp, lambda ii: design(ii, False))
    resid, r2 = month_ols_resid(ctx, comp, design)
    ic_raw, ic_sec, ic_res = base_obj.ic(comp), base_obj.ic(sec_only), base_obj.ic(resid)
    res["ic_definitions"] = {"raw": {"ic": float(ic_raw.mean()), "t": tstat(ic_raw)},
                             "sector_neutral": {"ic": float(ic_sec.mean()), "t": tstat(ic_sec)},
                             "risk_model_residual (size, momentum, beta, ivol, liquidity, value + sector)": {"ic": float(ic_res.mean()), "t": tstat(ic_res)},
                             "mean_monthly_r2_of_composite_on_styles_and_sectors": float(r2.mean())}
    res["ic_by_size_tercile"] = {lab: {"raw": float(base_obj.ic(comp, base_obj.terc == t).mean()), "risk_model_residual": float(base_obj.ic(resid, base_obj.terc == t).mean())}
                                 for t, lab in enumerate(("small", "mid", "large"))}
    print("IC definitions:", json.dumps(_clean(res["ic_definitions"])), flush=True)
    print("IC by size tercile:", json.dumps(_clean(res["ic_by_size_tercile"])), flush=True)

    # (3) Is the composite just a value+quality screen? Rank correlation, top-quintile overlap, and IC of the simple screen alone
    screen = urank(xall_te(ctx, QV_SCREEN[0]), ctx) + urank(xall_te(ctx, QV_SCREEN[1]), ctx)
    ic_scr = base_obj.ic(screen)
    corr = pd.Series(rank_corr_by_month(base_obj, comp, screen))
    d = pd.DataFrame({"m": base_obj.months[ctx.um], "c": comp[ctx.um], "s": screen[ctx.um]})
    ov = d.groupby("m").apply(lambda g: (g["c"].rank(pct=True).ge(0.8) & g["s"].rank(pct=True).ge(0.8)).sum() / max(1, g["c"].rank(pct=True).ge(0.8).sum()), include_groups=False)
    res["value_quality_screen"] = {"screen_ic": float(ic_scr.mean()), "screen_ic_t": tstat(ic_scr), "composite_ic": float(ic_raw.mean()),
                                   "paired_composite_minus_screen": paired(ic_raw, ic_scr), "mean_rank_corr_composite_vs_screen": float(corr.mean()),
                                   "mean_top_quintile_overlap_share (random = 0.20)": float(ov.mean())}
    print("Composite vs value+quality screen:", json.dumps(_clean(res["value_quality_screen"])), flush=True)

    # (4) Leave-one-group-out PORTFOLIOS (LP lc_t10) - the N19 drop-liquidity ablation and the same for every group. IR is noisy (s.e. ~0.46).
    if not args.smoke or True:
        base = make_variants(FLOOR)["lc_t10"]
        rows = []
        for j, g in enumerate(FACTOR_GROUPS):
            keep = [k for k in range(len(FACTOR_GROUPS)) if k != j]
            s = base_obj.G7[:, keep].mean(axis=1)
            row, net, r = run_variant(base_obj.preds(s), f"drop_{g}", base, "comp")
            rows.append({"dropped_group": g, "ic": float(base_obj.ic(s).mean()), "ir_gross": row["ir_gross"], "ir_net": row["ir_net"], "max_dd_net_pct": row["max_dd_net_pct"],
                         "cy2025_net": row["cy2025_net"]})
            print(f"  drop {g:30s}: IC {rows[-1]['ic']:.4f} IR gross {row['ir_gross']:+.2f} net {row['ir_net']:+.2f}", flush=True)
        row, net, r = run_variant(base_obj.preds(comp), "full", base, "comp")
        rows.append({"dropped_group": "(none, full composite)", "ic": float(ic_raw.mean()), "ir_gross": row["ir_gross"], "ir_net": row["ir_net"], "max_dd_net_pct": row["max_dd_net_pct"],
                     "cy2025_net": row["cy2025_net"]})
        pd.DataFrame(rows).to_csv(OUT / "leave_one_group_out.csv", index=False)
        res["leave_one_group_out_portfolios"] = rows
    pd.DataFrame({"shapley_ic": pd.Series(phi), "leave_one_out_ic": pd.Series(loo)}).rename_axis("group").to_csv(OUT / "group_contributions.csv")
