...
DESIGN. Frozen experiments/largecap harness (copied in below); frozen composite; LP `lc_t10` monthly net returns are rescaled ex post by the gate scale (a dollar-neutral book scales linearly; a cost of 7.5bp x 2.0 gross x |change in scale| is
charged). All state variables at target month T use only data realised by the end of month T-1. Gates: G1 trailing-12m composite IC < 0; G2 z of the 3-month mean absolute return of the seven factor-group sleeves (top-minus-bottom decile of each group) > 1;
G3 z of last month's composite D10-D1 < -1; G4 z of S&P 500 21-day realised vol > 1; G5 continuous vol-scaling clip(median vol / vol, 0.5, 1.5); G6 any of G1-G3.

Run:  .venv/bin/python experiments/crowding_gate/crowding_gate.py [--smoke]
Outputs (output/): results.json, summary.csv, state_variables.csv, run.log, portfolio_{holdings,returns}_base_t10_lc_comp.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/crowding_gate/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Driver: crowding-state / confidence gates on the frozen composite book
# ---------------------------------------------------------------------------
GATE_SCALE = 0.5  # gross scaling when a gate fires (fixed in advance)
Z_THRESH = 1.0  # z-score threshold (fixed in advance, NOT tuned)
SCALE_CHANGE_COST = 2.0 * 7.5e-4  # gross 2.0 x ~7.5bp average one-way tiered cost, charged per unit change in the scale


def expanding_z(x, min_periods=24):
    mu, sd = x.expanding(min_periods=min_periods).mean(), x.expanding(min_periods=min_periods).std()
    return (x - mu) / sd


def spx_realised_vol_at_month_end():
    sp = pd.read_csv(CACHE / "SP500.csv", parse_dates=["observation_date"]).dropna(subset=["SP500"]).sort_values("observation_date")
    lr = np.log(sp["SP500"]).diff()
    sp["vol21"] = lr.rolling(21).std() * np.sqrt(252)
    sp["month"] = sp["observation_date"].values.astype("datetime64[M]")
    sp["month"] = pd.to_datetime(sp["month"]) + pd.offsets.MonthEnd(0)
    return sp.groupby("month")["vol21"].last()


def state_variables(ctx):
    """All series are indexed by the TARGET month T they would gate and use only information realised by the end of month T-1."""
    rows = np.arange(len(ctx.tm))
    G7 = grp_scores(ctx, rows)
    comp = G7.mean(axis=1)
    u = ctx.u & np.isfinite(ctx.y)
    tm = pd.to_datetime(ctx.tm)
    ic = monthly_rank_ic(tm[u].to_numpy(), comp[u], ctx.y[u])
    ic.index = pd.to_datetime(ic.index)

    def spread(score):
        d = pd.DataFrame({"m": tm[u], "s": score[u], "y": ctx.y[u]})
        d["dec"] = d.groupby("m")["s"].transform(lambda s: pd.qcut(s.rank(method="first"), 10, labels=False))
        g = d.groupby(["m", "dec"])["y"].mean().unstack()
        return g[9] - g[0]

    d10d1 = spread(comp)
    sleeves = pd.DataFrame({g: spread(G7[:, j]) for j, g in enumerate(FACTOR_GROUPS)})
    idx = d10d1.index
    st = pd.DataFrame(index=idx)
    lag = lambda s, k=1: s.shift(k)
    st["ic12"] = lag(ic.reindex(idx).rolling(12, min_periods=12).mean())  # trailing-12m realised composite IC known at T-1
    st["sleeve_abs3"] = lag(sleeves.abs().mean(axis=1).rolling(3, min_periods=3).mean())  # size of recent sleeve moves (unwind = big, correlated)
    st["book_mom"] = lag(d10d1)  # last month's realised D10-D1 of the composite itself
    vol = spx_realised_vol_at_month_end()
    st["spx_vol"] = vol.shift(1).reindex(idx)  # S&P 21-day realised vol at the end of T-1 (index i is month T; shift(1) -> month T-1)
    st["z_sleeve"], st["z_book"], st["z_vol"] = expanding_z(st["sleeve_abs3"]), expanding_z(st["book_mom"]), expanding_z(st["spx_vol"])
    st["vol_med"] = st["spx_vol"].expanding(min_periods=24).median()
    return st, d10d1


def apply_gate(net, scale):
    s = scale.reindex(net.index).fillna(1.0)
    return s * net - SCALE_CHANGE_COST * s.diff().abs().fillna(0.0), s
...
def main():
    ...
    row0, net0, res0 = run_variant(base_obj.preds(base_obj.comp), "base_t10", base, "comp")
    net0 = net0.copy()
    net0.index = pd.to_datetime(net0.index)
    st, d10d1 = state_variables(ctx)
    st.to_csv(OUT / "state_variables.csv")

    one = pd.Series(1.0, index=net0.index)
    fire = lambda cond: pd.Series(np.where(cond.reindex(net0.index).fillna(False), GATE_SCALE, 1.0), index=net0.index)
    scales = {
        "G0_ungated": one,
        "G1_trailing_IC12_lt_0": fire(st["ic12"] < 0),
        "G2_sleeve_moves_z_gt_1": fire(st["z_sleeve"] > Z_THRESH),
        "G3_book_momentum_z_lt_-1": fire(st["z_book"] < -Z_THRESH),
        "G4_spx_vol_z_gt_1": fire(st["z_vol"] > Z_THRESH),
        "G5_vol_scaled_(Moreira-Muir)": (st["vol_med"] / st["spx_vol"]).clip(0.5, 1.5).reindex(net0.index).fillna(1.0),
    }
    scales["G6_any_of_G1_G2_G3"] = pd.Series(np.minimum.reduce([scales[k].to_numpy() for k in list(scales)[1:4]]), index=net0.index)
    # The failure mode from r/algotrading 1sjicuf: a threshold TUNED on a validation slice inherits that slice's regime. Tune G1's IC cutoff on
    # 2017-2020 using the D10-D1 spread as the P&L proxy (no LP exists before 2021), then apply it unchanged to the test window.
    cands = (-0.03, -0.02, -0.01, 0.0, 0.01, 0.02, 0.03)
    val = d10d1[(d10d1.index >= "2017-01-01") & (d10d1.index <= "2020-12-31")]
    def val_sharpe(c):
        s = pd.Series(np.where(st["ic12"].reindex(val.index) < c, GATE_SCALE, 1.0), index=val.index)
        r = s * val
        return float(r.mean() / r.std()) if r.std() > 0 else -9
    best_c = max(cands, key=val_sharpe)
    print(f"  validation-slice calibration of the IC cutoff (2017-2020, D10-D1 P&L): chosen {best_c} "
          f"(Sharpe by candidate: { {c: round(val_sharpe(c), 3) for c in cands} })", flush=True)
    scales["G1cal_IC12_cutoff_tuned_on_2017_2020"] = fire(st["ic12"] < best_c)

    rows = []
    stress_ts = {k: pd.Timestamp(k + "-01") + pd.offsets.MonthEnd(0) for k in STRESS_MONTHS}
    for name, sc in scales.items():
        r, s = apply_gate(net0, sc)
        act = r - 0.04 / 12.0
        ir = float(np.sqrt(12) * act.mean() / act.std())
        dd = float(100 * drawdown_series(r.reset_index(drop=True)).min())
        cy = r.groupby(r.index.year).apply(lambda x: (1 + x).prod() - 1)
        gated = s < 1.0 - 1e-9
        row = {"gate": name, "ir_net": ir, "cagr_net_pct": float(100 * ((1 + r).prod() ** (12 / len(r)) - 1)), "max_dd_pct": dd,
               "cy2025_net_pct": float(100 * cy.get(2025, np.nan)), "share_months_gated": float(gated.mean()), "avg_scale": float(s.mean()),
               "mean_net_ret_when_gated_pct": float(100 * net0[gated].mean()) if gated.any() else None,
               "mean_net_ret_when_not_gated_pct": float(100 * net0[~gated].mean())}
        for k, ts in stress_ts.items():
            row[f"scale_{k}"] = float(s.get(ts, np.nan))
            row[f"ret_{k}_pct"] = float(100 * r.get(ts, np.nan))
        row["stress_windows_ungated_sum_pct"] = float(100 * sum(net0.get(ts, 0.0) for k, ts in stress_ts.items() if k != "2021-01"))
        row["stress_windows_gated_sum_pct"] = float(100 * sum(r.get(ts, 0.0) for k, ts in stress_ts.items() if k != "2021-01"))
        if gated.any() and (~gated).any():
            row["gated_vs_not_t"] = float(stats_t(net0[gated], net0[~gated]))
        rows.append(row)
        print(f"  [{name:38s}] IR {ir:+.2f} | maxDD {dd:.0f}% | 2025 {row['cy2025_net_pct']:+.1f}% | gated {100 * row['share_months_gated']:.0f}% of months | "
              f"unwind windows ungated {row['stress_windows_ungated_sum_pct']:+.1f}% -> gated {row['stress_windows_gated_sum_pct']:+.1f}%", flush=True)
    summ = pd.DataFrame(rows)
    summ["d_ir_vs_ungated"] = summ["ir_net"] - float(summ.loc[summ["gate"] == "G0_ungated", "ir_net"].iloc[0])
    summ.to_csv(OUT / "summary.csv", index=False)
    (OUT / "results.json").write_text(json.dumps(_clean({"rows": rows, "validation_calibrated_cutoff": best_c, "z_threshold": Z_THRESH, "gate_scale": GATE_SCALE}), indent=2))
    print(f"\n{'=' * 78}\nSUMMARY\n{'=' * 78}")
    print(summ[["gate", "ir_net", "d_ir_vs_ungated", "max_dd_pct", "cy2025_net_pct", "share_months_gated", "stress_windows_ungated_sum_pct", "stress_windows_gated_sum_pct"]].to_string(
...
def stats_t(a, b):
    ...
    return (a.mean() - b.mean()) / np.sqrt(a.var(ddof=1) / len(a) + b.var(ddof=1) / len(b)) if len(a) > 2 and len(b) > 2 else float("nan")
