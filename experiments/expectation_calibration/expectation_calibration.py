...
DESIGN. Frozen experiments/largecap harness (copied in below); frozen composite; LP `lc_t10`. Computes: book effective breadth 1/sum(w^2), the unconstrained-law IR = IC*sqrt(N_eff*12) and the implied transfer coefficient; a haircut table (0/20/40/50/57%) of expected IC and IR
(gross and net of the assumed costs); minimum detectable IC and months needed at two-sided 5% / 80% power; stationary-bootstrap (6-month blocks, 2000 draws) 5/50/95% bands for mean IC and IR; first-vs-second-half IC and a trend regression.

Run:  .venv/bin/python experiments/expectation_calibration/expectation_calibration.py [--draws 2000] [--smoke]
Outputs (output/): results.json, haircut_table.csv, run.log, portfolio_{holdings,returns}_comp_t10_lc_comp.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/expectation_calibration/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Driver: expectation calibration (live-IC haircut, fundamental-law realisation, power analysis, bootstrap uncertainty)
# ---------------------------------------------------------------------------


def stationary_bootstrap(x, n_draws, block, rng):
    n = len(x)
    ...
    for b in range(n_draws):
        ...
        for t in range(n):
            idx[t] = i
            i = rng.integers(0, n) if rng.random() < 1.0 / block else (i + 1) % n
        out[b] = x[idx]
    return out
...
def main():
    ...
    base_obj = Base(ctx)
    ic = base_obj.ic(base_obj.comp)
    base = make_variants(FLOOR)["lc_t10"]
    row, net_s, res = run_variant(base_obj.preds(base_obj.comp), "comp_t10", base, "comp")
    fr = pd.read_csv(OUT / f"portfolio_returns_comp_t10_{TAG}_comp.csv", parse_dates=["target_month"]).set_index("target_month")
    hold = pd.read_csv(OUT / f"portfolio_holdings_comp_t10_{TAG}_comp.csv", parse_dates=["target_month"])
    gross, cost = fr["port_excess_ret"], fr["trade_cost"] + fr["borrow_cost"]
    T = len(gross)
    out = {"n_months": int(T), "realised": {"ic": float(ic.mean()), "ic_sd_monthly": float(ic.std(ddof=1)), "ic_t": tstat(ic), "ir_gross": row["ir_gross"], "ir_net": row["ir_net"]}}

    # (1) Fundamental law with the book's own effective breadth: IR_ann = TC * IC * sqrt(N_eff * 12)
    n_eff = (hold.groupby("target_month")["weight"].apply(lambda w: 1.0 / ((w / w.abs().sum()) ** 2).sum())).mean()
    ...
    for h in (0.0, 0.2, 0.4, 0.5, 0.57):
        g_h = (1 - h) * gross
        act_g = g_h - hurdle
        act_n = g_h - cost - hurdle
        tab.append({"haircut_on_gross_alpha": h, "expected_ic": float(ic.mean() * (1 - h)), "expected_ir_gross": float(np.sqrt(12) * act_g.mean() / gross.std()),
                    "expected_ir_net_of_assumed_costs": float(np.sqrt(12) * act_n.mean() / gross.std())})
    out["haircut_table"] = tab
    print(pd.DataFrame(tab).to_string(index=False, float_format=lambda v: f"{v:.3f}"), flush=True)

    # (3) Power analysis: how many months to detect the haircut IC, and what is detectable with 68 months (two-sided 5%, 80% power)
    sd = float(ic.std(ddof=1))
    z = 1.96 + 0.84
    out["power"] = {"monthly_ic_sd": sd, "se_of_mean_ic_68m": sd / np.sqrt(T), "min_detectable_ic_with_T_months": float(z * sd / np.sqrt(T)),
                    "months_needed_to_detect": {str(v): float((z * sd / v) ** 2) for v in (0.05, 0.037, 0.03, 0.02, 0.015, 0.01)}}
    print("power:", json.dumps(_clean(out["power"])), flush=True)

    # (4) Stationary-bootstrap uncertainty (block 6 months) for the mean IC and the gross / net IR of this single path
    rng = np.random.default_rng(0)
    bi = stationary_bootstrap(ic.to_numpy(), args.draws, 6, rng).mean(axis=1)
    idx_draws = stationary_bootstrap(np.arange(T, dtype=float), args.draws, 6, rng).astype(int)
    gnet = (gross - cost).to_numpy()
    gg = gross.to_numpy()
    ir_g = np.sqrt(12) * ((gg[idx_draws] - hurdle).mean(axis=1)) / gg[idx_draws].std(axis=1, ddof=1)
    ir_n = np.sqrt(12) * ((gnet[idx_draws] - hurdle).mean(axis=1)) / gnet[idx_draws].std(axis=1, ddof=1)
    q = lambda a: [float(np.quantile(a, 0.05)), float(np.quantile(a, 0.5)), float(np.quantile(a, 0.95))]
    out["bootstrap_5_50_95"] = {"mean_ic": q(bi), "ir_gross": q(ir_g), "ir_net": q(ir_n), "se_ir_net": float(ir_n.std()),
                                "share_draws_ir_net_gt_0": float((ir_n > 0).mean()), "share_draws_ic_gt_0": float((bi > 0).mean())}
    print("bootstrap (5/50/95%):", json.dumps(_clean(out["bootstrap_5_50_95"])), flush=True)

    # (5) In-sample decay: first vs second half and a trend regression of the monthly IC
    h1, h2 = ic[ic.index < "2024-01-01"], ic[ic.index >= "2024-01-01"]
    tt = np.arange(len(ic))
    slope, icpt = np.polyfit(tt, ic.to_numpy(), 1)
    se = np.sqrt(np.sum((ic.to_numpy() - (slope * tt + icpt)) ** 2) / (len(ic) - 2) / np.sum((tt - tt.mean()) ** 2))
    out["decay_within_sample"] = {"ic_2021_2023": float(h1.mean()), "ic_2024_2026": float(h2.mean()), "welch_t_h1_minus_h2": float((h1.mean() - h2.mean()) / np.sqrt(h1.var(ddof=1) / len(h1) + h2.var(ddof=1) / len(h2))),
                                  "trend_slope_ic_per_month": float(slope), "trend_t": float(slope / se), "ic_by_year": {int(y): float(v) for y, v in ic.groupby(ic.index.year).mean().items()}}
    print("decay:", json.dumps(_clean(out["decay_within_sample"])), flush=True)
    (OUT / "results.json").write_text(json.dumps(_clean(out), indent=2))
    pd.DataFrame(tab).to_csv(OUT / "haircut_table.csv", index=False)
