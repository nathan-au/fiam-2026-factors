"""
FIAM 2026 - Correlation-peer return gap / peer index signals on the large-cap composite (feat_peer_gap.py).

Research origin: docs/RESEARCH.md Part I sec 3.6 item 1 (Peer Return Gap 1.26%/mo t 3.81 in China; Avramov-Ge 'Dual peer effects' JFE 2026: a Peer Index predicts returns without reversal and ML on own characteristics does not subsume it).

HYPOTHESIS. Cross-stock information (peers' recent returns) is absent from a stock's own characteristics; peers' month-t return (lead-lag) or the own-minus-peer gap adds IC to the composite in the >= $2B universe, beyond plain industry returns (control).
...
Run:  .venv/bin/python experiments/feat_peer_gap/feat_peer_gap.py [--nperm 200] [--smoke]
...
HERE = Path(__file__).resolve().parent  # experiments/feat_peer_gap/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Correlation-peer signals. At characteristic month t: among stocks with market cap >= $500M in the same FF49 industry, each stock's
# peers are the 5 with the highest correlation of monthly returns over months t-59..t (>= 36 overlapping months). Only returns
# up to month t are used; a truncation-invariance test rebuilds everything from rows <= t.
# ---------------------------------------------------------------------------
K_PEERS, MIN_OVERLAP, WINDOW, PEER_MIN_ME = 5, 36, 60, 500.0


def path_cum(R):
    return np.prod(np.stack([1.0 + R.shift(k).to_numpy() for k in range(1, 12)]), axis=0) - 1.0 + 0 * R


def peer_builder(p):
    R, ME, FF = wide(p, "ret"), wide(p, "me"), wide(p, "ff49")
    cum = path_cum(R)
    nan = lambda: pd.DataFrame(np.nan, index=R.index, columns=R.columns)
    out = {"peer_ret_1m": nan(), "peer_gap_1m": nan(), "peer_ret_12_1": nan(), "ind_ret_1m": nan()}
    for t in R.index[R.index >= pd.Timestamp("2020-12-31")]:
        i = R.index.get_loc(t)
        win = R.iloc[max(0, i - WINDOW + 1): i + 1]
        r1, c12 = R.loc[t], cum.loc[t]
        ok = (ME.loc[t] >= PEER_MIN_ME) & FF.loc[t].notna() & r1.notna()
        for f, ids in ok[ok].index.to_series().groupby(FF.loc[t][ok]).groups.items():
            ids = list(ids)
            n = len(ids)
            if n < K_PEERS + 1:
                continue
            C = win[ids].corr(min_periods=MIN_OVERLAP).to_numpy().copy()
            np.fill_diagonal(C, -np.inf)
            C = np.where(np.isfinite(C), C, -np.inf)
            top = np.argsort(-C, axis=1)[:, :K_PEERS]
            valid = np.take_along_axis(C, top, axis=1).min(axis=1) > -np.inf
            a1, a12 = r1[ids].to_numpy(), c12[ids].to_numpy()
            p1 = a1[top].mean(axis=1)
            with np.errstate(invalid="ignore"):
                p12 = a12[top].mean(axis=1)
            out["peer_ret_1m"].loc[t, ids] = np.where(valid, p1, np.nan)
            out["peer_gap_1m"].loc[t, ids] = np.where(valid, a1 - p1, np.nan)
            out["peer_ret_12_1"].loc[t, ids] = np.where(valid, p12, np.nan)
            out["ind_ret_1m"].loc[t, ids] = (a1.sum() - a1) / (n - 1)
    return out


def make_blocks(ctx):
    panel = load_panel(["ret", "me", "ff49"], ctx)
    trunc = truncation_test(peer_builder, panel, n_dates=2)
    W = peer_builder(panel)
    v = {k: wide_to_meta(w, ctx.meta) for k, w in W.items()}
    blocks = {
        "peer_ret_1m": {"raw": v["peer_ret_1m"], "sign": +1, "note": "mean month-t return of the 5 correlation peers (lead-lag / Peer Index; Avramov-Ge)"},
        "peer_gap_1m": {"raw": v["peer_gap_1m"], "sign": -1, "note": "own month-t return minus peers' (Peer Return Gap: laggards catch up)"},
        "peer_ret_12_1": {"raw": v["peer_ret_12_1"], "sign": +1, "note": "peers' 12-1 momentum"},
        "ind_ret_1m_control": {"raw": v["ind_ret_1m"], "sign": +1, "note": "CONTROL: FF49 industry mean month-t return (do correlation peers beat plain industry?)"},
    }
    return blocks, {}, trunc
