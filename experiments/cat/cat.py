"""
FIAM 2026 - CatBoost strategy.

CatBoost grows OBLIVIOUS (symmetric) trees -- one split per level shared by the whole tree -- and uses
ordered boosting, which caps per-tree capacity and is the most heavily regularized boosting variant
available; the best a priori bet for a very low signal-to-noise label. Grid: depth {4,6} x l2_leaf_reg
{30,300}, learning rate 0.03, rsm 0.5, Bernoulli subsample 0.5; trees (100..800) picked per fold on
validation rank IC. Feature arms: `modern_nomom` (12, headline), `modern` (15, with momentum -- reference),
`all_nomom` (139).
...
Run:  .venv/bin/python experiments/cat/cat.py [--arms modern_nomom,modern,all_nomom,modern_nomom_trad] [--suffix _x] [--raw-target] [--smoke]
Outputs (output/): oos_predictions_cat_<arm>.csv, cat_feature_importance_<arm>.csv,
    portfolio_holdings_<portfolio>_cat_<arm>.csv, portfolio_returns_<portfolio>_cat_<arm>.csv,
    cat_results.json, cat_summary.csv     (<arm> in modern_nomom | modern | all_nomom)
"""
...
# ---------------------------------------------------------------------------
# 5. Model: CatBoost (symmetric/oblivious trees), walk-forward, validated on rank IC
# ---------------------------------------------------------------------------

MODEL_TAG = "cat"
ITERATIONS = 800
CHECKPOINTS = (100, 200, 400, 800)  # number of trees tried per config (chosen on validation IC)
# Oblivious (symmetric) trees use ONE split per level across the whole tree, which caps a tree's capacity;
# with a low learning rate, big L2 and 50% Bernoulli row subsampling this is the most heavily regularized
# boosting variant available. rsm = share of features considered per split.
GRID = [{"depth": d, "l2_leaf_reg": l2} for d in (4, 6) for l2 in (30.0, 300.0)]


def _norm(v):
    v = np.asarray(v, dtype=float)
    return v / v.sum() if v.sum() > 0 else np.full(len(v), 1.0 / len(v))
...
def fit_fold(Xtr, ytr, Xva):
    ...
    for p in GRID:
        m = CatBoostRegressor(
            iterations=ITERATIONS, learning_rate=0.03, depth=p["depth"], l2_leaf_reg=p["l2_leaf_reg"], rsm=0.5,
            bootstrap_type="Bernoulli", subsample=0.5, border_count=64, loss_function="RMSE",
            thread_count=-1, random_seed=SEED, verbose=False, allow_writing_files=False,
        ).fit(Xtr, ytr)
        imp = _norm(m.get_feature_importance())
        for k in CHECKPOINTS:
            cands.append({"params": {**p, "n_trees": k}, "val_pred": m.predict(Xva, ntree_end=k),
                          "predict": (lambda X, m=m, k=k: m.predict(X, ntree_end=k)), "importance": imp})
    return cands
