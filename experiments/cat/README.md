...
Implementation: `cat.py` (run with `.venv/bin/python experiments/cat/cat.py`). New file; no existing file edited.
...
- All-edges selection (see §1). Single seed; assumed costs; `all_nomom` not run. Same PM caveats as `experiments/rf_3/README.md` §5.

## Reproduce

```
.venv/bin/python experiments/cat/cat.py                                              # modern_nomom, modern  -> cat_results.json / cat_summary.csv
.venv/bin/python experiments/cat/cat.py --arms modern_nomom_trad,modern_trad --suffix _trad   # tradeable-universe training -> cat_results_trad.json
```
