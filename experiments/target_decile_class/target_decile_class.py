...
(validation IC is measured on the RAW return for every arm), LP portfolios. Families: Extra-Trees (classifier vs the harness's
regressor), forest-style LightGBM (multiclass vs regressor). Labels are within-month deciles over universe rows.

Run:  .venv/bin/python experiments/target_decile_class/target_decile_class.py [--models et,lgbm] [--seed 42] [--smoke]
...
HERE = Path(__file__).resolve().parent  # experiments/target_decile_class/
OUTPUT = HERE / "output"
...
# Arms: `control` = regression on the winsorised raw return (same family, same features). The classifier arms fit the
# within-month decile label (0..9) of next-month excess return and score by the probability-weighted expected decile.
ARMS = {
    "control": {"target": "raw", "winsor": True},
    "clf_expdec": {"target": "decile", "winsor": False,
                   "fitters": {"et": make_fit_et_clf("expdec"), "lgbm": make_fit_lgbm_clf("expdec")}},
    "clf_top_minus_bottom": {"target": "decile", "winsor": False,
                             "fitters": {"et": make_fit_et_clf("top_minus_bottom"), "lgbm": make_fit_lgbm_clf("top_minus_bottom")}},
    "clf_argmax": {"target": "decile", "winsor": False,  # ablation: hard class prediction (should be worse than expected value)
                   "fitters": {"et": make_fit_et_clf("argmax"), "lgbm": make_fit_lgbm_clf("argmax")}},
}
ALT_SCORES = []
