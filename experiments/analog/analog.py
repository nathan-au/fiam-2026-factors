"""
FIAM 2026 - Analog forecasting ("lookalike" stocks): a memory-based, no-fitted-model strategy.

Idea: instead of fitting a function from characteristics to returns, remember every historical stock-month whose
next-month return is already known (the training window) and forecast a stock by what happened next to its
nearest LOOKALIKES -- the K historical stock-months closest to it in characteristic-rank space, from ANY stock and
ANY earlier month. Prediction = (weighted) mean of the lookalikes' next-month return, each measured RELATIVE to
...
Design choices (all documented in experiments/analog/README.md):
  ...
  * Memory bank: rows whose target month is before the fold's validation window (`tr` of every other script) --
    never validation or test rows. For `<arm>_inv` the bank contains only LP-investable stock-months (price >= $5,
...
# ---------------------------------------------------------------------------
# 5. Analog (k-nearest-lookalike) forecasting
# ---------------------------------------------------------------------------

MODEL_TAG = "analog"
KS = (25, 100, 400, 1600)
WEIGHTINGS = ("uniform", "inverse_distance")
KMAX = max(KS)
CHUNK = 5000
...
def knn_predict(nn, Xq, y_bank, bank_inv, ks=KS):
    """Lookalike forecasts for every query row, for every (K, weighting); also the mean distance to the nearest 100
    lookalikes and the share of those 100 that are LP-investable (diagnostics)."""
    out = {(k, w): np.empty(len(Xq)) for k in ks for w in WEIGHTINGS}
    mean_d, share_inv = np.empty(len(Xq)), np.empty(len(Xq))
    kmax = max(ks)
    for s in range(0, len(Xq), CHUNK):
        d, ix = nn.kneighbors(Xq[s:s + CHUNK], n_neighbors=kmax)
        yy = y_bank[ix]
        for k in ks:
            out[(k, "uniform")][s:s + CHUNK] = yy[:, :k].mean(axis=1)
            w = 1.0 / (d[:, :k] + 1e-3)
            out[(k, "inverse_distance")][s:s + CHUNK] = (yy[:, :k] * w).sum(axis=1) / w.sum(axis=1)
        mean_d[s:s + CHUNK] = d[:, :100].mean(axis=1)
        share_inv[s:s + CHUNK] = bank_inv[ix[:, :100]].mean(axis=1)
    return out, mean_d, share_inv


def run_walk_forward_analog(ctx, features, arm, label):
    X = ctx.Xall[:, ctx.cols(features)]
    inv_only = arm.endswith("_inv")
    pred = np.zeros(len(ctx.meta))
    diag = np.zeros((len(ctx.meta), 2))
    fold_info = []
    for fold in ctx.folds:
        t0 = time.time()
        bank = fold["tr"][ctx.inv[fold["tr"]]] if inv_only else fold["tr"]
        va = fold["va"][ctx.inv[fold["va"]]] if inv_only else fold["va"]
        yb = ctx.y_dm[bank]
        yb = np.clip(yb, *np.quantile(yb, [0.01, 0.99]))
        nn = NearestNeighbors(n_neighbors=KMAX, algorithm="auto", n_jobs=-1).fit(X[bank])
        vp, _, _ = knn_predict(nn, X[va], yb, ctx.inv[bank])
        months_va, yva = ctx.tm[va], ctx.y[va]
        cands = [{"K": k, "weights": w, "val_ic": float(monthly_rank_ic(months_va, vp[(k, w)], yva).mean())} for (k, w) in vp]
        best = max(cands, key=lambda c: c["val_ic"])
        tp, md, si = knn_predict(nn, X[fold["te"]], yb, ctx.inv[bank], ks=(best["K"],))
        pred[fold["sl"]] = tp[(best["K"], best["weights"])]
        diag[fold["sl"], 0], diag[fold["sl"], 1] = md, si
        fold_info.append({"test_year": fold["year"], "chosen": {"K": best["K"], "weights": best["weights"]}, "val_ic": best["val_ic"],
                          "n_bank": int(len(bank)), "n_val": int(len(va)), "candidates": cands})
        print(f"  [{label} fold {fold['year']}] bank={len(bank):,} chosen K={best['K']} {best['weights']} val_ic={best['val_ic']:.4f} "
              f"(candidates {min(c['val_ic'] for c in cands):.4f}..{max(c['val_ic'] for c in cands):.4f}) {time.time() - t0:.0f}s", flush=True)
    return pred, diag, fold_info


def run_arm_analog(ctx, features, arm):
    print(f"\n{'=' * 78}\nMODEL {MODEL_TAG} | ARM {arm} ({len(features)} characteristics)\n{'=' * 78}", flush=True)
    t0 = time.time()
    pred_vec, diag, fold_info = run_walk_forward_analog(ctx, features, arm, f"{MODEL_TAG}/{arm}")
    preds = preds_frame(ctx, pred_vec)
    keep = ((ctx.meta["target_month"] >= OOS_START) & (ctx.meta["target_month"] <= OOS_END)).to_numpy()
    diag = diag[keep]
    preds.to_csv(OUT / f"oos_predictions_{MODEL_TAG}_{arm}.csv", index=False)
    pinv = investable_mask(preds)
    ic_all = monthly_rank_ic(preds["target_month"].to_numpy(), preds["pred"].to_numpy(), preds[TARGET_COL].to_numpy())
    ic_inv = monthly_rank_ic(preds.loc[pinv, "target_month"].to_numpy(), preds.loc[pinv, "pred"].to_numpy(), preds.loc[pinv, TARGET_COL].to_numpy())
    model_stats = {
        "n_features": len(features), "oos_r2_pct": 100 * oos_r2(preds[TARGET_COL].values, preds["pred"].values),
        "test_rank_ic": float(ic_all.mean()), "test_rank_ic_share_positive": float((ic_all > 0).mean()),
        "test_rank_ic_investable": float(ic_inv.mean()), "test_rank_ic_investable_t": float(ic_inv.mean() / ic_inv.std() * np.sqrt(len(ic_inv))),
        "mean_val_rank_ic": float(np.mean([f["val_ic"] for f in fold_info])), "pred_rank_autocorr": pred_rank_autocorr(preds),
        "mean_dist_to_100_lookalikes_investable_queries": float(diag[pinv, 0].mean()),
        "mean_dist_to_100_lookalikes_all_queries": float(diag[:, 0].mean()),
        "share_of_lookalikes_that_are_investable__investable_queries": float(diag[pinv, 1].mean()),
        "share_of_lookalikes_that_are_investable__non_investable_queries": float(diag[~pinv, 1].mean()),
    }
    print(f"  model-level: {json.dumps(_clean(model_stats))}", flush=True)
    out = {"features": features, "model_stats": model_stats, "fold_selection": fold_info, "variants": {}}
    rows = []
    for name, cfg in VARIANTS.items():
        res = evaluate_variant(preds, name, cfg, MODEL_TAG, arm)
        out["variants"][name] = res
        rows.append(summary_row(MODEL_TAG, arm, name, res, model_stats))
        g, n = res["gross"], res["net"]
        print(f"  [{name:8s}] gross IR {g['information_ratio']:.2f} / net IR {n['information_ratio']:.2f} | beta {g['beta']:+.2f} (t {g['beta_tstat']:+.2f}) "
              f"roll12 [{g['rolling_beta_12m_min']:+.2f},{g['rolling_beta_12m_max']:+.2f}] | turnover {100 * res['drift_adjusted_one_way_turnover_pct_of_gross']:.0f}% | "
              f"maxDD {100 * g['max_drawdown']:.0f}% | pos {g['min_n_positions']}-{g['max_n_positions']}", flush=True)
    print(f"  arm done in {(time.time() - t0) / 60:.1f} min", flush=True)
    return out, rows
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser()
    ap.add_argument("--arms", default="modern_nomom,modern_nomom_inv,modern")
    ap.add_argument("--suffix", default="")
    ap.add_argument("--smoke", action="store_true")
    args = ap.parse_args()
    if args.smoke:
        OUT = OUTPUT / "_smoke_analog"
        OUT.mkdir(exist_ok=True)
    print("Loading model table...", flush=True)
    df, stock_vars = load_model_table()
    if args.smoke:
        df = df[df["permno"] % 4 == 0].reset_index(drop=True)
    # Return relative to the month's cross-sectional median across ALL stocks (used only for lookalikes' returns).
    df["y_dm"] = df[TARGET_COL] - df.groupby("target_month")[TARGET_COL].transform("median")
    df = cross_sectional_rank_transform(df, stock_vars)
    ctx = Context(df, stock_vars, max_folds=1 if args.smoke else None)
    ctx.y_dm = df["y_dm"].to_numpy(dtype=np.float64)
    ctx.inv = investable_mask(df)
    del df
    arms = {"modern": MODERN_FEATURES, "modern_nomom": MODERN_NOMOM_FEATURES, "modern_inv": MODERN_FEATURES, "modern_nomom_inv": MODERN_NOMOM_FEATURES}
    results, all_rows = {}, []
    for arm in args.arms.split(","):
        results[arm], rows = run_arm_analog(ctx, arms[arm], arm)
        all_rows += rows
        (OUT / f"{MODEL_TAG}_results{args.suffix}.json").write_text(json.dumps(_clean(results), indent=2))
        pd.DataFrame(all_rows).to_csv(OUT / f"{MODEL_TAG}_summary{args.suffix}.csv", index=False)
    summ = pd.DataFrame(all_rows)
    cols = ["arm", "portfolio", "ir_gross", "ir_net", "beta", "roll_beta_max", "turnover_one_way_pct_gross", "max_dd_net_pct", "test_rank_ic", "mean_val_rank_ic"]
    print(f"\n{'=' * 78}\nSUMMARY {MODEL_TAG}\n{'=' * 78}")
    print(summ[cols].to_string(index=False, float_format=lambda v: f"{v:.2f}"))
