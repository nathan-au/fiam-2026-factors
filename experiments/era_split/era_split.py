...
code section. Arms: orig / era_avg / era_softmin / era_dir (eras = months) / era_dir_size (eras = month x size tercile) and the same with init_score = a*composite (`*_init`; a = fitted slope floored at +0.005 so the prior cannot flip sign - a first run without the floor was invalid, see README). It is NOT the upstream scikit-learn-erasplit fork.

Run:  .venv/bin/python experiments/era_split/era_split.py [--seed 42] [--arms orig,era_dir,...] [--smoke]
...
HERE = Path(__file__).resolve().parent  # experiments/era_split/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Experiment code: Era Splitting (arXiv 2309.14496) re-implemented in a small numpy histogram GBDT.
# The upstream scikit-learn-erasplit fork is not used (needs a source build of a patched scikit-learn); the split CRITERIA are re-implemented
# from the paper's description (only the abstract and the repo README were read), so this tests the idea, not the fork.
#   orig        pooled second-order gain (standard gradient-boosted trees) -- the control, same code path as the others
#   era_avg     sum over eras of each era's own gain
#   era_softmin Boltzmann soft-minimum over eras of each era's per-row gain (a split must help in EVERY era)
#   era_dir     directional: an era's gain counts + if its left-vs-right direction agrees with the pooled direction, - if not
# ---------------------------------------------------------------------------
from scipy.special import logsumexp

N_BINS = 20
A_FLOOR = 0.005
ERA_CFG = dict(depth=3, n_rounds=250, lr=0.03, lam=100.0, min_leaf=300, colsample=0.5, subsample=0.5, checkpoints=(50, 100, 150, 250))


class Tree:
    def __init__(self):
        self.nodes = []  # (feat, bin, left, right) or (None, None, value)
    ...
    def predict(self, Xb):
        ...
        while stack:
            ...
            if nd[0] is None:
                out[idx] = nd[2]
                continue
            go_left = Xb[idx, nd[0]] <= nd[1]
            stack.append((nd[2], idx[go_left]))
            stack.append((nd[3], idx[~go_left]))
        return out


def _split_scores(G, H, criterion, lam, min_leaf, n_eras):
    """G, H: (E, F, B) era-wise gradient / count histograms of a node. Returns (F, B-1) scores (-inf where invalid)."""
    Gp, Hp = G.sum(0), H.sum(0)
    GLp, HLp = np.cumsum(Gp, 1)[:, :-1], np.cumsum(Hp, 1)[:, :-1]
    Gt, Ht = Gp.sum(1, keepdims=True), Hp.sum(1, keepdims=True)
    GRp, HRp = Gt - GLp, Ht - HLp
    valid = (HLp >= min_leaf) & (HRp >= min_leaf)
    gain = lambda gl, hl, gr, hr, gt, ht, l: gl ** 2 / (hl + l) + gr ** 2 / (hr + l) - gt ** 2 / (ht + l)
    if criterion == "orig":
        s = gain(GLp, HLp, GRp, HRp, Gt, Ht, lam)
    else:
        GL, HL = np.cumsum(G, 2)[:, :, :-1], np.cumsum(H, 2)[:, :, :-1]
        Ge, He = G.sum(2, keepdims=True), H.sum(2, keepdims=True)
        GR, HR = Ge - GL, He - HL
        le = lam / n_eras
        ge = gain(GL, HL, GR, HR, Ge, He, le)  # (E, F, B-1), >= 0 in every era for every split
        if criterion == "era_avg":
            s = ge.sum(0)
        elif criterion == "era_softmin":
            u = ge / (He + 1.0)
            alpha = 1.0 / (np.mean(u[:, valid]) + 1e-15) if valid.any() else 1.0
            s = -(logsumexp(-alpha * u, axis=0) - np.log(u.shape[0])) / alpha * u.shape[0] * float(np.mean(He))
        elif criterion == "era_dir":
            # residual r = -g, so the left/right mean residual difference is -(GL/HL) + (GR/HR)
            dl = -(GL / (HL + 1e-9)) + (GR / (HR + 1e-9))
            dp = -(GLp / (HLp + 1e-9)) + (GRp / (HRp + 1e-9))
            agree = np.sign(dl) * np.sign(dp)[None]
            s = (ge * agree).sum(0)
        else:
...
def fit_tree(Xb, g, era, n_eras, rows, feats, criterion, cfg, rng):
    B, tree = N_BINS, Tree()
    Fs = len(feats)
    ...
    def build(idx, depth):
        ...
        gs = g[idx]
        leaf = lambda: tree.nodes.__setitem__(node_id, (None, None, float(-gs.sum() / (len(idx) + cfg["lam"]))))
        if depth == cfg["depth"] or len(idx) < 2 * cfg["min_leaf"]:
            leaf()
            return node_id
        key = ((era[idx][:, None] * Fs + np.arange(Fs)[None, :]) * B + Xb[idx][:, feats]).ravel()
        size = n_eras * Fs * B
        G = np.bincount(key, weights=np.repeat(gs, Fs), minlength=size).reshape(n_eras, Fs, B)
        H = np.bincount(key, minlength=size).reshape(n_eras, Fs, B).astype(float)
        sc = _split_scores(G, H, criterion, cfg["lam"], cfg["min_leaf"], n_eras)
        if not np.isfinite(sc).any() or sc.max() <= 0:
            leaf()
            return node_id
        fi, bi = np.unravel_index(int(np.argmax(sc)), sc.shape)
        f = feats[fi]
        left = Xb[idx, f] <= bi
        l_id = build(idx[left], depth + 1)
        r_id = build(idx[~left], depth + 1)
        tree.nodes[node_id] = (int(f), int(bi), l_id, r_id)
        return node_id
...
def fit_era_gbm(Xb, y, era, criterion, init, cfg, seed, Xeval):
    """Boosted squared-error trees on (y - init). Returns {k: predictions on each array in Xeval after k trees} for the checkpoints."""
    rng = np.random.default_rng(seed)
    n, F = Xb.shape
    n_eras = int(era.max()) + 1
    pred = np.zeros(n)  # boosted part
    per_tree = [[] for _ in Xeval]
    for t in range(cfg["n_rounds"]):
        g = (init + pred) - y
        rows = np.sort(rng.choice(n, int(n * cfg["subsample"]), replace=False))
        feats = np.sort(rng.choice(F, max(1, int(F * cfg["colsample"])), replace=False))
        tr = fit_tree(Xb, g, era, n_eras, rows, feats, criterion, cfg, rng)
        pred += cfg["lr"] * tr.predict(Xb)
        for j, Xe in enumerate(Xeval):
            per_tree[j].append(cfg["lr"] * tr.predict(Xe))
    cum = [np.cumsum(np.array(p), axis=0) for p in per_tree]
    return {k: [c[k - 1] if k > 0 else np.zeros(len(c[0])) for c in cum] for k in (0,) + tuple(cfg["checkpoints"])}


def bin_features(X):
    return np.clip(((X + 1.0) / 2.0 * N_BINS).astype(np.int64), 0, N_BINS - 1).astype(np.uint8)


ARMS = {  # arm: (criterion, era definition, composite init)
    "orig": ("orig", "month", False),
    "era_avg": ("era_avg", "month", False),
    "era_softmin": ("era_softmin", "month", False),
    "era_dir": ("era_dir", "month", False),
    "era_dir_size": ("era_dir", "month_x_size", False),  # H1: eras = month x size tercile
    "orig_init": ("orig", "month", True),
    "era_dir_init": ("era_dir", "month", True),
    "era_dir_size_init": ("era_dir", "month_x_size", True),
}


def size_tercile_all(ctx):
    d = pd.DataFrame({"m": ctx.tm, "me": ctx.raw_me})[ctx.u]
    t = d.groupby("m")["me"].transform(lambda s: pd.qcut(s.rank(method="first"), 3, labels=False))
    out = np.zeros(len(ctx.tm), dtype=int)
    out[t.index.to_numpy()] = t.to_numpy()
    return out
...
def main():
    ...
    Xb_all = bin_features(ctx.Xall[:, ctx.cols(FEATURES)])
    terc = size_tercile_all(ctx)
    n_te = sum(len(f["te"]) for f in ctx.folds)
    arms = args.arms.split(",")
    pred = {a: np.zeros(n_te) for a in arms}
    pred["comp"] = np.zeros(n_te)
    info = {a: [] for a in arms}
    cfg = ERA_CFG
    for fold in ctx.folds:
        ...
        a_hat = float(C[tr] @ ydm / (C[tr] @ C[tr]))
        a_slope = max(a_hat, A_FLOOR)  # the composite prior must keep its pre-specified sign (a ~0 / negative training slope would flip it)
        pred["comp"][fold["sl"]] = C[te]
        mon = pd.factorize(ctx.tm[tr])[0]
        for arm in arms:
            t1 = time.time()
            crit, era_def, use_init = ARMS[arm]
            era = mon if era_def == "month" else mon * 3 + terc[tr]
            era = pd.factorize(era)[0]
            init = a_slope * C[tr] if use_init else np.zeros(len(tr))
            out = fit_era_gbm(Xb_all[tr], ydm, era, crit, init, cfg, SEED, [Xb_all[va], Xb_all[te]])
            base_va = a_slope * C[va] if use_init else np.zeros(len(va))
            base_te = a_slope * C[te] if use_init else np.zeros(len(te))
            best = None
            for k, (pv, pt) in out.items():
                if k == 0 and not use_init:
                    continue
                ic = float(monthly_rank_ic(ctx.tm[va], base_va + pv, ctx.y[va]).mean())
                if best is None or ic > best[0]:
                    best = (ic, k, base_te + pt)
            pred[arm][fold["sl"]] = best[2]
            info[arm].append({"test_year": fold["year"], "n_trees": best[1], "val_ic": best[0], "n_eras": int(era.max() + 1), "a_used": a_slope, "a_fitted": a_hat})
            print(f"  [fold {fold['year']}] {arm:18s} n_eras={int(era.max() + 1):3d} chosen n_trees={best[1]:3d} val_ic={best[0]:.4f} ({time.time() - t1:.0f}s)", flush=True)
    ic_store, rows, results = {}, [], {"fold_selection": info, "config": cfg, "n_bins": N_BINS}
    ic_c = base_obj.ic(pred["comp"])
    for name in ["comp"] + arms:
        p = pred[name]
        ic = base_obj.ic(p)
        ic_store[name] = ic
        lp = lp_stats(base_obj, p, name)["lc_t10"]
        pg = paired(ic, ic_c)
        ri, cor = resid_ic(base_obj, p) if name != "comp" else (pd.Series([np.nan]), pd.Series([np.nan]))
        rows.append({"arm": name, "ic": float(ic.mean()), "ic_t": tstat(ic), "d_ic_vs_comp": pg["mean_diff"], "paired_t_vs_comp": pg["t"],
                     "resid_ic_after_comp": float(ri.mean()), "ic_small": float(base_obj.ic(p, base_obj.terc == 0).mean()),
                     "ic_mid": float(base_obj.ic(p, base_obj.terc == 1).mean()), "ic_large": float(base_obj.ic(p, base_obj.terc == 2).mean()),
                     "ir_gross": lp["ir_gross"], "ir_net": lp["ir_net"], "beta": lp["beta"], "max_dd_net_pct": lp["max_dd_net_pct"],
                     "avg_trees_chosen": float(np.mean([i["n_trees"] for i in info[name]])) if name in info else None})
        print(f"  {name:18s} IC {rows[-1]['ic']:.4f} (t {rows[-1]['ic_t']:.2f}) vs comp {rows[-1]['d_ic_vs_comp']:+.4f} (paired t {rows[-1]['paired_t_vs_comp']:+.2f}) | "
              f"IR gross {lp['ir_gross']:+.2f} net {lp['ir_net']:+.2f} | avg trees {rows[-1]['avg_trees_chosen']}", flush=True)
    summ = pd.DataFrame(rows)
    orig = summ.loc[summ["arm"] == "orig", "ic"].iloc[0] if "orig" in arms else np.nan
    summ["paired_t_vs_orig"] = [paired(ic_store[a], ic_store["orig"])["t"] if "orig" in ic_store else np.nan for a in summ["arm"]]
    summ["d_ic_vs_orig"] = summ["ic"] - orig
