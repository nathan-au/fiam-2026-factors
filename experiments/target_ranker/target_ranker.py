...
settings; arms `rank_ndcg`, `rank_pairwise`, and `rankic_pairwise` = my re-implementation of a rank-IC-weighted pairwise objective as
a custom XGBoost objective, since only the paper's abstract was read) and `lgbm` (control forest-style LGBMRegressor; arm `lambdarank`).
Labels: within-month decile 0..9 on universe rows. Candidate selection uses validation rank IC on the raw return.

Run:  .venv/bin/python experiments/target_ranker/target_ranker.py [--models xgb,lgbm] [--seed 42] [--smoke]
...
HERE = Path(__file__).resolve().parent  # experiments/target_ranker/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Classification and learning-to-rank fitters (labels = within-month decile 0..9 of next-month excess return, computed
# on universe rows; the gaussianised rank is used by the pairwise IC objective).
# ---------------------------------------------------------------------------
from sklearn.ensemble import ExtraTreesClassifier

CLF_ROUNDS, CLF_CHECKPOINTS = 300, (100, 200, 300)


def _score_from_proba(P, classes, how):
    if how == "expdec":  # probability-weighted expected decile (NOT argmax; cf. Quantitativo "probabilistic momentum")
        return P @ classes
    if how == "argmax":
        return classes[np.argmax(P, axis=1)].astype(float) + 1e-3 * (P @ classes)  # tiny tie-break
    if how == "top_minus_bottom":
        return P[:, -1] - P[:, 0]
    raise ValueError(how)


def make_fit_et_clf(how):
    def fit(Xtr, ytr, Xva, aux):
        cands = []
        for p in GRID:
            m = ExtraTreesClassifier(n_estimators=N_ESTIMATORS, max_features=p["max_features"],
                                     min_samples_leaf=p["min_samples_leaf"], bootstrap=False, n_jobs=-1,
                                     random_state=SEED).fit(Xtr, ytr.astype(int))
            f = lambda X, m=m: _score_from_proba(m.predict_proba(X), m.classes_.astype(float), how)
            cands.append({"params": {**p, "score": how}, "val_pred": f(Xva), "predict": f,
                          "importance": m.feature_importances_})
        return cands
    return fit


def make_fit_lgbm_clf(how):
    def fit(Xtr, ytr, Xva, aux):
        cands, nf = [], Xtr.shape[1]
        for p in LGB_GRID:
            m = lgb.LGBMClassifier(objective="multiclass", n_estimators=CLF_ROUNDS, learning_rate=0.02,
                                   num_leaves=p["num_leaves"], min_child_samples=p["min_child_samples"],
                                   colsample_bytree=0.5, subsample=0.5, subsample_freq=1, extra_trees=True,
                                   reg_lambda=100.0, max_bin=63, n_jobs=-1, random_state=SEED, verbose=-1).fit(Xtr, ytr.astype(int))
            g = m.booster_.feature_importance(importance_type="gain").astype(float)
            imp = g / g.sum() if g.sum() > 0 else np.full(nf, 1.0 / nf)
            cl = m.classes_.astype(float)
            for k in CLF_CHECKPOINTS:
                f = lambda X, m=m, k=k: _score_from_proba(m.predict_proba(X, num_iteration=k), cl, how)
                cands.append({"params": {**p, "n_trees": k, "score": how}, "val_pred": f(Xva), "predict": f, "importance": imp})
        return cands
    return fit


def _group_sort(m_tr):
    order = np.argsort(m_tr, kind="stable")
    _, counts = np.unique(m_tr[order], return_counts=True)
    return order, counts


XGB_GRID = [{"max_depth": d} for d in (2, 4)]
XGB_ROUNDS, XGB_CHECKPOINTS = 400, (100, 200, 400)


def make_fit_xgb_rank(objective):
    """xgboost.XGBRanker with monthly query groups. Labels: decile 0..9 (linear NDCG gain for rank:ndcg)."""
    def fit(Xtr, ytr, Xva, aux):
        order, counts = _group_sort(aux["m_tr"])
        Xs, ys = Xtr[order], ytr[order].astype(int)
        cands, nf = [], Xtr.shape[1]
        for p in XGB_GRID:
            m = xgb.XGBRanker(objective=objective, n_estimators=XGB_ROUNDS, learning_rate=0.05, max_depth=p["max_depth"],
                              subsample=0.5, colsample_bytree=0.5, reg_lambda=100.0, min_child_weight=1.0,
                              tree_method="hist", n_jobs=8, random_state=SEED, lambdarank_pair_method="topk"
                              if objective == "rank:ndcg" else "mean", lambdarank_num_pair_per_sample=8,
                              ndcg_exp_gain=False).fit(Xs, ys, group=counts)
            g = m.get_booster().get_score(importance_type="total_gain")
            imp = np.array([g.get(f"f{i}", 0.0) for i in range(nf)])
            imp = imp / imp.sum() if imp.sum() > 0 else np.full(nf, 1.0 / nf)
            for k in XGB_CHECKPOINTS:
                f = lambda X, m=m, k=k: m.predict(X, iteration_range=(0, k))
                cands.append({"params": {**p, "n_trees": k, "objective": objective}, "val_pred": f(Xva), "predict": f, "importance": imp})
        return cands
    return fit


def make_fit_lgbm_lambdarank():
    def fit(Xtr, ytr, Xva, aux):
        order, counts = _group_sort(aux["m_tr"])
        Xs, ys = Xtr[order], ytr[order].astype(int)
        cands, nf = [], Xtr.shape[1]
        for p in LGB_GRID:
            m = lgb.LGBMRanker(objective="lambdarank", n_estimators=LGB_ROUNDS, learning_rate=0.02, num_leaves=p["num_leaves"],
                               min_child_samples=p["min_child_samples"], colsample_bytree=0.5, subsample=0.5, subsample_freq=1,
                               extra_trees=True, reg_lambda=100.0, max_bin=63, n_jobs=-1, random_state=SEED, verbose=-1,
                               label_gain=list(range(10)), lambdarank_truncation_level=100).fit(Xs, ys, group=counts)
            imp = _lgb_gain(m, nf)
            for k in LGB_CHECKPOINTS:
                f = lambda X, m=m, k=k: m.predict(X, num_iteration=k)
                cands.append({"params": {**p, "n_trees": k, "objective": "lambdarank"}, "val_pred": f(Xva), "predict": f, "importance": imp})
        return cands
    return fit


def make_fit_xgb_rankic():
    """Rank-IC-weighted pairwise objective, a re-implementation in the spirit of LambdaRankIC (arXiv 2605.00501; only the
    abstract was read). For a pair (i, j) with target rank r_i > r_j the gain in Spearman IC from putting i above j is
    proportional to |r_i - r_j| * |rank(p_i) - rank(p_j)| (swapping two predicted ranks changes sum(rank_p * rank_y) by exactly
    (rp_i - rp_j)(ry_i - ry_j)); the lambda is that weight times the logistic pair probability. Pairs are subsampled
    (K partners per row per round) for speed. Custom XGBoost objective; the regressor's raw score is the prediction."""
    K = 12
    ...
    def fit(Xtr, ytr, Xva, aux):
        ...
        gy = ctx_gauss_rank(ytr[order], starts)  # gaussianised within-group rank of the label (labels are deciles: use their ranks)
        rng = np.random.default_rng(SEED)

        def obj(pred, dtrain):
            grad, hess = np.zeros_like(pred), np.full_like(pred, 1e-3)
            for a, b in zip(starts[:-1], starts[1:]):
                n = b - a
                p, r = pred[a:b], gy[a:b]
                rp = (np.argsort(np.argsort(p)) + 0.5) / n  # current predicted rank in (0,1)
                i = np.repeat(np.arange(n), K)
                j = rng.integers(0, n, size=n * K)
                d = r[i] - r[j]
                keep = d != 0
                i, j, d = i[keep], j[keep], d[keep]
                sgn = np.sign(d)  # +1 if i should be above j
                s = (p[i] - p[j]) * sgn
                rho = 1.0 / (1.0 + np.exp(np.clip(s, -30, 30)))  # prob the pair is mis-ordered
                w = np.abs(d) * (np.abs(rp[i] - rp[j]) + 0.05) * n / K
                lam = w * rho * sgn
                np.add.at(grad, a + i, -lam)
                np.add.at(grad, a + j, lam)
                h = w * rho * (1 - rho)
                np.add.at(hess, a + i, h)
                np.add.at(hess, a + j, h)
            return grad, np.maximum(hess, 1e-3)

        cands, nf = [], Xtr.shape[1]
        dtr = xgb.DMatrix(Xs, label=np.zeros(len(Xs)))
        for p in XGB_GRID:
            booster = xgb.train({"max_depth": p["max_depth"], "eta": 0.05, "subsample": 0.5, "colsample_bytree": 0.5,
                                 "lambda": 100.0, "min_child_weight": 1.0, "tree_method": "hist", "nthread": 8,
                                 "seed": SEED, "base_score": 0.0}, dtr, num_boost_round=XGB_ROUNDS, obj=obj)
            g = booster.get_score(importance_type="total_gain")
            imp = np.array([g.get(f"f{i}", 0.0) for i in range(nf)])
            imp = imp / imp.sum() if imp.sum() > 0 else np.full(nf, 1.0 / nf)
            for k in XGB_CHECKPOINTS:
                f = lambda X, b=booster, k=k: b.predict(xgb.DMatrix(X), iteration_range=(0, k))
                cands.append({"params": {**p, "n_trees": k, "objective": "rankic_pairwise"}, "val_pred": f(Xva), "predict": f, "importance": imp})
        return cands
    return fit


def ctx_gauss_rank(y_sorted, starts):
    """Within-group Gaussianised rank of y (ties averaged) for group boundaries `starts`."""
    out = np.zeros(len(y_sorted))
    for a, b in zip(starts[:-1], starts[1:]):
        r = pd.Series(y_sorted[a:b]).rank(method="average").to_numpy()
        out[a:b] = norm.ppf((r - 0.5) / (b - a))
    return out


def fit_xgb_reg(Xtr, ytr, Xva, aux):
    """Same booster settings as the XGBRanker arms, squared-error objective on the fit target (the xgb family's control)."""
    cands, nf = [], Xtr.shape[1]
    for p in XGB_GRID:
        m = xgb.XGBRegressor(n_estimators=XGB_ROUNDS, learning_rate=0.05, max_depth=p["max_depth"], subsample=0.5,
                             colsample_bytree=0.5, reg_lambda=100.0, min_child_weight=50.0, tree_method="hist",
                             n_jobs=8, random_state=SEED).fit(Xtr, ytr)
        g = m.get_booster().get_score(importance_type="total_gain")
        imp = np.array([g.get(f"f{i}", 0.0) for i in range(nf)])
        imp = imp / imp.sum() if imp.sum() > 0 else np.full(nf, 1.0 / nf)
        for k in XGB_CHECKPOINTS:
            f = lambda X, m=m, k=k: m.predict(X, iteration_range=(0, k))
            cands.append({"params": {**p, "n_trees": k}, "val_pred": f(Xva), "predict": f, "importance": imp})
    return cands


FITTERS["xgb"] = fit_xgb_reg


# Arms. Families: `xgb` (control = XGBRegressor, same booster settings), `lgbm` (control = forest-style LGBMRegressor).
# Ranker arms use monthly query groups on the decile label (0..9); `rankic_pairwise` uses the gaussianised rank as label.
ARMS = {
    "control": {"target": "raw", "winsor": True},
    "rank_ndcg": {"target": "decile", "winsor": False, "fitters": {"xgb": make_fit_xgb_rank("rank:ndcg")}},
    "rank_pairwise": {"target": "decile", "winsor": False, "fitters": {"xgb": make_fit_xgb_rank("rank:pairwise")}},
    "lambdarank": {"target": "decile", "winsor": False, "fitters": {"lgbm": make_fit_lgbm_lambdarank()}},
    "rankic_pairwise": {"target": "rank_gauss", "winsor": False, "fitters": {"xgb": make_fit_xgb_rankic()}},
}
ALT_SCORES = []
MODELS_DEFAULT = "xgb,lgbm"
