"""
FIAM 2026 - FINRA short-interest blocks on the large-cap composite (feat_short_interest.py).

Research origin: docs/RESEARCH.md Part I sec 3.7 (short interest: surprise in short interest negatively predicts the cross-section; 'more valuable as a short-selection / squeeze-risk filter given our Jan-2021 loss').

HYPOTHESIS. Heavily shorted stocks and stocks with rising short interest underperform (informed short sellers); as extra composite groups they add IC, and they identify short-leg names to avoid.

EXTERNAL DATA (Round D of docs/RESEARCH.md Part I sec 4; FIAM sec 4 permits external data joined on a dated identifier): FINRA bi-monthly short interest (mid-month settlement file per characteristic month), cache/finra_short_interest/; symbols matched through the panel ticker.
...
Run:  .venv/bin/python experiments/feat_short_interest/feat_short_interest.py [--nperm 200] [--smoke]      (needs network on the first run to fill the cache)
...
HERE = Path(__file__).resolve().parent  # experiments/feat_short_interest/
OUTPUT = HERE / "output"
