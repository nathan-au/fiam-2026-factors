...
Run:  .venv/bin/python experiments/macro_condition/macro_condition.py [--models et,lgbm] [--seed 42] [--smoke]      (network on the first run for FRED)
Outputs (output/): results.json, summary.csv, monthly_ic.csv, run.log, oos_predictions_universe_<model>__<arm>.csv, portfolio_{holdings,returns}_<variant>_lc_<model>__<arm>.csv
"""
...
HERE = Path(__file__).resolve().parent  # experiments/macro_condition/
OUTPUT = HERE / "output"
...
# ---------------------------------------------------------------------------
# Macro-state conditioning: month-level features (same value for every stock) added to the tree models' inputs, so trees can condition
# firm-factor relations on the macro state (Gu-Kelly-Xiu style). EXTERNAL DATA: FRED T10Y2Y (10y-2y slope) and BAA10Y (Baa - 10y spread), downloaded
# on first run to cache/ (like TB3MS.csv / SP500.csv), and VIXCLS (VIX close). Value used for characteristic month t =
# last observation on or before month-end t (known at t).
# ---------------------------------------------------------------------------
import urllib.request

MACRO = {"ycurve_10y2y": "T10Y2Y", "credit_baa10y": "BAA10Y", "vix": "VIXCLS"}


def fred_series(sid):
    f = CACHE / f"{sid}.csv"
    if not f.exists():
        req = urllib.request.Request(f"https://fred.stlouisfed.org/graph/fredgraph.csv?id={sid}", headers={"User-Agent": "fiam-2026-research-bot"})
        with urllib.request.urlopen(req, timeout=60) as r:
            f.write_bytes(r.read())
    d = pd.read_csv(f, parse_dates=["observation_date"]).rename(columns={sid: "v"})
    d["v"] = pd.to_numeric(d["v"], errors="coerce")
    return d.dropna().set_index("observation_date")["v"]


def month_end_value(series, month_ends):
    s = series.sort_index()
    return s.reindex(s.index.union(month_ends)).ffill().reindex(month_ends).to_numpy()


def add_macro_features(ctx):
    """Append the macro columns to ctx.Xall for every context row (aligned through the row's characteristic month = target month - 1)."""
    char_month = (pd.to_datetime(ctx.tm) - pd.offsets.MonthEnd(1))
    uniq = pd.DatetimeIndex(sorted(set(char_month)))
    cols = {}
    for name, sid in MACRO.items():
        cols[name] = pd.Series(month_end_value(fred_series(sid), uniq), index=uniq)
    # PLACEBOS: the yield-curve slope with its values permuted ACROSS MONTHS (same marginal distribution, one value per month, no relation to the calendar):
    # if the trees gain as much from a shuffled slope, the gain is regime memorisation, not macro information.
    slope = cols["ycurve_10y2y"]
    for k in (1, 2, 3):
        perm = np.random.default_rng(1000 + k).permutation(len(slope))
        cols[f"placebo{k}_shuffled_ycurve"] = pd.Series(slope.to_numpy()[perm], index=uniq)
    extra = np.column_stack([cols[k].reindex(char_month).to_numpy() for k in cols]).astype(np.float32)
    assert np.isfinite(extra).all(), "macro features have gaps"
    base = ctx.Xall.shape[1]
    ctx.Xall = np.column_stack([ctx.Xall, extra])
    ctx.col_idx.update({k: base + i for i, k in enumerate(cols)})
    return list(cols)


ARMS = {
    "control": {"extra": []},
    "plus_ycurve": {"extra": ["ycurve_10y2y"]},
    "plus_credit": {"extra": ["credit_baa10y"]},
    "plus_vix": {"extra": ["vix"]},
    "plus_all_macro": {"extra": ["ycurve_10y2y", "credit_baa10y", "vix"]},
    "placebo1": {"extra": ["placebo1_shuffled_ycurve"]},
    "placebo2": {"extra": ["placebo2_shuffled_ycurve"]},
    "placebo3": {"extra": ["placebo3_shuffled_ycurve"]},
}
...
def main():
    ...
    macro_names = add_macro_features(ctx)
    print(f"  macro features appended: {macro_names}; distinct months in training data: {len(set(ctx.tm))}", flush=True)
    ...
    for model in args.models.split(","):
        for arm, spec in ARMS.items():
            key = f"{model}__{arm}"
            t0 = time.time()
            print(f"\n{'=' * 78}\n{key}  (extra features {spec['extra']}, seed={SEED})\n{'=' * 78}", flush=True)
            pred, imp, fold_info = run_wf(ctx, FEATURES + spec["extra"], ctx.y, model, key, winsor=True)
            stats, ic, preds = score_arm(ctx, pred, key, variants, OUT)
            stats["fold_selection"] = fold_info
            stats["macro_importance"] = {k: float(imp.get(k, 0.0)) for k in spec["extra"]}
            results[key], ics[key] = stats, ic
            g = stats[HEADLINE_VARIANT]
            print(f"  IC {stats['universe_rank_ic']:.4f} (t {stats['universe_rank_ic_t']:.2f}) | lc_t10 IR gross {g['ir_gross']:.2f} net {g['ir_net']:.2f} | macro importance "
                  f"{ {k: round(v, 3) for k, v in stats['macro_importance'].items()} } | {time.time() - t0:.0f}s", flush=True)
    ...
    for model in args.models.split(","):
        ...
        for arm in ARMS:
            key = f"{model}__{arm}"
            pg = paired(ics[key], ics[ck]) if arm != "control" else {"mean_diff": 0.0, "t": np.nan}
            g = results[key][HEADLINE_VARIANT]
            rows.append({"key": key, "ic": results[key]["universe_rank_ic"], "ic_t": results[key]["universe_rank_ic_t"], "d_ic_vs_control": pg["mean_diff"], "paired_t": pg["t"],
                         "ir_gross_t10": g["ir_gross"], "ir_net_t10": g["ir_net"], "macro_importance_sum": float(sum(results[key]["macro_importance"].values()))})
