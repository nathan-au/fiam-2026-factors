"""
FIAM 2026 - Nonlinear ML strategy: XGBoost.

Self-contained sibling of ols.py (not imported from it, per project convention
...
being a schedule placeholder.

Pipeline:
  1. Load the monthly characteristics panel and the 147 predictor columns.
  2. Cross-sectionally median-fill + rank-transform each predictor to [-1, 1]
     within each characteristic month.
  3. Walk forward: expanding training window, 2-year rolling validation
     window, one calendar year of test. Refit annually, predict monthly.
     Splits are assigned by TARGET month. Unlike ols.py, validation is used
     here: a small hyperparameter grid (max_depth x learning_rate) is fit on
     the training fold with early stopping against the validation fold, and
     the combination with lowest validation MSE is kept -- coefficients
...
# ---------------------------------------------------------------------------
# 1. Load data and build the model table (ported from ols.py, unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2. Expanding-window walk-forward XGBoost, tuned on the validation fold
# ---------------------------------------------------------------------------
...
def fit_best_xgb(
    ...
    """Grid-search (max_depth, learning_rate) with early stopping against the
    validation fold; return the model with lowest validation MSE. Trees are
    always fit on X_train/y_train only -- validation is used purely to pick
    the hyperparameters and the early-stopping iteration, never to fit
    coefficients, per docs/FIAM.md's split of duties."""
    best_model = None
    best_params = None
    best_val_mse = np.inf
    ...
    for params in PARAM_GRID:
        ...
        if val_mse < best_val_mse:
            ...
            best_model = model
            best_params = {**params, "best_iteration": int(model.best_iteration)}

    return best_model, best_params, best_val_mse


def run_walk_forward(df: pd.DataFrame, stock_vars: list[str]) -> tuple[pd.DataFrame, list[dict]]:
    ...
    for test_year in test_years:
        ...
        X_val = val[stock_vars].values
        y_val = val[TARGET_COL].values

        model, params, val_mse = fit_best_xgb(X_train, y_train, X_val, y_val)
        ...
        importances = pd.Series(model.feature_importances_, index=stock_vars)
        ...
        print(
            ...
            f"best_params=depth={params['max_depth']} lr={params['learning_rate']} "
            f"n_trees={params['best_iteration']} val_mse={val_mse:.6f}"
        )
...
def build_beta_neutral_portfolio(preds: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    construction -- identical LP to ols.py, still constrained on beta_60m
    (docs/NEXT.md flags betabab_1260d as a follow-up swap, not done here).
    Each month, solve a small linear program that maximizes expected
    (predicted) return subject to
        sum(w) = 0                      (dollar/net neutral)
        sum(w_i * beta_60m_i) = 0       (beta neutral)
...
def compute_short_book_characteristics(holdings_df: pd.DataFrame) -> dict:
    ...
    of the short leg. No direct borrow-cost/hard-to-borrow data is provided
    in this dataset, so small market cap is used as the stated proxy for
    "plausibly hard to borrow"."""
...
if __name__ == "__main__":
    ...
    print("Walk-forward expanding-window XGBoost (validation-tuned)...")
    preds, avg_importance, fold_params = run_walk_forward(df, stock_vars)
    preds = preds[(preds["target_month"] >= OOS_START) & (preds["target_month"] <= OOS_END)]
    preds.to_csv(OUTPUT / "oos_predictions_xgb.csv", index=False)

    avg_importance.index.name = "variable"
    avg_importance.rename("gain_importance").to_csv(OUTPUT / "xgb_feature_importance.csv")
    print("\nTop 15 features by average gain importance across folds:")
    for name, val in avg_importance.head(15).items():
        print(f"  {name}: {val:.4f}")
    ...
    all_results = {
        ...
        "fold_hyperparameters": fold_params,
    }
    ...
    bn_holdings.to_csv(OUTPUT / "portfolio_holdings_beta_neutral_xgb.csv", index=False)

    print("Computing performance vs T-bill+4% and S&P 500...")
    bn_results = compute_performance(bn_stats, tag="beta_neutral_xgb")
